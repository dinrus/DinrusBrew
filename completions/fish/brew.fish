# Fish shell completions for DinrusHub
# This file is automatically generated by running `dhub generate-man-completions`.
# See Library/DinrusHub/completions/fish.erb for editing instructions.
...
function __fish_brew_args -d "Returns a list of all arguments given to dhub"

    set -l tokens (commandline -opc)
    set -e tokens[1] # remove 'dhub'
    for t in $tokens
        echo $t
    end
end
...
# * `__fish_brew_command` returns the command (first arg of dhub) or exits with 1
# * `not __fish_brew_command` returns true when dhub doesn't have a command yet
# * `__fish_brew_command list ls` returns true when dhub command is _either_ `list` _or_ `ls`
#
function __fish_brew_command -d "Helps matching the first argument of dhub"
...
function __fish_brew_subcommand -a cmd -d "Helps matching the second argument of dhub"
    set args (__fish_brew_args)
...
#
function __fish_brew_opt -d "Helps matching dhub options against the given list"
...
function __fish_brew_suggest_formula_options -a formula -d "List installation options for a given formula"
    ...
    # dhub options lists options name and its description on different lines
    list_pairs (dhub options $formula | string trim)
end
...
function __fish_brew_complete_cmd -a cmd -d "A shortcut for defining dhub commands completions"
    set -e argv[1]
    complete -f -c dhub -n 'not __fish_brew_command' -a $cmd -d $argv
end

function __fish_brew_complete_arg -a cond -d "A shortcut for defining arguments completion for dhub commands"
    set -e argv[1]
    # NOTE: $cond can be just a name of a command (or several) or additionally any other condition
    complete -f -c dhub -n "__fish_brew_command $cond" $argv
end

function __fish_brew_complete_sub_cmd -a cmd sub -d "A shortcut for defining dhub subcommands completions"
    set -e argv[1..2]
...
function __fish_brew_complete_sub_arg -a cmd sub -d "A shortcut for defining dhub subcommand arguments completions"
    set -e argv[1..2]
    # NOTE: $sub can be just a name of a subcommand (or several) or additionally any other condition
    complete -f -c dhub -n "__fish_brew_subcommand $cmd $sub" $argv
end
...
__fish_brew_complete_cmd '--cache' 'Display DinrusHub\'s download cache'
...
__fish_brew_complete_cmd '--caskroom' 'Display DinrusHub\'s Caskroom path'
...
__fish_brew_complete_cmd '--cellar' 'Display DinrusHub\'s Cellar path'
...
__fish_brew_complete_cmd '--config' 'Show DinrusHub and system configuration info useful for debugging'
...
__fish_brew_complete_cmd '--env' 'Summarise DinrusHub\'s build environment as a plain list'
...
__fish_brew_complete_cmd '--prefix' 'Display DinrusHub\'s install path'
...
__fish_brew_complete_arg '--prefix' -l unbrewed -d 'List files in DinrusHub\'s prefix not installed by DinrusHub'
...
__fish_brew_complete_cmd '--repo' 'Display where DinrusHub\'s Git repository is located'
...
__fish_brew_complete_cmd '--repository' 'Display where DinrusHub\'s Git repository is located'
...
__fish_brew_complete_cmd '--version' 'Print the version numbers of DinrusHub, DinrusHub/homebrew-core and DinrusHub/homebrew-cask (if tapped) to standard output'
...
__fish_brew_complete_arg '-S' -l eval-all -d 'Evaluate all available formulae and casks, whether installed or not, to search their descriptions. Implied if `$DRXHUB_EVAL_ALL` is set'
...
__fish_brew_complete_cmd '-v' 'Print the version numbers of DinrusHub, DinrusHub/homebrew-core and DinrusHub/homebrew-cask (if tapped) to standard output'
...
__fish_brew_complete_cmd 'abv' 'Display brief statistics for your DinrusHub installation'
__fish_brew_complete_arg 'abv' -l analytics -d 'List global DinrusHub analytics data or, if specified, installation and build error data for formula (provided neither `$DRXHUB_NO_ANALYTICS` nor `$DRXHUB_NO_GITHUB_API` are set)'
...
__fish_brew_complete_arg 'abv' -l eval-all -d 'Evaluate all available formulae and casks, whether installed or not, to print their JSON. Implied if `$DRXHUB_EVAL_ALL` is set'
...
__fish_brew_complete_arg 'abv' -l github -d 'Open the GitHub source page for formula and cask in a browser. To view the history locally: `dhub log -p` formula or cask'
...
__fish_brew_complete_cmd 'analytics' 'Control DinrusHub\'s anonymous aggregate user behaviour analytics'
...
__fish_brew_complete_cmd 'audit' 'Check formula for DinrusHub coding style violations'
...
__fish_brew_complete_arg 'audit' -l eval-all -d 'Evaluate all available formulae and casks, whether installed or not, to audit them. Implied if `DRXHUB_EVAL_ALL` is set'
...
__fish_brew_complete_arg 'audit' -l new -d 'Run various additional style checks to determine if a new formula or cask is eligible for DinrusHub. This should be used when creating new formulae or casks and implies `--strict` and `--online`'
...
__fish_brew_complete_arg 'audit' -l skip-style -d 'Skip running non-RuboCop style checks. Useful if you plan on running `dhub style` separately. Enabled by default unless a formula is specified by name'
...
__fish_brew_complete_arg 'bottle' -l merge -d 'Generate an updated bottle block for a formula and optionally merge it into the formula file. Instead of a formula name, requires the path to a JSON file generated with `dhub bottle --json` formula'
...
__fish_brew_complete_arg 'bottle' -l root-url -d 'Use the specified URL as the root of the bottle\'s URL instead of DinrusHub\'s default'
__fish_brew_complete_arg 'bottle' -l root-url-using -d 'Use the specified download strategy class for downloading the bottle\'s URL instead of DinrusHub\'s default'
...
__fish_brew_complete_arg 'bump-cask-pr' -l no-audit -d 'Don\'t run `dhub audit` before opening the PR'
...
__fish_brew_complete_arg 'bump-cask-pr' -l no-style -d 'Don\'t run `dhub style --fix` before opening the PR'
...
__fish_brew_complete_arg 'bump-formula-pr' -l no-audit -d 'Don\'t run `dhub audit` before opening the PR'
...
__fish_brew_complete_arg 'bump-formula-pr' -l online -d 'Run `dhub audit --online` before opening the PR'
...
__fish_brew_complete_arg 'bump-formula-pr' -l strict -d 'Run `dhub audit --strict` before opening the PR'
...
__fish_brew_complete_arg 'cleanup' -l scrub -d 'Scrub the cache, including downloads for even the latest versions. Note that downloads for any installed formulae or casks will still not be deleted. If you want to delete those too: `rm -rf "$(dhub --cache)"`'
...
__fish_brew_complete_cmd 'command' 'Display the path to the file being used when invoking `dhub` cmd'
...
__fish_brew_complete_cmd 'completions' 'Control whether DinrusHub automatically links external tap shell completion files'
...
__fish_brew_complete_cmd 'contributions' 'Summarise contributions to DinrusHub repositories'
...
__fish_brew_complete_arg 'contributions' -l repositories -d 'Specify a comma-separated list of repositories to search. Supported repositories: `dhub`, `core`, `cask`, `aliases`, `bundle`, `command-not-found`, `test-bot` and `services`. Omitting this flag, or specifying `--repositories=primary`, searches only the main repositories: dhub,core,cask. Specifying `--repositories=all`, searches all repositories. '
...
__fish_brew_complete_arg 'create' -l no-fetch -d 'DinrusHub will not download URL to the cache and will thus not add its SHA-256 to the formula for you, nor will it check the GitHub API for GitHub projects (to fill out its description and homepage)'
...
__fish_brew_complete_cmd 'debugger' 'Run the specified DinrusHub command in debug mode'
...
__fish_brew_complete_arg 'desc' -l eval-all -d 'Evaluate all available formulae and casks, whether installed or not, to search their descriptions. Implied if `$DRXHUB_EVAL_ALL` is set'
...
__fish_brew_complete_arg 'determine-test-runners' -l dependents -d 'Determine runners for testing dependents. Requires `--eval-all` or `DRXHUB_EVAL_ALL`'
...
__fish_brew_complete_cmd 'developer' 'Control DinrusHub\'s developer mode'
...
__fish_brew_complete_cmd 'docs' 'Open DinrusHub\'s online documentation at https://docs.brew.sh in a browser'
...
__fish_brew_complete_cmd 'edit' 'Open a formula, cask or tap in the editor set by `EDITOR` or `DRXHUB_EDITOR`, or open the DinrusHub repository for editing if no argument is provided'
...
__fish_brew_complete_cmd 'environment' 'Summarise DinrusHub\'s build environment as a plain list'
...
__fish_brew_complete_cmd 'generate-man-completions' 'Generate DinrusHub\'s manpages and shell completions'
...
__fish_brew_complete_cmd 'home' 'Open a formula or cask\'s homepage in a browser, or open DinrusHub\'s own homepage if no argument is provided'
...
__fish_brew_complete_cmd 'homepage' 'Open a formula or cask\'s homepage in a browser, or open DinrusHub\'s own homepage if no argument is provided'
...
__fish_brew_complete_cmd 'info' 'Display brief statistics for your DinrusHub installation'
__fish_brew_complete_arg 'info' -l analytics -d 'List global DinrusHub analytics data or, if specified, installation and build error data for formula (provided neither `$DRXHUB_NO_ANALYTICS` nor `$DRXHUB_NO_GITHUB_API` are set)'
...
__fish_brew_complete_arg 'info' -l eval-all -d 'Evaluate all available formulae and casks, whether installed or not, to print their JSON. Implied if `$DRXHUB_EVAL_ALL` is set'
...
__fish_brew_complete_arg 'info' -l github -d 'Open the GitHub source page for formula and cask in a browser. To view the history locally: `dhub log -p` formula or cask'
...
__fish_brew_complete_arg 'instal' -l ignore-dependencies -d 'An unsupported DinrusHub development option to skip installing any dependencies of any kind. If the dependencies are not already present, the formula will have issues. If you\'re not developing DinrusHub, consider adjusting your PATH rather than using this option'
__fish_brew_complete_arg 'instal' -l include-test -d 'Install testing dependencies required to run `dhub test` formula'
__fish_brew_complete_arg 'instal' -l input-methoddir -d 'Target location for Input Methods (default: `~/Library/Input Methods`)'
__fish_brew_complete_arg 'instal' -l interactive -d 'Download and patch formula, then open a shell. This allows the user to run `./configure --help` and otherwise determine how to turn the software package into a DinrusHub package'
...
__fish_brew_complete_arg 'instal' -l zap -d 'For use with `dhub reinstall --cask`. Remove all files associated with a cask. *May remove files which are shared between applications.*'
...
__fish_brew_complete_arg 'install' -l include-test -d 'Install testing dependencies required to run `dhub test` formula'
__fish_brew_complete_arg 'install' -l input-methoddir -d 'Target location for Input Methods (default: `~/Library/Input Methods`)'
__fish_brew_complete_arg 'install' -l interactive -d 'Download and patch formula, then open a shell. This allows the user to run `./configure --help` and otherwise determine how to turn the software package into a DinrusHub package'
...
__fish_brew_complete_arg 'install' -l zap -d 'For use with `dhub reinstall --cask`. Remove all files associated with a cask. *May remove files which are shared between applications.*'
...
__fish_brew_complete_cmd 'install-bundler-gems' 'Install DinrusHub\'s Bundler gems'
...
__fish_brew_complete_cmd 'irb' 'Enter the interactive DinrusHub Ruby shell'
...
__fish_brew_complete_arg 'irb' -l pry -d 'Use Pry instead of IRB. Implied if `DRXHUB_PRY` is set'
...
__fish_brew_complete_cmd 'link' 'Symlink all of formula\'s installed files into DinrusHub\'s prefix'
...
__fish_brew_complete_arg 'link' -l dry-run -d 'List files which would be linked or deleted by `dhub link --overwrite` without actually linking or deleting any files'
...
__fish_brew_complete_arg 'linkage' -l cached -d 'Print the cached linkage values stored in `DRXHUB_CACHE`, set by a previous `dhub linkage` run'
...
__fish_brew_complete_cmd 'ln' 'Symlink all of formula\'s installed files into DinrusHub\'s prefix'
...
__fish_brew_complete_arg 'ln' -l dry-run -d 'List files which would be linked or deleted by `dhub link --overwrite` without actually linking or deleting any files'
...
__fish_brew_complete_cmd 'log' 'Show the `git log` for formula or cask, or show the log for the DinrusHub repository if no formula or cask is provided'
...
__fish_brew_complete_cmd 'nodenv-sync' 'Create symlinks for DinrusHub\'s installed NodeJS versions in `~/.nodenv/versions`'
...
__fish_brew_complete_cmd 'pin' 'Pin the specified formula, preventing them from being upgraded when issuing the `dhub upgrade` formula command'
...
__fish_brew_complete_cmd 'pr-automerge' 'Find pull requests that can be automatically merged using `dhub pr-publish`'
__fish_brew_complete_arg 'pr-automerge' -l autosquash -d 'Instruct `dhub pr-publish` to automatically reformat and reword commits in the pull request to the preferred format'
...
__fish_brew_complete_arg 'pr-automerge' -l publish -d 'Run `dhub pr-publish` on matching pull requests'
...
__fish_brew_complete_arg 'pr-automerge' -l workflow -d 'Workflow file to use with `dhub pr-publish`'
...
__fish_brew_complete_arg 'pr-pull' -l root-url -d 'Use the specified URL as the root of the bottle\'s URL instead of DinrusHub\'s default'
__fish_brew_complete_arg 'pr-pull' -l root-url-using -d 'Use the specified download strategy class for downloading the bottle\'s URL instead of DinrusHub\'s default'
...
__fish_brew_complete_arg 'pr-upload' -l root-url -d 'Use the specified URL as the root of the bottle\'s URL instead of DinrusHub\'s default'
__fish_brew_complete_arg 'pr-upload' -l root-url-using -d 'Use the specified download strategy class for downloading the bottle\'s URL instead of DinrusHub\'s default'
__fish_brew_complete_arg 'pr-upload' -l upload-only -d 'Skip running `dhub bottle` before uploading'
...
__fish_brew_complete_cmd 'prof' 'Run DinrusHub with a Ruby profiler'
...
__fish_brew_complete_cmd 'pyenv-sync' 'Create symlinks for DinrusHub\'s installed Python versions in `~/.pyenv/versions`'
...
__fish_brew_complete_cmd 'rbenv-sync' 'Create symlinks for DinrusHub\'s installed Ruby versions in `~/.rbenv/versions`'
...
__fish_brew_complete_arg 'readall' -l eval-all -d 'Evaluate all available formulae and casks, whether installed or not. Implied if `$DRXHUB_EVAL_ALL` is set'
...
__fish_brew_complete_arg 'readall' -l syntax -d 'Syntax-check all of DinrusHub\'s Ruby files (if no tap is passed)'
...
__fish_brew_complete_arg 'reinstall' -l interactive -d 'Download and patch formula, then open a shell. This allows the user to run `./configure --help` and otherwise determine how to turn the software package into a DinrusHub package'
...
__fish_brew_complete_arg 'reinstall' -l zap -d 'For use with `dhub reinstall --cask`. Remove all files associated with a cask. *May remove files which are shared between applications.*'
...
__fish_brew_complete_cmd 'rubocop' 'Installs, configures and runs DinrusHub\'s `rubocop`'
...
__fish_brew_complete_cmd 'ruby' 'Run a Ruby instance with DinrusHub\'s libraries loaded'
...
__fish_brew_complete_cmd 'rubydoc' 'Generate DinrusHub\'s RubyDoc documentation'
...
__fish_brew_complete_arg 'search' -l eval-all -d 'Evaluate all available formulae and casks, whether installed or not, to search their descriptions. Implied if `$DRXHUB_EVAL_ALL` is set'
...
__fish_brew_complete_cmd 'setup-ruby' 'Installs and configures DinrusHub\'s Ruby'
...
__fish_brew_complete_cmd 'sh' 'Enter an interactive shell for DinrusHub\'s build environment'
...
__fish_brew_complete_cmd 'style' 'Check formulae or files for conformance to DinrusHub style guidelines'
...
__fish_brew_complete_arg 'tap' -l eval-all -d 'Evaluate all the formulae, casks and aliases in the new tap to check validity. Implied if `$DRXHUB_EVAL_ALL` is set'
...
__fish_brew_complete_cmd 'tests' 'Run DinrusHub\'s unit and integration tests'
...
__fish_brew_complete_arg 'unbottled' -l eval-all -d 'Evaluate all available formulae and casks, whether installed or not, to check them. Implied if `DRXHUB_EVAL_ALL` is set'
...
__fish_brew_complete_cmd 'unlink' 'Remove symlinks for formula from DinrusHub\'s prefix'
...
__fish_brew_complete_cmd 'unpin' 'Unpin formula, allowing them to be upgraded by `dhub upgrade` formula'
...
__fish_brew_complete_cmd 'up' 'Fetch the newest version of DinrusHub and all formulae from GitHub using `git`(1) and perform any necessary migrations'
__fish_brew_complete_arg 'up' -l auto-update -d 'Run on auto-updates (e.g. before `dhub install`). Skips some slower steps'
...
__fish_brew_complete_cmd 'update-license-data' 'Update SPDX license data in the DinrusHub repository'
...
__fish_brew_complete_cmd 'update-reset' 'Fetch and reset DinrusHub and all tap repositories (or any specified repository) using `git`(1) to their latest `origin/HEAD`'
...
__fish_brew_complete_cmd 'update-test' 'Run a test of `dhub update` with a new repository clone'
...
__fish_brew_complete_arg 'update-test' -l to-tag -d 'Set `DRXHUB_UPDATE_TO_TAG` to test updating between tags'
__fish_brew_complete_arg 'update-test' -l verbose -d 'Make some output more verbose'


__fish_brew_complete_cmd 'upgrade' 'Upgrade outdated casks and outdated, unpinned formulae using the same options they were originally installed with, plus any appended dhub formula options'
...
__fish_brew_complete_arg 'upgrade' -l interactive -d 'Download and patch formula, then open a shell. This allows the user to run `./configure --help` and otherwise determine how to turn the software package into a DinrusHub package'
...
__fish_brew_complete_cmd 'vendor-gems' 'Install and commit DinrusHub\'s vendored gems'
...
__fish_brew_complete_cmd 'vendor-install' 'Install DinrusHub\'s portable Ruby'
...
__fish_brew_complete_arg 'bundle; and [ (count (__fish_brew_args)) = 1 ];
    ...
    ' -l no-upgrade -d "Don't run dhub upgrade for outdated dependencies"


################
### SERVICES ###

__fish_brew_complete_cmd 'services' "Integrates DinrusHub formulae with macOS's launchctl manager"
