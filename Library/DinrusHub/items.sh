homebrew-items() {
  ...
  # DRXHUB_MACOS is set by brew.sh
  # shellcheck disable=SC2154
  if [[ -n "${DRXHUB_MACOS}" ]]
  then
    sed_extended_regex_flag="-E"
    find_args=("-E" "${DRXHUB_REPOSITORY}/Library/Taps")
  else
    sed_extended_regex_flag="-r"
    find_args=("${DRXHUB_REPOSITORY}/Library/Taps" "-regextype" "posix-extended")
  fi

  # DRXHUB_REPOSITORY is set by brew.sh
  # shellcheck disable=SC2154
  [[ -d "${DRXHUB_REPOSITORY}/Library/Taps" ]] || return
  items="$(
    find "${find_args[@]}" \
      -type d \( \
