...
module DinrusHub
  ...
  module Diagnostic
    ...
    class Checks
      ...
      ############# @!group HELPERS
      # Finds files in `DRXHUB_PREFIX` *and* /usr/local.
      # Specify paths relative to a prefix, e.g. "include/foo.h".
      # Sets @found for your convenience.
      def find_relative_paths(*relative_paths)
        @found = [DRXHUB_PREFIX, "/usr/local"].uniq.reduce([]) do |found, prefix|
      ...
      def please_create_pull_requests(what = "unsupported configuration")
        <<~EOS
          ...
          It is expected behaviour that DinrusHub will be buggy and slow.
          Do not create any issues about this on DinrusHub's GitHub repositories.
          Do not create any issues even if you think this message is unrelated.
          Any opened issues will be immediately closed without response.
          Do not ask for help from DinrusHub or its maintainers on social media.
          You may ask for help in DinrusHub's discussions but are unlikely to receive a response.
      ...
      def examine_git_origin(repository_path, desired_origin)
        ...
        if current_origin.nil?
          <<~EOS
            ...
            Without a correctly configured origin, DinrusHub won't update
          ...
          <<~EOS
            ...
            With a non-standard origin, DinrusHub won't update properly.
      ...
      def broken_tap(tap)
        ...
        repo = GitRepository.new(DRXHUB_REPOSITORY)
      ...
      def check_for_installed_developer_tools
        ...
        <<~EOS
          Не установлены инструменты разработки.
      ...
      def check_for_stray_dylibs
        ...
        __check_stray_files "/usr/local/lib", "*.dylib", allow_list, <<~EOS
          ...
          building DinrusHub formulae and may need to be deleted.
      ...
      def check_for_broken_symlinks
        ...
        inject_file_list broken_symlinks, <<~EOS
          Broken symlinks were found. Remove them with `dhub cleanup`:
        EOS
      end

      def check_tmpdir_sticky_bit
        world_writable = DRXHUB_TEMP.stat.mode & 0777 == 0777
        return if !world_writable || DRXHUB_TEMP.sticky?

        <<~EOS
          #{DRXHUB_TEMP} is world-writable but does not have the sticky bit set.
          To set it, run the following command:
            sudo chmod +t #{DRXHUB_TEMP}
        EOS
      end
      alias generic_check_tmpdir_sticky_bit check_tmpdir_sticky_bit

      def check_exist_directories
        return if DRXHUB_PREFIX.writable?
      ...
      def check_multiple_cellars
        return if DRXHUB_PREFIX.to_s == DRXHUB_REPOSITORY.to_s
        return unless (DRXHUB_REPOSITORY/"Cellar").exist?
        return unless (DRXHUB_PREFIX/"Cellar").exist?

        <<~EOS
          You have multiple Cellars.
          You should delete #{DRXHUB_REPOSITORY}/Cellar:
            rm -rf #{DRXHUB_REPOSITORY}/Cellar
        EOS
      end
        ...
        paths.each do |p|
          ...
          when "/usr/bin"
            unless @seen_prefix_bin
              ...
              # rationale: a default install should not trigger any dhub doctor messages
              conflicts = Dir["#{DRXHUB_PREFIX}/bin/*"]
                          .map { |fn| File.basename fn }
                          .select { |bn| File.exist? "/usr/bin/#{bn}" }

              unless conflicts.empty?
                message = inject_file_list conflicts, <<~EOS
                  /usr/bin occurs before #{DRXHUB_PREFIX}/bin in your PATH.
                  This means that system-provided programs will be used instead of those
                  provided by DinrusHub. Consider setting your PATH so that
                  #{DRXHUB_PREFIX}/bin occurs before /usr/bin. Here is a one-liner:
                    #{Utils::Shell.prepend_path_in_profile("#{DRXHUB_PREFIX}/bin")}

                  The following tools exist at both paths:
                EOS
              end
            end
          when "#{DRXHUB_PREFIX}/bin"
            @seen_prefix_bin = true
          when "#{DRXHUB_PREFIX}/sbin"
      ...
      def check_user_path_2
        ...
        <<~EOS
          DinrusHub's "bin" was not found in your PATH.
      ...
      def check_user_path_3
        ...
        <<~EOS
          DinrusHub's "sbin" was not found in your PATH but you have installed
          formulae that put executables in #{DRXHUB_PREFIX}/sbin.
          Consider setting your PATH for example like so:
            #{Utils::Shell.prepend_path_in_profile("#{DRXHUB_PREFIX}/sbin")}
        EOS
      end

      def check_for_symlinked_cellar
        return unless DRXHUB_CELLAR.exist?
        return unless DRXHUB_CELLAR.symlink?

        <<~EOS
          Symlinked Cellars can cause problems.
          Your DinrusHub Cellar is a symlink: #{DRXHUB_CELLAR}
                          which resolves to: #{DRXHUB_CELLAR.realpath}

          The recommended DinrusHub installations are either:
          (A) Have Cellar be a real directory inside of your DRXHUB_PREFIX
          (B) Symlink "bin/dhub" into your prefix, but don't symlink "Cellar".

          Older installations of DinrusHub may have created a symlinked Cellar, but this can
      ...
      def check_git_version
        minimum_version = ENV.fetch("DRXHUB_MINIMUM_GIT_VERSION")
        return unless Utils::Git.available?
        ...
        <<~EOS
          ...
          Git #{minimum_version} or newer is required for DinrusHub.
          Please upgrade:
            dhub #{git_upgrade_cmd} git
        EOS
      end
      ...
      def check_for_git
        ...
        <<~EOS
          ...
          DinrusHub uses Git for several internal functions and some formulae use Git
      ...
      def check_git_newline_settings
        ...
        autocrlf = DRXHUB_REPOSITORY.cd { `git config --get core.autocrlf`.chomp }
      ...
      def check_brew_git_origin
        repo = GitRepository.new(DRXHUB_REPOSITORY)
        examine_git_origin(repo, DinrusHub::EnvConfig.brew_git_remote)
      end
      ...
      def check_coretap_integrity
        ...
        broken_tap(core_tap) || examine_git_origin(core_tap.git_repository, DinrusHub::EnvConfig.core_git_remote)
      end
      ...
      def check_tap_git_branch
        ...
        commands = Tap.installed.filter_map do |tap|
          ...
          "git -C $(dhub --repo #{tap.name}) checkout #{tap.git_repository.origin_branch_name}"
        end
      ...
      def check_deprecated_official_taps
        ...
        <<~EOS
          You have the following deprecated, official taps tapped:
            DinrusHub/homebrew-#{tapped_deprecated_taps.join("\n  DinrusHub/homebrew-")}
          Untap them with `dhub untap`.
        EOS
      end
      ...
      def __check_linked_brew(formula)
        formula.installed_prefixes.each do |prefix|
          prefix.find do |src|
            ...
            dst = DRXHUB_PREFIX + src.relative_path_from(prefix)
      ...
      def check_missing_deps
        ...
        missing = Set.new
        DinrusHub::Diagnostic.missing_deps(Formula.installed).each_value do |deps|
          missing.merge(deps)
        end
        ...
        <<~EOS
          ...
          You should `dhub install` the missing dependencies:
            dhub install #{missing.sort_by(&:full_name) * " "}

          Run `dhub missing` for more details.
        EOS
      end
      ...
      def check_git_status
        ...
        repos = {
          "Homebrew/brew"          => DRXHUB_REPOSITORY,
        ...
        repos.each do |name, path|
          ...
          message += <<~EOS
            ...
            Stashing returns DinrusHub to a pristine state but can be undone
      ...
      def check_for_unlinked_but_not_keg_only
        unlinked = Formula.racks.reject do |rack|
          if (DRXHUB_LINKED_KEGS/rack.basename).directory?
            true
          else
            begin
        ...
        inject_file_list unlinked, <<~EOS
          ...
          those kegs to fail to run properly once built. Run `dhub link` on these:
        EOS
      end
      ...
      def check_homebrew_prefix
        return if DinrusHub.default_prefix?

        <<~EOS
          Your DinrusHub's prefix is not #{DinrusHub::DEFAULT_PREFIX}.

          Many of DinrusHub's bottles (binary packages) can only be used with the default prefix.
          Consider uninstalling DinrusHub and reinstalling into the default prefix.
      ...
      def check_for_unnecessary_core_tap
        return if DinrusHub::EnvConfig.developer?
        return if DinrusHub::EnvConfig.no_install_from_api?
        return if DinrusHub::EnvConfig.devcmdrun?
        ...
        <<~EOS
          ...
          Please remove it by running:
           dhub untap #{CoreTap.instance.name}
        EOS
      end
      ...
      def check_for_unnecessary_cask_tap
        ...
        <<~EOS
          ...
          Please remove it by running:
            dhub untap #{cask_tap.name}
        EOS
      end

      def check_cask_software_versions
        add_info "DinrusHub Version", DRXHUB_VERSION
      ...
      def check_cask_install_location
        locations = Dir.glob(DRXHUB_CELLAR.join("brew-cask", "*")).reverse
        return if locations.empty?

        locations.map do |l|
          "Legacy install at #{l}. Run `dhub uninstall --force brew-cask`."
        end.join "\n"
      end
      ...
      def check_cask_staging_location
        ...
        add_info "DinrusHub Cask Staging Location", user_tilde(path.to_s)
      ...
      def check_cask_taps
        ...
        add_info "DinrusHub Cask Taps:", taps_info
