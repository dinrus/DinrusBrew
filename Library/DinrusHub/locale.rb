...
class Locale
  ...
  def initialize(language, script, region)
    raise ArgumentError, "#{self.class} не может быть пуст" if language.nil? && region.nil? && script.nil?

    {
      ...
      raise ParserError, "'#{value}' не соответствует #{regex}" unless value&.match?(regex)
