...
class FormulaInstaller
  ...
  ETC_VAR_DIRS = T.let([DRXHUB_PREFIX/"etc", DRXHUB_PREFIX/"var"].freeze, T::Array[Pathname])
    ...
    unless bottle.compatible_locations?
      if output_warning
        ...
        opoo <<~EOS
          ...
          - DRXHUB_CELLAR: #{bottle.cellar} (у вас #{DRXHUB_CELLAR})
          - DRXHUB_PREFIX: #{prefix} (у вас #{DRXHUB_PREFIX})
        EOS
      end
      return false
    end
  ...
  def check_install_sanity
    ...
    if DinrusHub.default_prefix? &&
      ...
      if message
        message += <<~EOS
          При наличии смелости можно попробовать установить из исходников:
            dhub install --build-from-source #{formula}

          Как правило, большинство формул  не удаётся построить из исходников.
          Как правило, DinrusHub при построении из исходников будет часто ошибаться и медлить.
          Никуда не жалуйтесь о проблемах при построениях из репозиториев исходников GitHub.
        EOS
        raise CannotInstallFormulaError, message
      end
    end
    ...
    raise CannotInstallFormulaError,
          "Нужно выполнить `dhub unpin #{pinned_unsatisfied_deps * " "}`, т.к. для установки " \
  ...
  def build_bottle_postinstall
    ...
    (etc_var_postinstall - @etc_var_preinstall).each do |file|
      Pathname.new(file).cp_path_sub(DRXHUB_PREFIX, formula.bottle_prefix)
    end
  end
  ...
  def install
    ...
    start_time = Time.now
    DinrusHub::Install.perform_build_from_source_checks if !pour_bottle? && DevelopmentTools.installed?
...
on_request: installed_on_request?, options:)
    ...
    unless @poured_bottle
      ...
      brew_prefix = formula.prefix/".dhub"
      brew_prefix.mkpath
    ...
    end_time = Time.now
    DinrusHub.messages.package_installed(formula.name, end_time - start_time)
  end
  ...
  def check_conflicts
    ...
    conflicts = formula.conflicts.select do |c|
      ...
      raise if DinrusHub::EnvConfig.developer?

      $stderr.puts "Отчитайтесь о проблеме по тапу #{formula.tap&.full_name}".squeeze(" ")
      $stderr.puts " (нет Homebrew/brew или DinrusHub/homebrew-core)!" unless formula.core_formula?
      false
    else
  ...
  def install_dependency(dep, inherited_options)
    ...
    if df.tap.present? && tab.present? && (tab_tap = tab.source["tap"].presence) &&
      ...
      odie <<~EOS
        ...
        Вначале выполните `dhub uninstall #{df}`."
      EOS
    end
  ...
  def caveats
    ...
    audit_installed if DinrusHub::EnvConfig.developer?
    ...
    DinrusHub.messages.record_caveats(formula.name, caveats)
  end
  ...
  def finish
    ...
    if skip_link?
      unless quiet?
        ...
        puts "  dhub link #{formula.full_name}"
      end
    else
      link(keg)
    end
    ...
    DinrusHub::Install.global_post_install
    ...
    if build_bottle? || skip_post_install?
      unless quiet?
        ...
        puts "Это можно выполнить вручную посредством:"
        puts "  dhub postinstall #{formula.full_name}"
      end
    else
    ...
    unless build_bottle?
      ...
      sbom.write(validate: DinrusHub::EnvConfig.developer?)
    end
    ...
    if formula.name == "curl" &&
      ...
      ENV["DRXHUB_CURL"] = (formula.opt_bin/"curl").to_s
  ...
  def summary
    s = +""
    s << "#{DinrusHub::EnvConfig.install_badge}  " unless DinrusHub::EnvConfig.no_emoji?
  ...
  def link(keg)
    ...
    DinrusHub::Unlink.unlink_versioned_formulae(formula, verbose: verbose?)

    link_overwrite_backup = {} # Hash: conflict file -> backup file
    backup_dir = DRXHUB_CACHE/"Backup"
    ...
    begin
      ...
      if formula.link_overwrite?(conflict_file) && !link_overwrite_backup.key?(conflict_file)
        backup_file = backup_dir/conflict_file.relative_path_from(DRXHUB_PREFIX).to_s
        backup_file.parent.mkpath
        FileUtils.mv conflict_file, backup_file
        link_overwrite_backup[conflict_file] = backup_file
        retry
      end
      ofail "Шаг `dhub link` завершён неуспешно"
      puts "Формула построена, но не создана символьная ссылка в #{DRXHUB_PREFIX}"
      puts e
      puts
      ...
      puts "  dhub link #{formula.name}"
      @show_summary_heading = true
    # Handle all other possible exceptions when linking.
    rescue Exception => e # rubocop:disable Lint/RescueException
      ofail "Во время шага `dhub link` произошла неожидаемая ошибка."
      puts "Формула построена, но не создана символьная ссылка в  #{DRXHUB_PREFIX}"
      puts e
    ...
    opoo "Эти файлы переписаны при шаге `dhub link`:"
  ...
  def post_install_formula_path
    ...
    keg_formula_path = formula.opt_prefix/".dhub/#{formula.name}.rb"
  ...
  def post_install
    args = [
      ...
      *DRXHUB_RUBY_EXEC_ARGS,
      ...
      "--",
      DRXHUB_LIBRARY_PATH/"postinstall.rb"
    ]
    ...
    if Sandbox.available?
      ...
      Keg.keg_link_directories.each do |dir|
        sandbox.allow_write_path "#{DRXHUB_PREFIX}/#{dir}"
      end
      sandbox.run(*args)
    else
      Utils.safe_fork do
  ...
  # Handle all possible exceptions when postinstall does not complete.
    ...
    require "utils/backtrace"
    odebug e, Utils::Backtrace.clean(e), always_display: DinrusHub::EnvConfig.developer?
  ...
  def fetch
    ...
    check_attestation = if pour_bottle?(output_warning: true)
      ...
      @formula = DinrusHub::API::Formula.source_download(formula) if formula.loaded_from_api?
    ...
    # to explicitly `dhub install gh` without already having a version for bootstrapping.
    ...
    if check_attestation &&
       DinrusHub::Attestation.enabled? &&
       formula.tap&.core_tap? &&
       formula.name != "gh"
      ohai "Проверка аттестации для #{formula.name}"
      begin
        DinrusHub::Attestation.check_core_attestation T.cast(downloadable_object, Bottle)
      rescue DinrusHub::Attestation::GhIncompatible
        ...
        raise CannotInstallFormulaError, <<~EOS
          ...
          by performing an upgrade before retrying:
            ...
            dhub upgrade gh
        EOS
      rescue DinrusHub::Attestation::GhAuthInvalid
        # Only raise an error if we explicitly opted-in to verification.
        raise CannotInstallFormulaError, <<~EOS if DinrusHub::EnvConfig.verify_attestations?
          The bottle for #{formula.name} could not be verified.

          This typically indicates an invalid GitHub API token.

          If you have `DRXHUB_GITHUB_API_TOKEN` set, check it is correct
        ...
        ENV["DRXHUB_NO_VERIFY_ATTESTATIONS"] = "1"
      rescue DinrusHub::Attestation::GhAuthNeeded
        raise CannotInstallFormulaError, <<~EOS
          The bottle for #{formula.name} could not be verified.

          This typically indicates a missing GitHub API token, which you
          can resolve either by setting `DRXHUB_GITHUB_API_TOKEN` or
          by running:

            gh auth login
        EOS
      rescue DinrusHub::Attestation::MissingAttestationError, DinrusHub::Attestation::InvalidAttestationError => e
        raise CannotInstallFormulaError, <<~EOS
  ...
  def pour
    ...
    return if cellar == DRXHUB_CELLAR.to_s && prefix == DRXHUB_PREFIX.to_s

    return unless ENV["DRXHUB_RELOCATE_BUILD_PREFIX"]

    keg.relocate_build_prefix(keg, prefix, DRXHUB_PREFIX)
  end
  ...
  def forbidden_license_check
    forbidden_licenses = DinrusHub::EnvConfig.forbidden_licenses.to_s.dup
    ...
    if invalid_licenses.present?
      opoo <<~EOS
        DRXHUB_FORBIDDEN_LICENSES contains invalid license identifiers: #{invalid_licenses.to_sentence}
    ...
    unless ignore_deps?
      compute_dependencies.each do |(dep, _options)|
        ...
        raise CannotInstallFormulaError, <<~EOS
          ...
          its licenses were forbidden by #{owner} in `DRXHUB_FORBIDDEN_LICENSES`:
    ...
    raise CannotInstallFormulaError, <<~EOS
      #{formula.name}'s licenses are all forbidden by #{owner} in `DRXHUB_FORBIDDEN_LICENSES`:
  ...
  def forbidden_tap_check
    ...
    unless ignore_deps?
      compute_dependencies.each do |(dep, _options)|
        ...
        error_message << "has not allowed this tap in `DRXHUB_ALLOWED_TAPS`" unless dep_tap.allowed_by_env?
        error_message << " and\n" if !dep_tap.allowed_by_env? && dep_tap.forbidden_by_env?
        error_message << "has forbidden this tap in `DRXHUB_FORBIDDEN_TAPS`" if dep_tap.forbidden_by_env?
    ...
    error_message = "The installation of #{formula.full_name} has the tap #{formula_tap}\n" \
                    "but #{owner} "
    error_message << "has not allowed this tap in `DRXHUB_ALLOWED_TAPS`" unless formula_tap.allowed_by_env?
    error_message << " and\n" if !formula_tap.allowed_by_env? && formula_tap.forbidden_by_env?
    error_message << "has forbidden this tap in `DRXHUB_FORBIDDEN_TAPS`" if formula_tap.forbidden_by_env?
    error_message << ".#{owner_contact}"
  ...
  def forbidden_formula_check
    forbidden_formulae = Set.new(DinrusHub::EnvConfig.forbidden_formulae.to_s.split)
    ...
    unless ignore_deps?
      compute_dependencies.each do |(dep, _options)|
        ...
        raise CannotInstallFormulaError, <<~EOS
          ...
          but the #{dep_name} formula was forbidden by #{owner} in `DRXHUB_FORBIDDEN_FORMULAE`.#{owner_contact}
        EOS
      end
    end
    ...
    raise CannotInstallFormulaError, <<~EOS
      ...
      in `DRXHUB_FORBIDDEN_FORMULAE`.#{owner_contact}
    EOS
  end
