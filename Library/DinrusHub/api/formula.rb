...
module DinrusHub
  module API
    ...
    module Formula
      ...
      def self.fetch(name)
        DinrusHub::API.fetch "formula/#{name}.json"
      end
      ...
      def self.source_download(formula)
        ...
        download = DinrusHub::API::Download.new(
          "https://raw.githubusercontent.com/#{tap}/#{git_head}/#{path}",
          formula.ruby_source_checksum,
          cache: DRXHUB_CACHE_API_SOURCE/"#{tap}/#{git_head}/Formula",
        )
        download.fetch

        with_env(DRXHUB_FORBID_PACKAGES_FROM_PATHS: nil) do
      ...
      def self.cached_json_file_path
        if DinrusHub::API.internal_json_v3?
          DRXHUB_CACHE_API/INTERNAL_V3_API_FILENAME
        else
          DRXHUB_CACHE_API/DEFAULT_API_FILENAME
        end
      end
      ...
      def self.download_and_cache_data!
        if DinrusHub::API.internal_json_v3?
          json_formulae, updated = DinrusHub::API.fetch_json_api_file INTERNAL_V3_API_FILENAME
          overwrite_cache! T.cast(json_formulae, T::Hash[String, T.untyped])
        else
          json_formulae, updated = DinrusHub::API.fetch_json_api_file DEFAULT_API_FILENAME
      ...
      def self.write_names_and_aliases(regenerate: false)
        ...
        return unless DinrusHub::API.write_names_file(all_formulae.keys, "formula", regenerate:)

        (DRXHUB_CACHE_API/"formula_aliases.txt").open("w") do |file|
