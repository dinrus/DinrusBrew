  ...
  module API
    ...
    module Cask
      ...
      def self.fetch(token)
        DinrusHub::API.fetch "cask/#{token}.json"
      end
      ...
      def self.source_download(cask)
        ...
        download = DinrusHub::API::Download.new(
          ...
          checksum,
          mirrors: [
            "#{DRXHUB_API_DEFAULT_DOMAIN}/cask-source/#{File.basename(path)}",
          ],
          cache:   DRXHUB_CACHE_API_SOURCE/"#{tap}/#{git_head}/Cask",
        )
        download.fetch
      ...
      def self.download_and_cache_data!
        json_casks, updated = DinrusHub::API.fetch_json_api_file DEFAULT_API_FILENAME
      ...
      def self.write_names(regenerate: false)
        ...
        DinrusHub::API.write_names_file(all_casks.keys, "cask", regenerate:)
      end
    end
  end
end
