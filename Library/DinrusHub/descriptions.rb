...
class Descriptions
  ...
  def self.search(string_or_regex, field, cache_store,
                  eval_all = DinrusHub::EnvConfig.eval_all?, cache_store_hash: false)
    cache_store.populate_if_empty!(eval_all:) unless cache_store_hash

    results = case field
    when :name
      DinrusHub::Search.search(cache_store, string_or_regex) { |name, _| name }
    when :desc
      DinrusHub::Search.search(cache_store, string_or_regex) { |_, desc| desc }
    when :either
      DinrusHub::Search.search(cache_store, string_or_regex)
    end
