...
class Requirement
  ...
  def satisfied_result_parent
    ...
    if parent.to_s =~ %r{^#{Regexp.escape(DRXHUB_CELLAR)}/([\w+-.@]+)/[^/]+/(s?bin)/?$}o
      parent = DRXHUB_PREFIX/"opt/#{Regexp.last_match(1)}/#{Regexp.last_match(2)}"
    end
    parent
  end
  ...
  def modify_build_environment(env: nil, cc: nil, build_bottle: false, bottle_arch: nil)
    ...
    return if ["#{DRXHUB_PREFIX}/bin", "#{DRXHUB_PREFIX}/bin"].include?(parent.to_s)
