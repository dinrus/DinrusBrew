...
module OS
  ...
  def self.mac?
    return false if ENV["DRXHUB_TEST_GENERIC_OS"]
  ...
  ::OS_VERSION = T.let(ENV.fetch("DRXHUB_OS_VERSION").freeze, String)
  ...
  if OS.mac?
    ...
    if !OS::Mac.version.prerelease? &&
       ...
       (DRXHUB_PREFIX.to_s == DRXHUB_DEFAULT_PREFIX ||
       (DRXHUB_PREFIX.to_s == DRXHUB_MACOS_ARM_DEFAULT_PREFIX && Hardware::CPU.arm?))
