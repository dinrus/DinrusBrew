...
module DinrusHub
  ...
  module Style
    ...
    def self.check_style_impl(files, output_type,
      ...
      Array(files).map(&method(:Pathname))
        ...
        when ".yml"
          ...
          ruby_files << path
          shell_files += if [DRXHUB_PREFIX, DRXHUB_REPOSITORY].include?(path)
            shell_scripts
          else
            path.glob("**/*.sh")
    ...
    RUBOCOP = (DRXHUB_LIBRARY_PATH/"utils/rubocop.rb").freeze
    ...
    def self.run_rubocop(files, output_type,
      ...
      base_dir = Dir.pwd
      if files.blank? || files == [DRXHUB_REPOSITORY]
        files = [DRXHUB_LIBRARY_PATH]
        base_dir = DRXHUB_LIBRARY_PATH
      elsif files.any? { |f| f.to_s.start_with?(DRXHUB_REPOSITORY/"docs") || (f.basename.to_s == "docs") }
        args << "--config" << (DRXHUB_REPOSITORY/"docs/docs_rubocop_style.yml")
      elsif files.any? { |f| f.to_s.start_with? DRXHUB_LIBRARY_PATH }
        base_dir = DRXHUB_LIBRARY_PATH
      else
        args << "--config" << (DRXHUB_LIBRARY/".rubocop.yml")
        base_dir = DRXHUB_LIBRARY if files.any? { |f| f.to_s.start_with? DRXHUB_LIBRARY }
      end

      args += files

      DRXHUB_CACHE.mkpath
      cache_dir = DRXHUB_CACHE.realpath
      cache_env = { "XDG_CACHE_HOME" => "#{cache_dir}/style" }

      FileUtils.rm_rf cache_env["XDG_CACHE_HOME"] if reset_cache

      ruby_args = DRXHUB_RUBY_EXEC_ARGS.dup
      case output_type
      when :print
        args << "--debug" if debug
    ...
    def self.run_shfmt(files, fix: false)
      ...
      files.delete(DRXHUB_REPOSITORY/"completions/bash/dhub")
      files.delete(DRXHUB_REPOSITORY/"Dockerfile")
    ...
    def self.run_actionlint(files)
      ...
      system actionlint, "-shellcheck", shellcheck,
             "-config-file", DRXHUB_REPOSITORY/".github/actionlint.yaml",
    ...
    def self.shell_scripts
      [
        DRXHUB_ORIGINAL_BREW_FILE,
        DRXHUB_REPOSITORY/"completions/bash/dhub",
        DRXHUB_REPOSITORY/"Dockerfile",
        *DRXHUB_REPOSITORY.glob(".devcontainer/**/*.sh"),
        *DRXHUB_REPOSITORY.glob("package/scripts/*"),
        *DRXHUB_LIBRARY.glob("DinrusHub/**/*.sh").reject { |path| path.to_s.include?("/vendor/") },
        *DRXHUB_LIBRARY.glob("DinrusHub/shims/**/*").map(&:realpath).uniq
                         .reject(&:directory?)
                         .reject { |path| path.basename.to_s == "cc" }
                         .select do |path|
                           %r{^#! ?/bin/(?:ba)?sh( |$)}.match?(path.read(13))
                         end,
        *DRXHUB_LIBRARY.glob("DinrusHub/{dev-,}cmd/*.sh"),
        *DRXHUB_LIBRARY.glob("DinrusHub/{cask/,}utils/*.sh"),
      ]
    end

    def self.github_workflow_files
      DRXHUB_REPOSITORY.glob(".github/workflows/*.yml")
    end
    ...
    def self.shfmt
      ...
      DRXHUB_LIBRARY/"DinrusHub/utils/shfmt.sh"
    end
