...
module DinrusHub
  ...
  module Manpages
    ...
    SOURCE_PATH = (DRXHUB_LIBRARY_PATH/"manpages").freeze
    TARGET_MAN_PATH = (DRXHUB_REPOSITORY/"manpages").freeze
    TARGET_DOC_PATH = (DRXHUB_REPOSITORY/"docs").freeze
    ...
    def self.regenerate_man_pages(quiet:)
      ...
      $stderr.puts(warnings)
      File.write(TARGET_MAN_PATH/"dhub.1", roff)
    end

    def self.build_man_page(quiet:)
      template = (SOURCE_PATH/"dhub.1.md.erb").read
      ...
      variables = Variables.new(
        ...
        lead:                       readme.read[/(DinrusHub's \[Project Leader.*\.)/, 1]
                                      .gsub(/\[([^\]]+)\]\([^)]+\)/, '\1'),
        plc:                        readme.read[/(DinrusHub's \[Project Leadership Committee.*\.)/, 1]
                                      .gsub(/\[([^\]]+)\]\([^)]+\)/, '\1'),
        tsc:                        readme.read[/(DinrusHub's \[Technical Steering Committee.*\.)/, 1]
                                      .gsub(/\[([^\]]+)\]\([^)]+\)/, '\1'),
        maintainers:                readme.read[/(DinrusHub's maintainers .*\.)/, 1]
                                      .gsub(/\[([^\]]+)\]\([^)]+\)/, '\1'),
    ...
    def self.generate_cmd_manpages(cmd_paths)
      ...
      cmd_paths.sort_by { sort_key_for_path(_1) }
               .each do |cmd_path|
        cmd_man_page_lines = if (cmd_parser = DinrusHub::CLI::Parser.from_cmd_path(cmd_path))
    ...
    def self.cmd_parser_manpage_lines(cmd_parser)
      ...
      lines += cmd_parser.processed_options.filter_map do |short, long, desc, hidden|
        ...
        if long.present?
          next if DinrusHub::CLI::Parser.global_options.include?([short, long, desc])
          next if DinrusHub::CLI::Parser.global_cask_options.any? do |_, option, kwargs|
    ...
    def self.global_cask_options_manpage
      ...
      lines += DinrusHub::CLI::Parser.global_cask_options.map do |_, long, kwargs|
    ...
    def self.global_options_manpage
      ...
      lines += DinrusHub::CLI::Parser.global_options.map do |short, long, desc|
        generate_option_doc(short, long, desc)
      end
      lines.join("\n")
    end
    ...
    def self.env_vars_manpage
      lines = DinrusHub::EnvConfig::ENVS.flat_map do |env, hash|
        entry = "`#{env}`\n\n: #{hash[:description]}\n"
