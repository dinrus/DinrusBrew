...
module DinrusHub
  ...
  module Completions
    ...
    COMPLETIONS_DIR = T.let((DRXHUB_REPOSITORY/"completions").freeze, Pathname)
    TEMPLATE_DIR = T.let((DRXHUB_LIBRARY_PATH/"completions").freeze, Pathname)
    ...
    BASH_NAMED_ARGS_COMPLETION_FUNCTION_MAPPING = T.let({
      ...
      diagnostic_check:  '__brewcomp "${__DRXHUB_DOCTOR_CHECKS=$(dhub doctor --list-checks)}"',
    ...
    def self.link!
      ...
      Tap.installed.each do |tap|
        Utils::Link.link_completions tap.path, "dhub completions link"
      end
    end
    ...
    def self.show_completions_message_if_needed
      ...
      ohai "DinrusHub completions for external commands are unlinked by default!"
      puts <<~EOS
    ...
    def self.update_shell_completions!
      ...
      (COMPLETIONS_DIR/"bash/dhub").atomic_write generate_bash_completion_file(commands)
      (COMPLETIONS_DIR/"zsh/_brew").atomic_write generate_zsh_completion_file(commands)
      (COMPLETIONS_DIR/"fish/dhub.fish").atomic_write generate_fish_completion_file(commands)
    end
    ...
    def self.generate_zsh_completion_file(commands)
      variables = Variables.new(
        aliases:                      Commands::DRXHUB_INTERNAL_COMMAND_ALIASES.filter_map do |alias_cmd, command|
        ...
        builtin_command_descriptions: commands.filter_map do |command|
          next if Commands::DRXHUB_INTERNAL_COMMAND_ALIASES.key? command
