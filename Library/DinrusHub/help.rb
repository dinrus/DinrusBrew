...
module DinrusHub
  ...
  module Help
    ...
    def self.help(cmd = nil, empty_argv: false, usage_error: nil, remaining_args: [])
      if cmd.nil?
        # Handle `dhub` (no arguments).
        if empty_argv
          $stderr.puts DRXHUB_HELP_MESSAGE
          exit 1
        end

        # Handle `dhub (-h|--help|--usage|-?|help)` (no other arguments).
        puts DRXHUB_HELP_MESSAGE
        exit 0
      end
      ...
      if usage_error
        $stderr.puts path ? command_help(cmd, path, remaining_args:) : DRXHUB_HELP_MESSAGE
        $stderr.puts
        onoe usage_error
        exit 1
      end

      # Resume execution in `dhub.rb` for unknown commands.
      return if path.nil?
    ...
    def self.command_help(cmd, path, remaining_args:)
      ...
      output ||= if output.blank?
        opoo "No help text in: #{path}" if DinrusHub::EnvConfig.developer?
        DRXHUB_HELP_MESSAGE
      end
    ...
    def self.comment_help(path)
      ...
      Formatter.format_help_text(help_lines.join, width: Formatter::COMMAND_DESC_WIDTH)
               ...
               .sub(/^\* /, "#{Tty.bold}Usage: dhub#{Tty.reset} ")
