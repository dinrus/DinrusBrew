...
class Resource
  ...
  def determine_url_mirrors
    ...
    if url.start_with?("https://github.com/Homebrew/glibc-bootstrap/releases/download")
      if (artifact_domain = DinrusHub::EnvConfig.artifact_domain.presence)
        artifact_url = url.sub("https://github.com", artifact_domain)
        return [artifact_url] if DinrusHub::EnvConfig.artifact_domain_no_fallback?
      ...
      if DinrusHub::EnvConfig.bottle_domain != DRXHUB_BOTTLE_DEFAULT_DOMAIN
        ...
        extra_urls << "#{DinrusHub::EnvConfig.bottle_domain}/glibc-bootstrap/#{tag}/#{filename}"
      end
    end

    # PyPI packages: PEP 503 – Simple Repository API <https://peps.python.org/pep-0503>
    if (pip_index_url = DinrusHub::EnvConfig.pip_index_url.presence)
  ...
  class BottleManifest < Resource
    ...
    def tab
      tab = manifest_annotations["sh.dhub.tab"]
    ...
    def bottle_size
      manifest_annotations["sh.dhub.bottle.size"]&.to_i
    end

    sig { returns(T.nilable(Integer)) }
    def installed_size
      manifest_annotations["sh.dhub.bottle.installed_size"]&.to_i
    end
    ...
    def manifest_annotations
      ...
      manifest_annotations = manifests_annotations.find do |m|
        next if m["sh.dhub.bottle.digest"] != bottle_digest
