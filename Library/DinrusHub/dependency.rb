...
# A dependency on another DinrusHub formula.
#
# @api internal
class Dependency
  include Dependable
...
class UsesFromMacOSDependency < Dependency
  ...
  def use_macos_install?
    ...
    if DinrusHub::SimulateSystem.simulating_or_running_on_macos?
      # Assume the oldest macOS version when simulating a generic macOS version
      return true if DinrusHub::SimulateSystem.current_os == :macos && !bounds.key?(:since)

      if DinrusHub::SimulateSystem.current_os != :macos
        current_os = MacOSVersion.from_symbol(DinrusHub::SimulateSystem.current_os)
