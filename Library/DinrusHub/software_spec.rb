...
class Bottle
  ...
  def fallback_on_error
    ...
    if @resource.url.start_with?(DinrusHub::EnvConfig.bottle_domain) &&
       DinrusHub::EnvConfig.bottle_domain != DRXHUB_BOTTLE_DEFAULT_DOMAIN
...
class BottleSpecification
  ...
  def initialize
    @rebuild = 0
    @repository = DinrusHub::DEFAULT_REPOSITORY
  ...
  def root_url(var = nil, specs = {})
    if var.nil?
      @root_url ||= if (github_packages_url = GitHubPackages.root_url_if_match(DinrusHub::EnvConfig.bottle_domain))
        github_packages_url
      else
        DinrusHub::EnvConfig.bottle_domain
      end
    else
  ...
  def compatible_locations?(tag: Utils::Bottles.tag)
    ...
    cellar_relocatable = cellar.size >= DRXHUB_CELLAR.to_s.size && ENV["DRXHUB_RELOCATE_BUILD_PREFIX"].present?
    prefix_relocatable = prefix.size >= DRXHUB_PREFIX.to_s.size && ENV["DRXHUB_RELOCATE_BUILD_PREFIX"].present?

    compatible_cellar = cellar == DRXHUB_CELLAR.to_s || cellar_relocatable
    compatible_prefix = prefix == DRXHUB_PREFIX.to_s || prefix_relocatable
