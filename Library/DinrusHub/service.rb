...
module DinrusHub
  ...
  class Service
    ...
    def std_service_path_env
      "#{DRXHUB_PREFIX}/bin:#{DRXHUB_PREFIX}/sbin:/usr/bin:/bin:/usr/sbin:/sbin"
    end
    ...
    def to_systemd_unit
      ...
      <<~SYSTEMD
        [Unit]
        Description=DinrusHub generated unit for #{@formula.name}
    ...
    def to_systemd_timer
      ...
      <<~SYSTEMD
        [Unit]
        Description=DinrusHub generated timer for #{@formula.name}
      ...
      sockets_var = if @sockets.present?
        @sockets.transform_values { |info| "#{info[:type]}://#{info[:host]}:#{info[:port]}" }
                .then do |sockets_hash|
                  # TODO: Remove this code when all users are running on versions of DinrusHub
    ...
    def self.replace_placeholders(string)
      string.gsub(DRXHUB_PREFIX_PLACEHOLDER, DRXHUB_PREFIX)
