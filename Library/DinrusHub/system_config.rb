...
module SystemConfig
  class << self
    ...
    def homebrew_repo
      GitRepository.new(DRXHUB_REPOSITORY)
    end
    ...
    def dump_tap_config(tap, out = $stdout)
      ...
      if (json_file = DinrusHub::API::DRXHUB_CACHE_API/json_file_name) && json_file.exist?
    ...
    def homebrew_config(out = $stdout)
      out.puts "DRXHUB_VERSION: #{DRXHUB_VERSION}"
    ...
    def homebrew_env_config(out = $stdout)
      out.puts "DRXHUB_PREFIX: #{DRXHUB_PREFIX}"
      {
        DRXHUB_REPOSITORY: DinrusHub::DEFAULT_REPOSITORY,
        DRXHUB_CELLAR:     DinrusHub::DEFAULT_CELLAR,
      }.freeze.each do |key, default|
        value = Object.const_get(key)
        out.puts "#{key}: #{value}" if value.to_s != default.to_s
      end

      DinrusHub::EnvConfig::ENVS.each do |env, hash|
        method_name = DinrusHub::EnvConfig.env_method_name(env, hash)

        if hash[:boolean]
          out.puts "#{env}: set" if DinrusHub::EnvConfig.send(method_name)
          next
        end

        value = DinrusHub::EnvConfig.send(method_name)
        next unless value
      ...
      out.puts "DinrusHub Ruby: #{describe_homebrew_ruby}"
    end
