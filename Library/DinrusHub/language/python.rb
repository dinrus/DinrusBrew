...
module Language
  ...
  module Python
    ...
    def self.homebrew_site_packages(python = "python3.7")
      DRXHUB_PREFIX/site_packages(python)
    end
      ...
      class Virtualenv
        ...
        def create(system_site_packages: true, without_pip: true)
          ...
          @venv_root.find do |f|
            ...
            next unless f.readlink.expand_path.to_s.start_with? DRXHUB_CELLAR

            rp = f.realpath.to_s
            version = rp.match %r{^#{DRXHUB_CELLAR}/python@(.*?)/}o
            version = "@#{version.captures.first}" unless version.nil?

            new_target = rp.sub %r{#{DRXHUB_CELLAR}/python#{version}/[^/]+}, Formula["python#{version}"].opt_prefix
            f.unlink
          ...
          Pathname.glob(@venv_root/"lib/python*/orig-prefix.txt").each do |prefix_file|
            ...
            version = prefix_path.match %r{^#{DRXHUB_CELLAR}/python@(.*?)/}o
            version = "@#{version.captures.first}" unless version.nil?

            prefix_path.sub! %r{^#{DRXHUB_CELLAR}/python#{version}/[^/]+}, Formula["python#{version}"].opt_prefix
          ...
          if (cfg_file = @venv_root/"pyvenv.cfg").exist?
            ...
            cfg.match(%r{= *(#{DRXHUB_CELLAR}/(python@[\d.]+)/[^/]+(?:/#{framework}/[\d.]+)?/bin)}) do |match|
