...
module OS
  ...
  module Mac
    raise "Loaded OS::Mac on generic OS!" if ENV["DRXHUB_TEST_GENERIC_OS"]
    ...
    VERSION = ENV.fetch("DRXHUB_MACOS_VERSION").chomp.freeze
    ...
    def self.full_version
      @full_version ||= if (fake_macos = ENV.fetch("DRXHUB_FAKE_MACOS", nil)) # for Portable Ruby building
    ...
    def self.macports_or_fink
      ...
      # read-only in order to try out DinrusHub, but this doesn't work as
    ...
    def self.app_with_bundle_id(*ids)
      ...
      paths.max_by { |bundle_path| DinrusHub::BundleVersion.from_info_plist(bundle_path/"Contents/Info.plist") }
    end
