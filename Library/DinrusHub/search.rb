...
module DinrusHub
  ...
  module Search
    ...
    def self.search_descriptions(string_or_regex, args, search_type: :desc)
      ...
      if args.formula? || both
        ...
        if eval_all
          ...
          descriptions = DinrusHub::API::Formula.all_formulae.transform_values { |data| data["desc"] }
      ...
      if eval_all
        ...
        descriptions = DinrusHub::API::Cask.all_casks.transform_values { |c| [c["name"].join(", "), c["desc"]] }
        Descriptions.search(string_or_regex, search_type, descriptions, eval_all, cache_store_hash: true).print
      end
    end

    def self.search_formulae(string_or_regex)
      if string_or_regex.is_a?(String) && string_or_regex.match?(DRXHUB_TAP_FORMULA_REGEX)
        return begin
    ...
    def self.search_casks(string_or_regex)
      if string_or_regex.is_a?(String) && string_or_regex.match?(DRXHUB_TAP_CASK_REGEX)
        return begin
