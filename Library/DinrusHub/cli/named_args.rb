    ...
    class NamedArgs < Array
      ...
      def to_formulae_and_casks_with_taps
        ...
        odie <<~ERROR
          ...
          You may need to run `dhub tap` to install additional taps.
        ERROR
      end
      ...
      def to_paths(only: parent.only_formula_or_cask, recurse_tap: false)
        ...
        @to_paths[only] ||= DinrusHub.with_no_api_env_if_needed(@without_api) do
          downcased_unique_named.flat_map do |name|
            ...
            if only.nil? && name.match?(LOCAL_PATH_REGEX) && path.exist?
              ...
              if formula_path.exist? ||
                 ...
                 DinrusHub::API::Formula.all_formulae.key?(path.basename.to_s))
                paths << formula_path
              end
              if cask_path.exist? ||
                 (!DinrusHub::EnvConfig.no_install_from_api? &&
                 !CoreCaskTap.instance.installed? &&
                 DinrusHub::API::Cask.all_casks.key?(path.basename.to_s))
                paths << cask_path
              end
      ...
      def homebrew_tap_cask_names
        downcased_unique_named.grep(DRXHUB_CASK_TAP_CASK_REGEX)
      end
      ...
      def load_formula_or_cask(name, only: nil, method: nil, warn: nil)
        DinrusHub.with_no_api_env_if_needed(@without_api) do
          ...
          if only == :formula
            ...
            cask = begin
              ...
              if unreadable_error
                onoe <<~EOS
                  Не удалось загрузить формулу: #{name}
                  #{unreadable_error}
                EOS
                opoo "#{name} принимается за каск."
              end
            ...
            if formula_or_kegs && !cask&.tap&.core_cask_tap?
              if cask || unreadable_error
                onoe <<~EOS if unreadable_error
                  Не удалось загрузить каск: #{name}
                  #{unreadable_error}
                EOS
      ...
      def resolve_default_keg(name)
        ...
        linked_keg_ref = DRXHUB_LINKED_KEGS/rack.basename
        opt_prefix = DRXHUB_PREFIX/"opt/#{rack.basename}"
        ...
        begin
          ...
          unless (prefix = f.latest_installed_prefix).directory?
            raise MultipleVersionsInstalledError, <<~EOS
              #{rack.basename} имеет несколько установленных версий.
              Выполните `dhub uninstall --force #{rack.basename}, чтобы удалить их все.
            EOS
          end
