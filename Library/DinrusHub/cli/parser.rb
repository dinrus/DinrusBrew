  ...
  module CLI
    class Parser
      ...
      def self.from_cmd_path(cmd_path)
        ...
        begin
          if require?(cmd_path)
            cmd = DinrusHub::AbstractCommand.command(cmd_name)
            if cmd
              cmd.parser
            else
              # FIXME: remove once commands are all subclasses of `AbstractCommand`:
              DinrusHub.send(cmd_args_method_name)
            end
          end
      ...
      def self.global_cask_options
        [
          [:flag, "--appdir=", {
            description: "Целевое расположение приложений " \
                         "(дефолт: `#{Cask::Config::DEFAULT_DIRS[:appdir]}`).",
          }],
          [:flag, "--keyboard-layoutdir=", {
            description: "Целевое расположение Keyboard Layouts " \
                         "(дефолт: `#{Cask::Config::DEFAULT_DIRS[:keyboard_layoutdir]}`).",
          }],
          [:flag, "--colorpickerdir=", {
            description: "Целевое расположение Color Pickers " \
                         "(дефолт: `#{Cask::Config::DEFAULT_DIRS[:colorpickerdir]}`).",
          }],
          [:flag, "--prefpanedir=", {
            description: "Целевое расположение Preference Panes " \
                         "(дефолт: `#{Cask::Config::DEFAULT_DIRS[:prefpanedir]}`).",
          }],
          [:flag, "--qlplugindir=", {
            description: "Целевое расположение Quick Look Plugins " \
                         "(дефолт: `#{Cask::Config::DEFAULT_DIRS[:qlplugindir]}`).",
          }],
          [:flag, "--mdimporterdir=", {
            description: "Целевое расположение Spotlight Plugins " \
                         "(дефолт: `#{Cask::Config::DEFAULT_DIRS[:mdimporterdir]}`).",
          }],
          [:flag, "--dictionarydir=", {
            description: "Целевое расположение Dictionaries " \
                         "(дефолт: `#{Cask::Config::DEFAULT_DIRS[:dictionarydir]}`).",
          }],
          [:flag, "--fontdir=", {
            description: "Целевое расположени Fonts " \
                         "(дефолт: `#{Cask::Config::DEFAULT_DIRS[:fontdir]}`).",
          }],
          [:flag, "--servicedir=", {
            description: "Целевое расположение Services " \
                         "(дефолт: `#{Cask::Config::DEFAULT_DIRS[:servicedir]}`).",
          }],
          [:flag, "--input-methoddir=", {
            description: "Целевое расположение Input Methods " \
                         "(default: `#{Cask::Config::DEFAULT_DIRS[:input_methoddir]}`).",
          }],
          [:flag, "--internet-plugindir=", {
            description: "Целевое расположение Internet Plugins " \
                         "(дефолт: `#{Cask::Config::DEFAULT_DIRS[:internet_plugindir]}`).",
          }],
          [:flag, "--audio-unit-plugindir=", {
            description: "Целевое расположение Audio Unit Plugins " \
                         "(дефолт: `#{Cask::Config::DEFAULT_DIRS[:audio_unit_plugindir]}`).",
          }],
          [:flag, "--vst-plugindir=", {
            description: "Целевое расположение VST Plugins " \
                         "(дефолт: `#{Cask::Config::DEFAULT_DIRS[:vst_plugindir]}`).",
          }],
          [:flag, "--vst3-plugindir=", {
            description: "Целевое расположение VST3 Plugins " \
                         "(дефолт: `#{Cask::Config::DEFAULT_DIRS[:vst3_plugindir]}`).",
          }],
          [:flag, "--screen-saverdir=", {
            description: "Целевое расположение Screen Savers " \
                         "(дефолт: `#{Cask::Config::DEFAULT_DIRS[:screen_saverdir]}`).",
          }],
      ...
      def self.global_options
        [
          ["-d", "--debug",   "Показывать любую отладочную информацию."],
          ["-q", "--quiet",   "Сделать вывод менее детальным."],
          ["-v", "--verbose", "Сделать вывод более детальным."],
      ...
      sig {
        params(cmd: T.nilable(T.class_of(DinrusHub::AbstractCommand)), block: T.nilable(T.proc.bind(Parser).void)).void
      }
      def initialize(cmd = nil, &block)
        ...
        if cmd
          ...
          @is_dev_cmd = T.let(T.must(cmd_location.absolute_path).start_with?(Commands::DRXHUB_DEV_CMD_PATH),
                              T::Boolean)
          odeprecated(
            "`dhub #{@command_name}'. This command needs to be refactored, as it is written in a style that",
            "inherits from `DinrusHub::AbstractCommand' ( see https://docs.brew.sh/External-Commands )",
      ...
      def generate_help_text
        Formatter.format_help_text(@parser.to_s, width: Formatter::COMMAND_DESC_WIDTH)
                 .gsub(/\n.*?@@HIDDEN@@.*?(?=\n)/, "")
                 .sub(/^/, "#{Tty.bold}Использование: dhub#{Tty.reset} ")
      ...
      def generate_usage_banner
        ...
        command_names += Commands::DRXHUB_INTERNAL_COMMAND_ALIASES.filter_map do |command_alias, command|
        ...
        if @named_args_type.present? && @named_args_type != :none
          arg_type = if @named_args_type.is_a? Array
            ...
            types << "<подкоманда>" if @named_args_type.any?(String)
            types.join("|")
      ...
      def formulae(argv)
        ...
        named_args.filter_map do |arg|
          next if arg.match?(DRXHUB_CASK_TAP_CASK_REGEX)
      ...
      def value_for_env(env)
        ...
        method_name = :"#{env}?"
        if DinrusHub::EnvConfig.respond_to?(method_name)
          DinrusHub::EnvConfig.public_send(method_name)
        else
          ENV.fetch("DRXHUB_#{env.upcase}", nil)
        end
      end
    end
  end
