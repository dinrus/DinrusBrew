...
class Migrator
  ...
  class MigratorNoOldpathError < RuntimeError
    def initialize(oldname)
      super "#{DRXHUB_CELLAR/oldname} не существует."
    end
  end
  ...
  def initialize(formula, oldname, force: false)
    ...
    @formula = formula
    @old_cellar = DRXHUB_CELLAR/oldname
    ...
    @new_cellar = DRXHUB_CELLAR/formula.name
    ...
    unless old_linked_kegs.empty?
      @new_linked_keg_record = DRXHUB_CELLAR/"#{newname}/#{File.basename(old_linked_kegs.first)}"
    end

    @old_pin_record = DRXHUB_PINNED_KEGS/oldname
    @new_pin_record = DRXHUB_PINNED_KEGS/newname
  ...
  def from_same_tap_user?
    ...
    if formula_tap_user == old_tap_user
      true
    # DinrusHub didn't use to update tabs while performing tap-migrations,
  ...
  def move_to_new_directory
    ...
    if new_cellar.exist?
      ...
      odie "Remove #{new_cellar} and #{old_cellar} manually and run `dhub reinstall #{newname}`." if conflicted
    end
  ...
  def link_newname
    ...
    begin
      ...
      onoe "The `dhub link` step did not complete successfully."
      puts e
      puts
    ...
    # Any exception means the `dhub link` step did not complete.
    rescue Exception => e # rubocop:disable Lint/RescueException
