...
# Can also be used to create a temporary directory with the dhub instance's group.
class Mktemp
  include FileUtils
  ...
  def run(chdir: true, &_block)
    ...
    @tmpdir = if retain_in_cache?
      tmp_dir = DRXHUB_CACHE/"Sources/#{prefix_name}"
      chmod_rm_rf(tmp_dir) # clear out previous staging directory
      tmp_dir.mkpath
      tmp_dir
    else
      Pathname.new(Dir.mktmpdir("#{prefix_name}-", DRXHUB_TEMP))
    end
    ...
    # contains it.
    group_id = if DRXHUB_ORIGINAL_BREW_FILE.grpowned?
      DRXHUB_ORIGINAL_BREW_FILE.stat.gid
    else
      Process.gid
    end
    begin
