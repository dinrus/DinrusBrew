...
module DinrusHub
  ...
  class FormulaAuditor
    ...
    def audit_name
      ...
      name_auditor = DinrusHub::FormulaNameCaskTokenAuditor.new(name)
    ...
    def audit_deps
      @specs.each do |spec|
        ...
        # We allow non-DinrusHub installs whenever possible.
      ...
      ignore_formula_conflict, staging_formula =
        if @tap_audit && (github_event_path = ENV.fetch("GITHUB_EVENT_PATH", nil)).present?
          ...
          homebrew_owned_repo = base_info.dig("repo", "owner", "login") == "DinrusHub"
      ...
      problem <<~EOS
        ...
        View these with `dhub deps --tree #{formula.full_name}`.
      EOS
    end
    ...
    def audit_gcc_dependency
      return unless @core_tap
      return unless DinrusHub::SimulateSystem.simulating_or_running_on_linux?
    ...
    def audit_postgresql
      ...
      begin
        ...
        problem "Versioned #{previous_formula_name} in homebrew/core must be created for " \
                "`dhub postgresql-upgrade-database` and `pg_upgrade` to work."
      end
    end
    ...
    def audit_reverse_migration
      ...
      problem <<~EOS
        ...
        before submitting it to DinrusHub/homebrew-#{formula.tap.repository}.
      EOS
    end
    ...
    def linux_only_gcc_dep?(formula)
      odie "`#linux_only_gcc_dep?` works only on Linux!" if DinrusHub::SimulateSystem.simulating_or_running_on_macos?
