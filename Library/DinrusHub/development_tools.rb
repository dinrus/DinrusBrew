...
class DevelopmentTools
  class << self
    ...
    def locate(tool)
      ...
      (@locate ||= T.let({}, T.nilable(T::Hash[T.any(String, Symbol), T.untyped]))).fetch(tool) do |key|
        @locate[key] = if File.executable?((path = "/usr/bin/#{tool}"))
          Pathname.new path
        # DinrusHub GCCs most frequently; much faster to check this before xcrun
        elsif (path = DRXHUB_PREFIX/"bin/#{tool}").executable?
          path
        end
      end
    end
    ...
    def installation_instructions
      "Install Clang or run `dhub install gcc`."
    end
    ...
    def insecure_download_warning(resource)
      ...
      "Using `--insecure` with curl to download #{resource} because we need it to run " \
        "`dhub install #{package}` in order to download securely from now on. " \
    ...
    def gcc_version(cc)
      (@gcc_version ||= T.let({}, T.nilable(T::Hash[String, Version]))).fetch(cc) do
        path = DRXHUB_PREFIX/"opt/#{CompilerSelector.preferred_gcc}/bin"/cc
    ...
    def ca_file_handles_most_https_certificates?
      ...
      # older OS versions.
      ENV["DRXHUB_SYSTEM_CA_CERTIFICATES_TOO_OLD"].nil?
    end
    ...
    def ca_file_substitution_required?
      (!ca_file_handles_most_https_certificates? || ENV["DRXHUB_FORCE_BREWED_CA_CERTIFICATES"].present?) &&
        !(DRXHUB_PREFIX/"etc/ca-certificates/cert.pem").exist?
    end

    sig { returns(T::Boolean) }
    def curl_substitution_required?
      !curl_handles_most_https_certificates? && !DRXHUB_BREWED_CURL_PATH.exist?
    end
