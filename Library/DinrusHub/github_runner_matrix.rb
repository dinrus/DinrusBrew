...
class GitHubRunnerMatrix
  ...
  def linux_runner_spec
    linux_runner = ENV.fetch("DRXHUB_LINUX_RUNNER")
    ...
    LinuxRunnerSpec.new(
      ...
      container: {
        ...
        options: "--user=linuxbrew -e GITHUB_ACTIONS_DRXHUB_SELF_HOSTED",
      },
      workdir:   "/github/home",
  ...
  NEWEST_DRXHUB_CORE_MACOS_RUNNER = :sequoia
  OLDEST_DRXHUB_CORE_MACOS_RUNNER = :ventura
  NEWEST_DRXHUB_CORE_INTEL_MACOS_RUNNER = :sonoma

  sig { params(macos_version: MacOSVersion).returns(T::Boolean) }
  def runner_enabled?(macos_version)
    macos_version <= NEWEST_DRXHUB_CORE_MACOS_RUNNER && macos_version >= OLDEST_DRXHUB_CORE_MACOS_RUNNER
  end
  ...
  def generate_runners!
    ...
    if !@all_supported || ENV.key?("DRXHUB_LINUX_RUNNER")
      @runners << create_runner(:linux, :x86_64, linux_runner_spec)
    end

    github_run_id      = ENV.fetch("GITHUB_RUN_ID")
    long_timeout       = ENV.fetch("DRXHUB_MACOS_LONG_TIMEOUT", "false") == "true"
    use_github_runner  = ENV.fetch("DRXHUB_MACOS_BUILD_ON_GITHUB_RUNNER", "false") == "true"
    ...
    MacOSVersion::SYMBOLS.each_value do |version|
      ...
      next if !@all_supported && macos_version > NEWEST_DRXHUB_CORE_INTEL_MACOS_RUNNER
