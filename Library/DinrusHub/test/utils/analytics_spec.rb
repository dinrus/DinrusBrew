...
RSpec.describe Utils::Analytics do
  describe "::default_package_tags" do
    ...
    it "returns OS_VERSION and prefix when DRXHUB_PREFIX is a custom prefix on intel" do
      expect(DinrusHub).to receive(:default_prefix?).and_return(false).at_least(:once)
    ...
    it "returns OS_VERSION, ARM and prefix when DRXHUB_PREFIX is a custom prefix on arm" do
      expect(DinrusHub).to receive(:default_prefix?).and_return(false).at_least(:once)
      expect(described_class.default_package_tags).to have_key(:arch)
      expect(described_class.default_package_tags[:arch]).to eq DRXHUB_PHYSICAL_PROCESSOR
      expect(described_class.default_package_tags).to have_key(:prefix)
      expect(described_class.default_package_tags[:prefix]).to eq "custom-prefix"
    end

    it "returns OS_VERSION, Rosetta and prefix when DRXHUB_PREFIX is a custom prefix on Rosetta", :needs_macos do
      expect(DinrusHub).to receive(:default_prefix?).and_return(false).at_least(:once)
      expect(described_class.default_package_tags).to have_key(:prefix)
      expect(described_class.default_package_tags[:prefix]).to eq "custom-prefix"
    end

    it "does not include prefix when DRXHUB_PREFIX is the default prefix" do
      expect(DinrusHub).to receive(:default_prefix?).and_return(true).at_least(:once)
      expect(described_class.default_package_tags).to have_key(:prefix)
      expect(described_class.default_package_tags[:prefix]).to eq DRXHUB_PREFIX.to_s
    end
    ...
    it "includes developer when ENV['DRXHUB_DEVELOPER'] is set" do
      expect(DinrusHub::EnvConfig).to receive(:developer?).and_return(true)
  ...
  describe "::report_package_event" do
    ...
    context "when ENV vars is set" do
      it "returns nil when DRXHUB_NO_ANALYTICS is true" do
        ENV["DRXHUB_NO_ANALYTICS"] = "true"
        expect(described_class).not_to receive(:report_influx)
        described_class.report_package_event(:install, package_name:, tap_name:,
          on_request:, options:)
      end

      it "returns nil when DRXHUB_NO_ANALYTICS_THIS_RUN is true" do
        ENV["DRXHUB_NO_ANALYTICS_THIS_RUN"] = "true"
        expect(described_class).not_to receive(:report_influx)
        described_class.report_package_event(:install, package_name:, tap_name:,
          on_request:, options:)
      end

      it "returns nil when DRXHUB_ANALYTICS_DEBUG is true" do
        ENV.delete("DRXHUB_NO_ANALYTICS_THIS_RUN")
        ENV.delete("DRXHUB_NO_ANALYTICS")
        ENV["DRXHUB_ANALYTICS_DEBUG"] = "true"
  ...
  describe "::report_command_run" do
    ...
    let(:command_instance) do
      ...
      DinrusHub::DevCmd::Audit.new(["--tap=homebrew/core"])
    end
  ...
  describe "::report_test_bot_test" do
    ...
    it "passes to the influxdb method" do
      ...
      ENV.delete("DRXHUB_NO_ANALYTICS")
      ENV["DRXHUB_ANALYTICS_DEBUG"] = "true"
      ENV["DRXHUB_TEST_BOT_ANALYTICS"] = "true"
      expect(described_class).to receive(:report_influx).with(
        :test_bot_test,
