...
RSpec.describe Utils::Git do
  ...
  before do
    git = DRXHUB_SHIMS_PATH/"shared/git"

    DRXHUB_CACHE.cd do
      system git, "init"

      File.write("README.md", "README")
      system git, "add", DRXHUB_CACHE/"README.md"
      ...
      system git, "add", DRXHUB_CACHE/"LICENSE.txt"
      system git, "commit", "-m", "File added"
  ...
  describe "#cherry_pick!" do
    it "can cherry pick a commit" do
      expect(described_class.cherry_pick!(DRXHUB_CACHE, cherry_pick_commit)).to be_truthy
    end

    it "aborts when cherry picking an existing hash" do
      ENV["GIT_MERGE_VERBOSITY"] = "5" # Consistent output across git versions
      expect do
        described_class.cherry_pick!(DRXHUB_CACHE, file_hash_one)
  ...
  describe "#last_revision_commit_of_file" do
    it "gives last revision commit when before_commit is nil" do
      expect(
        described_class.last_revision_commit_of_file(DRXHUB_CACHE, file),
      ).to eq(file_hash_one)
    end

    it "gives revision commit based on before_commit when it is not nil" do
      expect(
        described_class.last_revision_commit_of_file(DRXHUB_CACHE,
                                                     file,
  ...
  describe "#file_at_commit" do
    it "returns file contents when file exists" do
      expect(described_class.file_at_commit(DRXHUB_CACHE, file, file_hash_one)).to eq("README")
    end

    it "returns empty when file doesn't exist" do
      expect(described_class.file_at_commit(DRXHUB_CACHE, "foo.txt", file_hash_one)).to eq("")
      expect(described_class.file_at_commit(DRXHUB_CACHE, "LICENSE.txt", file_hash_one)).to eq("")
    end
  end
  ...
  describe "#last_revision_commit_of_files" do
    context "when before_commit is nil" do
      it "gives last revision commit" do
        expect(
          described_class.last_revision_commit_of_files(DRXHUB_CACHE, files),
    ...
    context "when before_commit is not nil" do
      it "gives last revision commit" do
        expect(
          described_class.last_revision_commit_of_files(DRXHUB_CACHE,
                                                        files,
                                                        before_commit: file_hash_two),
  ...
  describe "#last_revision_of_file" do
    it "returns last revision of file" do
      expect(
        described_class.last_revision_of_file(DRXHUB_CACHE,
                                              DRXHUB_CACHE/file),
      ).to eq("README")
    end
    ...
    it "returns last revision of file based on before_commit" do
      expect(
        described_class.last_revision_of_file(DRXHUB_CACHE, DRXHUB_CACHE/file,
                                              before_commit: "0..3"),
      ).to eq("# README")
    end
  end
  ...
  describe "::available?" do
    ...
    it "returns false if git --version command does not succeed" do
      stub_const("DRXHUB_SHIMS_PATH", DRXHUB_PREFIX/"bin/shim")
  ...
  describe "::ensure_installed!" do
    ...
    context "when git is not already available" do
      ...
      it "raises error if can't install git" do
        stub_const("DRXHUB_BREW_FILE", DRXHUB_PREFIX/"bin/dhub")
        expect { described_class.ensure_installed! }.to raise_error("Git is unavailable")
      end

      unless ENV["DRXHUB_TEST_GENERIC_OS"]
        it "installs git" do
