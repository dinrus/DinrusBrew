...
RSpec.describe Utils do
  shared_examples "git_repository helper function" do |method_name|
    ...
    context "when Git is unavailable" do
      ...
      it "returns nil if `safe` parameter is `false`" do
        expect(described_class.public_send(method_name, DRXHUB_CACHE, safe: false)).to be_nil
      end

      it "raises an error if `safe` parameter is `true`" do
        expect { described_class.public_send(method_name, DRXHUB_CACHE, safe: true) }
  ...
  let(:head_revision) { DRXHUB_CACHE.cd { `git rev-parse HEAD`.chomp } }
  let(:short_head_revision) { DRXHUB_CACHE.cd { `git rev-parse --short HEAD`.chomp } }

  describe "::git_head" do
    it "returns the revision at HEAD" do
      expect(described_class.git_head(DRXHUB_CACHE)).to eq(head_revision)
      expect(described_class.git_head(DRXHUB_CACHE, length: 5)).to eq(head_revision[0...5])
      DRXHUB_CACHE.cd do
  ...
  describe "::git_short_head" do
    it "returns the short revision at HEAD" do
      expect(described_class.git_short_head(DRXHUB_CACHE)).to eq(short_head_revision)
      expect(described_class.git_short_head(DRXHUB_CACHE, length: 5)).to eq(head_revision[0...5])
      DRXHUB_CACHE.cd do
  ...
  describe "::git_branch" do
    ...
    it "returns the current Git branch" do
      expect(described_class.git_branch(DRXHUB_CACHE)).to eq(branch_name)
      DRXHUB_CACHE.cd do
  ...
  describe "::git_commit_message" do
    ...
    it "returns the commit message of HEAD" do
      expect(described_class.git_commit_message(DRXHUB_CACHE)).to eq(commit_message)
      expect(described_class.git_commit_message(DRXHUB_CACHE, commit: head_revision)).to eq(commit_message)
      DRXHUB_CACHE.cd do
