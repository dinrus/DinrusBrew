...
RSpec.describe Utils do
  describe "ruby_check_version_script" do
    subject do
      homebrew_env = ENV.select { |key, _| key.start_with?("DRXHUB_") }
      Bundler.with_unbundled_env do
        ENV.delete_if { |key,| key.start_with?("DRXHUB_") }
        ENV.update(homebrew_env)
        quiet_system "#{DRXHUB_LIBRARY_PATH}/utils/ruby_check_version_script.rb", required_ruby_version
      end
    end

    before do
      ENV.delete("DRXHUB_DEVELOPER")
      ENV.delete("DRXHUB_USE_RUBY_FROM_PATH")
    end
    ...
    describe "succeeds on newer mismatched major/minor required Ruby version and configured environment" do
      ...
      before do
        ...
        ENV["DRXHUB_USE_RUBY_FROM_PATH"] = "1"
      end
