...
RSpec.describe Utils::Bottles::Collector do
  ...
  describe "#specification_for" do
    ...
    it "does not use older tags when requested not to", :needs_macos do
      allow(DinrusHub::EnvConfig).to receive_messages(developer?: true, skip_or_later_bottles?: true)
    ...
    it "ignores DRXHUB_SKIP_OR_LATER_BOTTLES on release versions", :needs_macos do
      allow(DinrusHub::EnvConfig).to receive(:skip_or_later_bottles?).and_return(true)
