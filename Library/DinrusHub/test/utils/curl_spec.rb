...
RSpec.describe "Utils::Curl" do
  ...
  describe "::curl_executable" do
    it "returns `DRXHUB_BREWED_CURL_PATH` when `use_homebrew_curl` is `true`" do
      expect(curl_executable(use_homebrew_curl: true)).to eq(DRXHUB_BREWED_CURL_PATH)
    end

    it "returns curl shim path when `use_homebrew_curl` is `false` or omitted" do
      curl_shim_path = DRXHUB_SHIMS_PATH/"shared/curl"
  ...
  describe "::curl_args" do
    ...
    it "returns `--disable` as the first argument when DRXHUB_CURLRC is not set" do
      # --disable must be the first argument according to "man curl"
      expect(curl_args(*args).first).to eq("--disable")
    end

    it "doesn't return `--disable` as the first argument when DRXHUB_CURLRC is set but not a path" do
      ENV["DRXHUB_CURLRC"] = "1"
      expect(curl_args(*args).first).not_to eq("--disable")
    end

    it "doesn't return `--config` when DRXHUB_CURLRC is unset" do
      expect(curl_args(*args)).not_to include(a_string_starting_with("--config"))
    end

    it "returns `--config` when DRXHUB_CURLRC is a valid path" do
      Tempfile.create do |tmpfile|
        path = tmpfile.path
        ENV["DRXHUB_CURLRC"] = path
        # We still expect --disable
        expect(curl_args(*args).first).to eq("--disable")
        expect(curl_args(*args).join(" ")).to include("--config #{path}")
      end
    ensure
      ENV["DRXHUB_CURLRC"] = nil
    end
    ...
    it "uses `--retry 3` when DRXHUB_CURL_RETRIES is unset" do
      expect(curl_args(*args).join(" ")).to include("--retry 3")
    end

    it "uses the given value for `--retry` when DRXHUB_CURL_RETRIES is set" do
      ENV["DRXHUB_CURL_RETRIES"] = "10"
    ...
    it "uses DRXHUB_USER_AGENT_FAKE_SAFARI when `:user_agent` is `:browser` or `:fake`" do
      expect(curl_args(*args, user_agent: :browser).join(" "))
        .to include("--user-agent #{DRXHUB_USER_AGENT_FAKE_SAFARI}")
      expect(curl_args(*args, user_agent: :fake).join(" "))
        .to include("--user-agent #{DRXHUB_USER_AGENT_FAKE_SAFARI}")
    end

    it "uses DRXHUB_USER_AGENT_CURL when `:user_agent` is `:default` or omitted" do
      expect(curl_args(*args, user_agent: :default).join(" ")).to include("--user-agent #{DRXHUB_USER_AGENT_CURL}")
      expect(curl_args(*args, user_agent: nil).join(" ")).to include("--user-agent #{DRXHUB_USER_AGENT_CURL}")
      expect(curl_args(*args).join(" ")).to include("--user-agent #{DRXHUB_USER_AGENT_CURL}")
    end
    ...
    context "when `EnvConfig.curl_verbose?` is `true`" do
      before do
        allow(DinrusHub::EnvConfig).to receive(:curl_verbose?).and_return(true)
      end
    ...
    context "when `EnvConfig.curl_verbose?` is `false`" do
      before do
        allow(DinrusHub::EnvConfig).to receive(:curl_verbose?).and_return(false)
      end
