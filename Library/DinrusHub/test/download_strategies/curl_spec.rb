...
RSpec.describe CurlDownloadStrategy do
  ...
  describe "#fetch" do
    before do
      allow(DinrusHub::EnvConfig).to receive(:artifact_domain).and_return(artifact_domain)
  ...
  describe "#cached_location" do
    ...
    context "when URL ends with file" do
      it "falls back to the file name in the URL" do
        expect(cached_location).to eq(
          DRXHUB_CACHE/"downloads/3d1c0ae7da22be9d83fb1eb774df96b7c4da71d3cf07e1cb28555cf9a5e5af70--foo.tar.gz",
        )
      end
    end
    ...
    context "when URL file is in middle" do
      ...
      it "falls back to the file name in the URL" do
        expect(cached_location).to eq(
          DRXHUB_CACHE/"downloads/1ab61269ba52c83994510b1e28dd04167a2f2e8393a35a9c50c1f7d33fd8f619--foo.tar.gz",
        )
      end
    end
