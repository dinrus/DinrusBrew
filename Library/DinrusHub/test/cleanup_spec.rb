...
RSpec.describe DinrusHub::Cleanup do
  subject(:cleanup) { described_class.new }

  let(:ds_store) { Pathname.new("#{DRXHUB_CELLAR}/.DS_Store") }
  let(:lock_file) { Pathname.new("#{DRXHUB_LOCKS}/foo") }
  ...
  describe "::cleanup_logs" do
    let(:path) { (DRXHUB_LOGS/"delete_me") }
  ...
  describe "::cleanup_cache" do
    it "cleans up incomplete downloads" do
      incomplete = (DRXHUB_CACHE/"something.incomplete")
      incomplete.mkpath
    ...
    it "cleans up 'cargo_cache'" do
      cargo_cache = (DRXHUB_CACHE/"cargo_cache")
      cargo_cache.mkpath
    ...
    it "cleans up 'go_cache'" do
      go_cache = (DRXHUB_CACHE/"go_cache")
      go_cache.mkpath
    ...
    it "cleans up 'glide_home'" do
      glide_home = (DRXHUB_CACHE/"glide_home")
      glide_home.mkpath
    ...
    it "cleans up 'java_cache'" do
      java_cache = (DRXHUB_CACHE/"java_cache")
      java_cache.mkpath
    ...
    it "cleans up 'npm_cache'" do
      npm_cache = (DRXHUB_CACHE/"npm_cache")
      npm_cache.mkpath
    ...
    it "cleans up 'gclient_cache'" do
      gclient_cache = (DRXHUB_CACHE/"gclient_cache")
    ...
    it "cleans up all files and directories" do
      git = (DRXHUB_CACHE/"gist--git")
      gist = (DRXHUB_CACHE/"gist")
      svn = (DRXHUB_CACHE/"gist--svn")
    ...
    context "when cleaning old files in DRXHUB_CACHE" do
      let(:bottle) { (DRXHUB_CACHE/"testball--0.0.1.tag.bottle.tar.gz") }
      let(:testball) { (DRXHUB_CACHE/"testball--0.0.1") }
      let(:testball_resource) { (DRXHUB_CACHE/"testball--rsrc--0.0.1.txt") }

      before do
        FileUtils.touch bottle
        FileUtils.touch testball
        FileUtils.touch testball_resource
        (DRXHUB_CELLAR/"testball"/"0.0.1").mkpath
        # Create the latest version of testball so the older version is eligible for cleanup.
        (DRXHUB_CELLAR/"testball"/"0.1/bin").mkpath
  ...
  describe "::cleanup_python_site_packages" do
    context "when cleaning up Python modules" do
      let(:foo_module) { (DRXHUB_PREFIX/"lib/python3.99/site-packages/foo") }
    ...
    it "cleans up stale `*.pyc` files in the top-level `__pycache__`" do
      pycache = DRXHUB_PREFIX/"lib/python3.99/site-packages/__pycache__"
