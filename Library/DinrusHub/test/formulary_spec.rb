...
RSpec.describe Formulary do
  ...
  describe "::factory" do
    context "without the API" do
      ...
      it "errors when given a path but paths are disabled" do
        ENV["DRXHUB_FORBID_PACKAGES_FROM_PATHS"] = "1"
        FileUtils.cp formula_path, DRXHUB_TEMP
        temp_formula_path = DRXHUB_TEMP/formula_path.basename
        expect do
    ...
    context "with the API" do
      ...
      before do
        ...
        allow(DinrusHub::API::Formula).to receive_messages(all_aliases: {}, all_renames: {})
      ...
      it "returns a Formula when given a name" do
        allow(DinrusHub::API::Formula).to receive(:all_formulae).and_return formula_json_contents
        ...
        expect(formula.service.command).to eq(["#{DRXHUB_PREFIX}/opt/formula_name/bin/beanstalkd", "test"])
      ...
      it "returns a deprecated Formula when given a name" do
        allow(DinrusHub::API::Formula).to receive(:all_formulae).and_return formula_json_contents(deprecate_json)
      ...
      it "returns a disabled Formula when given a name" do
        allow(DinrusHub::API::Formula).to receive(:all_formulae).and_return formula_json_contents(disable_json)
      ...
      it "returns a Formula with variations when given a name", :needs_macos do
        allow(DinrusHub::API::Formula).to receive(:all_formulae).and_return formula_json_contents(variations_json)
  ...
  describe "::to_rack" do
    ...
    let(:rack_path) { DRXHUB_CELLAR/formula_name }
  ...
  describe "::core_path" do
    it "returns the path to a Formula in the core tap" do
      ...
      expect(described_class.core_path(name))
        .to eq(Pathname.new("#{DRXHUB_LIBRARY}/Taps/homebrew/homebrew-core/Formula/#{name}.rb"))
    end
  end
    ...
    context "when not using the API" do
      ...
      context "when a formula is migrated" do
        ...
        context "to a third-party tap" do
          ...
          after do
            FileUtils.rm_rf DRXHUB_TAP_DIRECTORY/"another"
          end
