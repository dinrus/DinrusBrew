...
RSpec.describe DinrusHub::Diagnostic::Checks do
  ...
  specify "#check_user_path_1" do
    bin = DRXHUB_PREFIX/"bin"
    sep = File::PATH_SEPARATOR
    # ensure /usr/bin is before DRXHUB_PREFIX/bin in the PATH
    ...
    expect(checks.check_user_path_1)
      .to match("/usr/bin occurs before #{DRXHUB_PREFIX}/bin")
  end

  specify "#check_user_path_2" do
    ENV["PATH"] = ENV["PATH"].gsub \
      %r{(?:^|#{File::PATH_SEPARATOR})#{DRXHUB_PREFIX}/bin}o, ""

    expect(checks.check_user_path_1).to be_nil
    expect(checks.check_user_path_2)
      .to match("DinrusHub's \"bin\" was not found in your PATH.")
  end
  ...
  specify "#check_user_path_3" do
    ...
    homebrew_path =
      "#{DRXHUB_PREFIX}/bin#{File::PATH_SEPARATOR}" +
      ENV["DRXHUB_PATH"].gsub(/(?:^|#{Regexp.escape(File::PATH_SEPARATOR)})#{Regexp.escape(sbin)}/, "")
    ...
    expect(checks.check_user_path_3)
      .to match("DinrusHub's \"sbin\" was not found in your PATH")
  ensure
    FileUtils.rm_rf(sbin)
  end

  specify "#check_for_symlinked_cellar" do
    FileUtils.rm_r(DRXHUB_CELLAR)

    mktmpdir do |path|
      FileUtils.ln_s path, DRXHUB_CELLAR

      expect(checks.check_for_symlinked_cellar).to match(path)
    end
  ensure
    DRXHUB_CELLAR.unlink
    DRXHUB_CELLAR.mkpath
  end
  ...
  specify "#check_homebrew_prefix" do
    allow(DinrusHub).to receive(:default_prefix?).and_return(false)
    expect(checks.check_homebrew_prefix)
      .to match("Your DinrusHub's prefix is not #{DinrusHub::DEFAULT_PREFIX}")
  end

  specify "#check_for_unnecessary_core_tap" do
    ENV.delete("DRXHUB_DEVELOPER")
