...
RSpec.describe DinrusHub::API do
  let(:text) { "foo" }
  let(:json) { '{"foo":"bar"}' }
  ...
  describe "::tap_from_source_download" do
    let(:api_cache_root) { DinrusHub::API::DRXHUB_CACHE_API_SOURCE }
    let(:cache_path) do
      api_cache_root/"DinrusHub"/"homebrew-core"/"cf5c386c1fa2cb54279d78c0990dd7a0fa4bc327"/"Formula"/"foo.rb"
    end
