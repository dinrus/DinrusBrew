...
RSpec.describe DinrusHub::API::Cask do
  let(:cache_dir) { mktmpdir }

  before do
    stub_const("DinrusHub::API::DRXHUB_CACHE_API", cache_dir)
  end
  ...
  def mock_curl_download(stdout:)
    ...
    allow(DinrusHub::API).to receive(:verify_and_parse_jws) do |json_data|
      [true, json_data]
    end
  end
  ...
  describe "::source_download", :needs_macos do
    ...
    before do
      allow_any_instance_of(DinrusHub::API::Download).to receive(:fetch)
      allow_any_instance_of(DinrusHub::API::Download).to receive(:symlink_location).and_return(
        TEST_FIXTURE_DIR/"cask/Casks/everything.rb",
      )
    end

    it "specifies the correct URL and sha256" do
      expect(DinrusHub::API::Download).to receive(:new).with(
