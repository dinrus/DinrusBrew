...
RSpec.describe "Internal Tap JSON -- Formula", type: :system do
  ...
  context "when generating JSON", :needs_macos do
    before do
      cp_r(TEST_FIXTURE_DIR/"internal_tap_json/homebrew-core", DRXHUB_TAP_DIRECTORY/"homebrew")

      # NOTE: Symlinks can't be copied recursively so we create them manually here.
      (DRXHUB_TAP_DIRECTORY/"homebrew/homebrew-core").tap do |core_tap|
  ...
  context "when loading JSON" do
    before do
      ENV["DRXHUB_INTERNAL_JSON_V3"] = "1"
      ENV.delete("DRXHUB_NO_INSTALL_FROM_API")

      allow(DinrusHub::API).to receive(:fetch_json_api_file)
    ...
    it "loads tap git head" do
      expect(DinrusHub::API::Formula.tap_git_head)
        .to eq(tap_git_head)
    end
