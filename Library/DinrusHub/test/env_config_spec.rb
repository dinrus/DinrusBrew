...
RSpec.describe DinrusHub::EnvConfig do
  ...
  describe ".env_method_name" do
    it "generates method names" do
      expect(env_config.env_method_name("DRXHUB_FOO", {})).to eql("foo")
    end

    it "generates boolean method names" do
      expect(env_config.env_method_name("DRXHUB_BAR", boolean: true)).to eql("bar?")
    end
  end

  describe ".artifact_domain" do
    it "returns value if set" do
      ENV["DRXHUB_ARTIFACT_DOMAIN"] = "https://brew.sh"
      expect(env_config.artifact_domain).to eql("https://brew.sh")
    end

    it "returns nil if empty" do
      ENV["DRXHUB_ARTIFACT_DOMAIN"] = ""
