...
RSpec.describe DinrusHub::DevCmd::Bottle do
    ...
    <<~EOS
      {
        "#{parameters[:name]}":{
           ...
           "bottle":{
              "root_url":"#{parameters[:root_url] || DRXHUB_BOTTLE_DEFAULT_DOMAIN}",
              "prefix":"/usr/local",
  ...
  it "builds a bottle for the given Formula", :integration_test, :needs_network do
    ...
    # `dhub bottle` should not fail with dead symlink
    # https://github.com/Homebrew/legacy-homebrew/issues/49007
    (DRXHUB_CELLAR/"testball/0.1").cd do
      FileUtils.ln_s "not-exist", "symlink"
    end

    begin
      expect { dhub "bottle", "--no-rebuild", "testball" }
        .to output(/testball--0\.1.*\.bottle\.tar\.gz/).to_stdout
        .and not_to_output.to_stderr
        .and be_a_success
      expect(DRXHUB_CELLAR/"testball-bottle.tar").not_to exist
    ensure
  ...
  describe "bottle_cmd" do
    ...
    describe "#merge_bottle_spec" do
      ...
      it "checks for conflicting checksums" do
        ...
        expected_checksum_hash[:cellar] = DinrusHub::DEFAULT_MACOS_CELLAR
    ...
    describe "::generate_sha256_line" do
      ...
      it "generates a string with default cellar path" do
        expect(homebrew.generate_sha256_line(:catalina, "deadbeef", DinrusHub::DEFAULT_LINUX_CELLAR, 0, 10)).to eq(
          <<~RUBY.chomp,
            sha256 catalina: "deadbeef"
          RUBY
        )
      ...
      context "with offsets" do
        ...
        it "generates a string with default cellar path" do
          expect(homebrew.generate_sha256_line(:catalina, "deadbeef", DinrusHub::DEFAULT_LINUX_CELLAR, 14, 30)).to eq(
            <<~RUBY.chomp,
              sha256               catalina:       "deadbeef"
            RUBY
          )
