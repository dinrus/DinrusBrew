...
RSpec.describe DinrusHub::DevCmd::DetermineTestRunners do
  ...
  let(:runner_env) do
    {
      "DRXHUB_LINUX_RUNNER"       => linux_runner,
      "DRXHUB_MACOS_LONG_TIMEOUT" => "false",
  ...
  let(:all_runners) do
    ...
    MacOSVersion::SYMBOLS.each_value do |v|
      ...
      next if macos_version < GitHubRunnerMatrix::OLDEST_DRXHUB_CORE_MACOS_RUNNER
      next if macos_version > GitHubRunnerMatrix::NEWEST_DRXHUB_CORE_MACOS_RUNNER

      out << "#{v}-arm64"
      next if macos_version > GitHubRunnerMatrix::NEWEST_DRXHUB_CORE_INTEL_MACOS_RUNNER
  ...
  it "assigns all runners for formulae without any requirements", :integration_test do
    ...
    expect { dhub "determine-test-runners", "testball", runner_env.merge({ "GITHUB_OUTPUT" => github_output }) }
