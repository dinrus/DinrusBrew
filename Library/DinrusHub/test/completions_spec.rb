...
RSpec.describe DinrusHub::Completions do
  let(:completions_dir) { DRXHUB_REPOSITORY/"completions" }
  let(:internal_path) { DRXHUB_REPOSITORY/"Library/Taps/homebrew/homebrew-bar" }
  let(:external_path) { DRXHUB_REPOSITORY/"Library/Taps/foo/homebrew-bar" }
  ...
  context "when linking or unlinking completions" do
    ...
    describe ".show_completions_message_if_needed" do
      ...
      it "shows the message if there are completions to link and the message hasn't already been shown" do
        ...
        message = /DinrusHub completions for external commands are unlinked by default!/
  ...
  context "when generating completions" do
    ...
    #     expect(completions_dir/"bash/dhub").to be_a_file
    #   end
    # end

    describe ".format_description" do
      it "escapes single quotes" do
        expect(described_class.format_description("DinrusHub's")).to eq "DinrusHub'\\''s"
      end

      it "escapes single quotes for fish" do
        expect(described_class.format_description("DinrusHub's", fish: true)).to eq "DinrusHub\\'s"
      end
      ...
      it "replaces newlines with spaces" do
        expect(described_class.format_description("DinrusHub\ncommand")).to eq "DinrusHub command"
      end

      it "removes trailing period" do
        expect(described_class.format_description("DinrusHub.")).to eq "DinrusHub"
      end
    end
    ...
    describe ".command_options" do
      ...
      it "returns an array of options for a shell command" do
        expected_options = {
          "--auto-update" => "Run on auto-updates (e.g. before `dhub install`). Skips some slower steps.",
    ...
    describe ".generate_zsh_completion_file" do
      it "returns the correct completion file" do
        ...
        expect(file).to match(/^    'update:Fetch the newest version of DinrusHub and all formulae from GitHub .*'$/)
        expect(file).to match(/^_brew_install\(\) {$/)
        expect(file).to match(/^_brew_missing\(\) {$/)
        expect(file).to match(/^_brew_update\(\) {$/)
        expect(file).to match(/^_brew "\$@"$/)
    ...
    describe ".generate_fish_subcommand_completion" do
      ...
      it "returns appropriate completion for a shell command" do
        ...
        expect(completion).to eq <<~COMPLETION
          __fish_brew_complete_cmd 'update' 'Fetch the newest version of DinrusHub and all formulae from GitHub using `git`(1) and perform any necessary migrations'
          __fish_brew_complete_arg 'update' -l auto-update -d 'Run on auto-updates (e.g. before `dhub install`). Skips some slower steps'
    ...
    describe ".generate_fish_completion_file" do
      it "returns the correct completion file" do
        ...
        expect(file).to match(/^__fish_brew_complete_cmd 'update' 'Fetch the newest version of DinrusHub .*'$/)
      end
    end
  end
end
