...
RSpec.describe Tapioca::Compilers::Args do
  let(:compiler) { described_class.new(Tapioca::Dsl::Pipeline.new(requested_constants: []), RBI::Tree.new, DinrusHub) }
  let(:list_parser) do
    require "cmd/list"
    DinrusHub::Cmd::List.parser
  end
  ...
  let(:update_python_resources_parser) do
    ...
    DinrusHub::DevCmd::UpdatePythonResources.parser
  end
