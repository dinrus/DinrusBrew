...
RSpec.describe FormulaPin do
  ...
  let(:name) { "double" }
  let(:formula) { instance_double(Formula, name:, rack: DRXHUB_CELLAR/name) }
  ...
  specify "#pin and #unpin" do
    ...
    expect(DRXHUB_PINNED_KEGS/name).to be_a_directory
    expect(DRXHUB_PINNED_KEGS.children.count).to eq(1)

    formula_pin.unpin
    expect(formula_pin).not_to be_pinned
    expect(DRXHUB_PINNED_KEGS).not_to be_a_directory
  end
end
