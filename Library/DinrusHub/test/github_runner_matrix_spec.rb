...
RSpec.describe GitHubRunnerMatrix do
  before do
    allow(ENV).to receive(:fetch).with("DRXHUB_LINUX_RUNNER").and_return("ubuntu-latest")
    allow(ENV).to receive(:fetch).with("DRXHUB_MACOS_LONG_TIMEOUT", "false").and_return("false")
    allow(ENV).to receive(:fetch).with("DRXHUB_MACOS_BUILD_ON_GITHUB_RUNNER", "false").and_return("false")
    allow(ENV).to receive(:fetch).with("GITHUB_RUN_ID").and_return("12345")
    allow(ENV).to receive(:fetch).with("DRXHUB_NO_INSTALL_FROM_API", nil).and_call_original
    allow(ENV).to receive(:fetch).with("DRXHUB_EVAL_ALL", nil).and_call_original
    allow(ENV).to receive(:fetch).with("DRXHUB_SIMULATE_MACOS_ON_LINUX", nil).and_call_original
    allow(ENV).to receive(:fetch).with("DRXHUB_FORBID_PACKAGES_FROM_PATHS", nil).and_call_original
  end

  let(:newest_supported_macos) do
    MacOSVersion::SYMBOLS.find { |k, _| k == described_class::NEWEST_DRXHUB_CORE_MACOS_RUNNER }
  end
    ...
    context "when it is a matrix for the `test_deps` job" do
      context "when testing formulae have no dependents" do
        it "activates no runners" do
          allow(DinrusHub::EnvConfig).to receive(:eval_all?).and_return(true)
