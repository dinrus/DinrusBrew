...
RSpec.describe Formula do
  ...
  example "installed alias with tap" do
    ...
    FileUtils.rm_rf DRXHUB_LIBRARY/"Taps/user"
  end

  specify "#prefix" do
    f = Testball.new
    expect(f.prefix).to eq(DRXHUB_CELLAR/f.name/"0.1")
    expect(f.prefix).to be_a(Pathname)
  end

  example "revised prefix" do
    f = Class.new(Testball) { revision(1) }.new
    expect(f.prefix).to eq(DRXHUB_CELLAR/f.name/"0.1_1")
  end
  ...
  specify "#any_version_installed?" do
    ...
    prefix = DRXHUB_CELLAR/f.name/"0.1"
    prefix.mkpath
  ...
  specify "#migration_needed" do
    ...
    oldname_prefix = (DRXHUB_CELLAR/"oldname/2.20")
    newname_prefix = (DRXHUB_CELLAR/"newname/2.10")
  ...
  describe "#latest_installed_prefix" do
    ...
    let(:stable_prefix) { DRXHUB_CELLAR/f.name/f.version }
    let(:head_prefix) { DRXHUB_CELLAR/f.name/f.head.version }
  ...
  describe "#latest_head_prefix" do
    ...
    it "returns the latest head prefix" do
      ...
      prefix = DRXHUB_CELLAR/f.name/"HEAD-222222_2"
  ...
  specify "test fixtures" do
    ...
    expect(f1.test_fixtures("foo")).to eq(Pathname.new("#{DRXHUB_LIBRARY_PATH}/test/support/fixtures/foo"))
  end
  ...
  describe "#service" do
    ...
    specify "service helpers return data" do
      ...
      expect(f.launchd_service_path).to eq(DRXHUB_PREFIX/"opt/formula_name/homebrew.mxcl.formula_name.plist")
      expect(f.systemd_service_path).to eq(DRXHUB_PREFIX/"opt/formula_name/homebrew.formula_name.service")
      expect(f.systemd_timer_path).to eq(DRXHUB_PREFIX/"opt/formula_name/homebrew.formula_name.timer")
    end
  end
  ...
  describe "#eligible_kegs_for_cleanup" do
    ...
    specify "with pinned Keg" do
      ...
      expect(f1.prefix).to eq((DRXHUB_PINNED_KEGS/f1.name).resolved_path)
      expect(f1).to be_latest_version_installed
      expect(f2).to be_latest_version_installed
      expect(f3).to be_latest_version_installed
  ...
  describe "#outdated_kegs" do
    let(:outdated_prefix) { (DRXHUB_CELLAR/"#{f.name}/1.11") }
    let(:same_prefix) { (DRXHUB_CELLAR/"#{f.name}/1.20") }
    let(:greater_prefix) { (DRXHUB_CELLAR/"#{f.name}/1.21") }
    let(:head_prefix) { (DRXHUB_CELLAR/"#{f.name}/HEAD") }
    let(:old_alias_target_prefix) { (DRXHUB_CELLAR/"#{old_formula.name}/1.0") }
    ...
    example "outdated mixed taps outdated version installed" do
      ...
      extra_outdated_prefix = DRXHUB_CELLAR/f.name/"1.0"
    ...
    describe ":fetch_head" do
      ...
      let(:testball_repo) { DRXHUB_PREFIX/"testball_repo" }

      example do
        outdated_stable_prefix = DRXHUB_CELLAR/"testball/1.0"
        head_prefix_a = DRXHUB_CELLAR/"testball/HEAD"
        head_prefix_b = DRXHUB_CELLAR/"testball/HEAD-aaaaaaa_1"
        head_prefix_c = DRXHUB_CELLAR/"testball/HEAD-18a7103"
    ...
    describe "with changed version scheme" do
      ...
      example do
        prefix = DRXHUB_CELLAR/"testball/0.1"
    ...
    describe "with version scheme" do
      ...
      example do
        head_prefix = DRXHUB_CELLAR/"testball/HEAD"
  ...
  describe "#on_system" do
    ...
    it "doesn't call code on Ventura", :needs_macos do
      DinrusHub::SimulateSystem.with os: :ventura do
        f.dhub { f.install }
    ...
    it "calls code on Linux", :needs_linux do
      DinrusHub::SimulateSystem.with os: :linux do
        f.dhub { f.install }
    ...
    it "calls code within `on_system :linux, macos: :monterey` on Monterey", :needs_macos do
      DinrusHub::SimulateSystem.with os: :monterey do
        f.dhub { f.install }
        expect(f.foo).to eq(1)
        expect(f.bar).to eq(0)
      end
    ...
    it "calls code within `on_system :linux, macos: :big_sur_or_older` on Big Sur", :needs_macos do
      DinrusHub::SimulateSystem.with os: :big_sur do
        f.dhub { f.install }
        expect(f.foo).to eq(0)
        expect(f.bar).to eq(1)
      end
    ...
    it "calls code within `on_system :linux, macos: :big_sur_or_older` on Catalina", :needs_macos do
      DinrusHub::SimulateSystem.with os: :catalina do
        f.dhub { f.install }
        expect(f.foo).to eq(0)
        expect(f.bar).to eq(1)
      end
  ...
  describe "on_{os_version} blocks", :needs_macos do
    ...
    it "only calls code within `on_catalina :or_older`" do
      DinrusHub::SimulateSystem.with os: :mojave do
        f.dhub { f.install }
        expect(f.test).to eq(3)
      end
    end
  ...
  describe "#specified_path" do
    ...
    context "when loaded from the API" do
      ...
      it "returns the API path" do
        expect(f.specified_path).to eq(DinrusHub::API::Formula.cached_json_file_path)
      end
    end
  end
end
