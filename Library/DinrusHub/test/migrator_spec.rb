...
RSpec.describe Migrator do
  ...
  let(:new_keg_record) { DRXHUB_CELLAR/"newname/0.1" }
  let(:old_keg_record) { DRXHUB_CELLAR/"oldname/0.1" }

  let(:old_tab) { Tab.empty }

  let(:keg) { Keg.new(old_keg_record) }
  let(:old_pin) { DRXHUB_PINNED_KEGS/"oldname" }
  ...
  before do |example|
    ...
    old_tab.tabfile = DRXHUB_CELLAR/"oldname/0.1/INSTALL_RECEIPT.json"
  ...
  describe "::new" do
    ...
    it "raises an error if the Taps differ" do
      keg = DRXHUB_CELLAR/"oldname/0.1"
      keg.mkpath
      tab = Tab.empty
      tab.tabfile = DRXHUB_CELLAR/"oldname/0.1/INSTALL_RECEIPT.json"
  ...
  specify "#repin" do
    (new_keg_record/"bin").mkpath
    expected_relative = new_keg_record.relative_path_from DRXHUB_PINNED_KEGS
  ...
  specify "#unlink_oldname" do
    expect(DRXHUB_LINKED_KEGS.children.count).to eq(1)
    expect((DRXHUB_PREFIX/"opt").children.count).to eq(1)

    migrator.unlink_oldname

    expect(DRXHUB_LINKED_KEGS).not_to exist
    expect(DRXHUB_LIBRARY/"bin").not_to exist
  end
  ...
  specify "#link_oldname_opt" do
    ...
    expect((DRXHUB_PREFIX/"opt/oldname").realpath).to eq(new_keg_record.realpath)
  end
  ...
  specify "#link_oldname_cellar" do
    ...
    expect((DRXHUB_CELLAR/"oldname").realpath).to eq(new_keg_record.parent.realpath)
  end

  specify "#update_tabs" do
    (new_keg_record/"bin").mkpath
    tab = Tab.empty
    tab.tabfile = DRXHUB_CELLAR/"newname/0.1/INSTALL_RECEIPT.json"
  ...
  specify "#migrate" do
    ...
    expect(DRXHUB_LINKED_KEGS/"oldname").not_to exist
    expect((DRXHUB_LINKED_KEGS/"newname").realpath).to eq(new_keg_record.realpath)
    expect(old_keg_record.realpath).to eq(new_keg_record.realpath)
    expect((DRXHUB_PREFIX/"opt/oldname").realpath).to eq(new_keg_record.realpath)
    expect((DRXHUB_CELLAR/"oldname").realpath).to eq(new_keg_record.parent.realpath)
    expect((DRXHUB_PINNED_KEGS/"newname").realpath).to eq(new_keg_record.realpath)
    expect(Tab.for_keg(new_keg_record).source["path"]).to eq(new_formula.path.to_s)
  end

  specify "#unlinik_oldname_opt" do
    new_keg_record.mkpath
    old_opt_record = DRXHUB_PREFIX/"opt/oldname"
  ...
  describe "#backup_oldname" do
    context "when cellar exists" do
      it "backs up the old name" do
        ...
        expect(DRXHUB_LINKED_KEGS/"oldname").to exist
        expect(DRXHUB_PREFIX/"opt/oldname").to exist
        expect(DRXHUB_PINNED_KEGS/"oldname").to be_a_symlink
