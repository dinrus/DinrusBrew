...
RSpec.describe "Bash" do
  ...
  describe "dhub" do
    subject { DRXHUB_LIBRARY_PATH.parent.parent/"bin/dhub" }
  ...
  describe "every `.sh` file" do
    it "has valid Bash syntax" do
      Pathname.glob("#{DRXHUB_LIBRARY_PATH}/**/*.sh").each do |path|
        relative_path = path.relative_path_from(DRXHUB_LIBRARY_PATH)
  ...
  describe "Bash completion" do
    subject { DRXHUB_LIBRARY_PATH.parent.parent/"completions/bash/dhub" }
  ...
  describe "every shim script" do
    it "has valid Bash syntax" do
      ...
      (DRXHUB_LIBRARY_PATH/"shims").find do |path|
