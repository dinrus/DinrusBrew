...
$LOAD_PATH.unshift(File.expand_path("#{ENV.fetch("DRXHUB_LIBRARY")}/Homebrew/test/support/lib"))

require_relative "support/extend/cachable"

require_relative "../global"

require "debug" if ENV["DRXHUB_DEBUG"]
...
RSpec.configure do |config|
  ...
  config.before(:each, :needs_network) do
    skip "Requires network connection." unless ENV["DRXHUB_TEST_ONLINE"]
  end

  config.before(:each, :needs_homebrew_core) do
    core_tap_path = "#{ENV.fetch("DRXHUB_LIBRARY")}/Taps/homebrew/homebrew-core"
  ...
  config.before(:each, :needs_svn) do
    svn_shim = DRXHUB_SHIMS_PATH/"shared/svn"
  ...
  config.before(:each, :needs_homebrew_curl) do
    ENV["DRXHUB_CURL"] = DRXHUB_BREWED_CURL_PATH
  ...
  config.around do |example|
    DinrusHub.raise_deprecation_exceptions = true
    ...
    @__homebrew_failed = DinrusHub.failed?
    ...
    begin
      if example.metadata.keys.exclude?(:focus) && !ENV.key?("DRXHUB_VERBOSE_TESTS")
      ...
      FileUtils.rm_rf [
        ...
        *Keg.must_exist_subdirectories,
        ...
        DRXHUB_PREFIX/"Frameworks",
        DRXHUB_LIBRARY/"Taps/homebrew/homebrew-cask",
        DRXHUB_LIBRARY/"Taps/homebrew/homebrew-bar",
        DRXHUB_LIBRARY/"Taps/homebrew/homebrew-bundle",
        DRXHUB_LIBRARY/"Taps/homebrew/homebrew-foo",
        DRXHUB_LIBRARY/"Taps/homebrew/homebrew-services",
        DRXHUB_LIBRARY/"Taps/homebrew/homebrew-shallow",
        DRXHUB_LIBRARY/"PinnedTaps",
        DRXHUB_REPOSITORY/".git",
      ...
      DinrusHub.failed = @__homebrew_failed
    end
  end
end
