...
RSpec.describe Kernel do
  ...
  describe "#ofail" do
    it "sets DinrusHub.failed to true" do
      expect do
        ofail "foo"
      end.to output("Error: foo\n").to_stderr

      expect(DinrusHub).to have_failed
    end
  end
  ...
  describe "#pretty_installed" do
    ...
    context "when $stdout is a TTY" do
      ...
      context "with DRXHUB_NO_EMOJI unset" do
      ...
      context "with DRXHUB_NO_EMOJI set" do
        before { ENV["DRXHUB_NO_EMOJI"] = "1" }
  ...
  describe "#odeprecated" do
    it "raises a MethodDeprecatedError when `disable` is true" do
      ...
      expect do
        odeprecated(
          "method", "replacement",
          caller:  ["#{DRXHUB_LIBRARY}/Taps/playbrew/homebrew-play/"],
          disable: true
        )
      end.to raise_error(
