...
RSpec.describe TestRunnerFormula do
  ...
  describe "#eval_all" do
    ...
    it "takes the value of `DRXHUB_EVAL_ALL` at instantiation time if not specified" do
      allow(DinrusHub::EnvConfig).to receive(:eval_all?).and_return(true)
      expect(described_class.new(testball).eval_all).to be(true)

      allow(DinrusHub::EnvConfig).to receive(:eval_all?).and_return(false)
      expect(described_class.new(testball).eval_all).to be(false)
    end
  end
  ...
  describe "#dependents" do
    let(:current_system) do
      current_arch = case DinrusHub::SimulateSystem.current_arch
      when :arm then :arm64
      when :intel then :x86_64
      end

      current_platform = case DinrusHub::SimulateSystem.current_os
      when :generic then :linux
      else DinrusHub::SimulateSystem.current_os
      end
