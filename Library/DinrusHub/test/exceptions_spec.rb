...
RSpec.describe "Exception" do
  describe MultipleVersionsInstalledError do
    subject(:error) do
      described_class.new <<~EOS
        ...
        Run `dhub uninstall --force foo` to remove all versions.
      EOS
    end
  ...
  describe NoSuchKegError do
    context "without a tap" do
      ...
      it(:to_s) { expect(error.to_s).to eq("No such keg: #{DRXHUB_CELLAR}/foo") }
    end
    ...
    context "with a tap" do
      ...
      it(:to_s) { expect(error.to_s).to eq("No such keg: #{DRXHUB_CELLAR}/foo from tap u/r") }
    end
  end
  ...
  describe TapFormulaOrCaskUnavailableError do
    ...
    it(:to_s) { expect(error.to_s).to match(%r{Please tap it and then try again: dhub tap u/r}) }
  end
  ...
  describe FormulaConflictError do
    ...
    it(:to_s) { expect(error.to_s).to match(/Please `dhub unlink bar` before continuing\./) }
  end
  ...
  describe BottleFormulaUnavailableError do
    subject(:error) { described_class.new("/foo.bottle.tar.gz", "foo/1.0/.dhub/foo.rb") }
