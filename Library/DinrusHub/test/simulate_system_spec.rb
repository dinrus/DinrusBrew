...
RSpec.describe DinrusHub::SimulateSystem do
  after do
  ...
  describe "::simulating_or_running_on_macos?" do
    ...
    it "returns true on Linux with DRXHUB_SIMULATE_MACOS_ON_LINUX", :needs_linux do
      described_class.clear
      ENV["DRXHUB_SIMULATE_MACOS_ON_LINUX"] = "1"
      expect(described_class.simulating_or_running_on_macos?).to be true
    end
  end
  ...
  describe "::simulating_or_running_on_linux?" do
    ...
    it "returns false on Linux with DRXHUB_SIMULATE_MACOS_ON_LINUX", :needs_linux do
      described_class.clear
      ENV["DRXHUB_SIMULATE_MACOS_ON_LINUX"] = "1"
      expect(described_class.simulating_or_running_on_linux?).to be false
    end
  ...
  describe "::current_os" do
    ...
    it "returns the current macOS version on macOS with DRXHUB_SIMULATE_MACOS_ON_LINUX", :needs_macos do
      described_class.clear
      ENV["DRXHUB_SIMULATE_MACOS_ON_LINUX"] = "1"
      expect(described_class.current_os).to eq MacOS.version.to_sym
    end

    it "returns `:macos` on Linux with DRXHUB_SIMULATE_MACOS_ON_LINUX", :needs_linux do
      described_class.clear
      ENV["DRXHUB_SIMULATE_MACOS_ON_LINUX"] = "1"
      expect(described_class.current_os).to eq :macos
    end
