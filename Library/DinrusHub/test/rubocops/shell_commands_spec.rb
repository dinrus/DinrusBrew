...
RSpec.describe RuboCop::Cop::DinrusHub::ShellCommands do
    ...
    it "reports and corrects an offense when `system` arguments should be separated" do
      expect_offense(<<~RUBY)
        class Foo < Formula
          def install
            system "foo bar"
                   ^^^^^^^^^ DinrusHub/ShellCommands: Separate `system` commands into `"foo", "bar"`
          end
        end
      RUBY
    ...
    it "reports and corrects an offense when `system` arguments involving interpolation should be separated" do
      expect_offense(<<~RUBY)
        class Foo < Formula
          def install
            system "\#{bin}/foo bar"
                   ^^^^^^^^^^^^^^^^ DinrusHub/ShellCommands: Separate `system` commands into `"\#{bin}/foo", "bar"`
          end
        end
      RUBY
        ...
        class Foo < Formula
          def install
            Utils.popen_read("foo bar")
                             ^^^^^^^^^ DinrusHub/ShellCommands: Separate `Utils.popen_read` commands into `"foo", "bar"`
          end
        end
      RUBY
        ...
        class Foo < Formula
          def install
            Utils.safe_popen_read("foo bar")
                                  ^^^^^^^^^ DinrusHub/ShellCommands: Separate `Utils.safe_popen_read` commands into `"foo", "bar"`
          end
        end
      RUBY
        ...
        class Foo < Formula
          def install
            Utils.popen_write("foo bar")
                              ^^^^^^^^^ DinrusHub/ShellCommands: Separate `Utils.popen_write` commands into `"foo", "bar"`
          end
        end
      RUBY
        ...
        class Foo < Formula
          def install
            Utils.safe_popen_write("foo bar")
                                   ^^^^^^^^^ DinrusHub/ShellCommands: Separate `Utils.safe_popen_write` commands into `"foo", "bar"`
          end
        end
      RUBY
        ...
        class Foo < Formula
          def install
            Utils.popen_read("\#{bin}/foo bar")
                             ^^^^^^^^^^^^^^^^ DinrusHub/ShellCommands: Separate `Utils.popen_read` commands into `"\#{bin}/foo", "bar"`
          end
        end
      RUBY
