    ...
    it "reports an offense when `skip_clean` is used" do
      expect_offense(<<~RUBY)
        class Foo < Formula
          ...
          url 'https://brew.sh/foo-1.0.tgz'
          skip_clean :all
          ^^^^^^^^^^^^^^^ FormulaAudit/Miscellaneous: `skip_clean :all` is deprecated; dhub no longer strips symbols. Pass explicit paths to prevent DinrusHub from removing empty folders.
        end
      RUBY
    end
