...
RSpec.shared_context "custom internal commands" do # rubocop:disable RSpec/ContextWording
  let(:cmds) do
    [
      # internal commands
      Commands::DRXHUB_CMD_PATH/"rbcmd.rb",
      Commands::DRXHUB_CMD_PATH/"shcmd.sh",

      # internal developer-commands
      Commands::DRXHUB_DEV_CMD_PATH/"rbdevcmd.rb",
      Commands::DRXHUB_DEV_CMD_PATH/"shdevcmd.sh",
    ]
  end
...
RSpec.describe Commands do
  ...
  describe "::path" do
    specify "returns the path for an internal command" do
      expect(described_class.path("rbcmd")).to eq(DRXHUB_LIBRARY_PATH/"cmd/rbcmd.rb")
      expect(described_class.path("shcmd")).to eq(DRXHUB_LIBRARY_PATH/"cmd/shcmd.sh")
      expect(described_class.path("idontexist1234")).to be_nil
    end

    specify "returns the path for an internal developer-command" do
      expect(described_class.path("rbdevcmd")).to eq(DRXHUB_LIBRARY_PATH/"dev-cmd/rbdevcmd.rb")
      expect(described_class.path("shdevcmd")).to eq(DRXHUB_LIBRARY_PATH/"dev-cmd/shdevcmd.sh")
    end
  end
end
