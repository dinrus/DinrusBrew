...
RSpec.describe DinrusHub::Search do
  ...
  describe "#search_descriptions" do
    let(:args) { DinrusHub::Cmd::Desc.new(["min_arg_placeholder"]).args }
    ...
    context "with api" do
      ...
      before do
        allow(DinrusHub::API::Formula).to receive(:all_formulae).and_return(api_formulae)
        allow(DinrusHub::API::Cask).to receive(:all_casks).and_return(api_casks)
      end
