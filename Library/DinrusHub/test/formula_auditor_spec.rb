...
RSpec.describe DinrusHub::FormulaAuditor do
  include FileUtils
  ...
  let(:origin_tap_path) { DRXHUB_TAP_DIRECTORY/"homebrew/homebrew-foo" }
  let(:origin_formula_path) { origin_tap_path/formula_subpath }
  let(:tap_path) { DRXHUB_TAP_DIRECTORY/"homebrew/homebrew-bar" }
    ...
    specify "Invalid command" do
      fa = formula_auditor "foo", <<~RUBY
        class Foo < Formula
          ...
          service do
            run [DRXHUB_PREFIX/"bin/something"]
          end
        end
      RUBY
  ...
  describe "#audit_github_repository" do
    specify "#audit_github_repository when DRXHUB_NO_GITHUB_API is set" do
      ENV["DRXHUB_NO_GITHUB_API"] = "1"
  ...
  describe "#audit_github_repository_archived" do
    specify "#audit_github_repository_archived when DRXHUB_NO_GITHUB_API is set" do
      fa = formula_auditor "foo", <<~RUBY, strict: true, online: true
        class Foo < Formula
          homepage "https://github.com/example/example"
          url "https://brew.sh/foo-1.0.tgz"
    ...
    describe "checksums" do
      ...
      describe "can be removed when switching schemes" do
        before do
          formula_gsub_origin_commit(
            'url "https://brew.sh/foo-1.0.tar.gz"',
            'url "https://foo.com/dhub/bar.git", tag: "1.0", revision: "f5e00e485e7aa4c5baa20355b27e3b84a6912790"',
          )
