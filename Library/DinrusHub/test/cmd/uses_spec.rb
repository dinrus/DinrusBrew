...
RSpec.describe DinrusHub::Cmd::Uses do
  include FileUtils
  ...
  it "prints the Formulae a given Formula is used by", :integration_test do
    ...
    %w[foo installed].each do |formula_name|
      keg_dir = DRXHUB_CELLAR/formula_name/"1.0"
      keg_dir.mkpath
      touch keg_dir/AbstractTab::FILENAME
    end

    expect { dhub "uses", "foo", "--eval-all", "--include-optional", "--missing", "--recursive" }
      .to output(/^(bar\noptional|optional\nbar)$/).to_stdout
  ...
  it "handles unavailable formula", :integration_test do
    ...
    expect_any_instance_of(DinrusHub::CLI::NamedArgs)
