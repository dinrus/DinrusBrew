...
RSpec.describe DinrusHub::Service do
  ...
  describe "#std_service_path_env" do
    it "returns valid std_service_path_env" do
      ...
      expect(path).to eq("#{DRXHUB_PREFIX}/bin:#{DRXHUB_PREFIX}/sbin:/usr/bin:/bin:/usr/sbin:/sbin")
    end
  end
  ...
  describe "#manual_command" do
    it "returns valid manual_command" do
      f = stub_formula do
        service do
          run "#{DRXHUB_PREFIX}/bin/beanstalkd"
          run_type :immediate
      ...
      expect(path).to eq("ETC_DIR=\"#{DRXHUB_PREFIX}/etc/beanstalkd\" #{DRXHUB_PREFIX}/bin/beanstalkd")
    end
    ...
    it "returns valid manual_command without variables" do
      ...
      path = f.service.manual_command
      expect(path).to eq("#{DRXHUB_PREFIX}/opt/formula_name/bin/beanstalkd")
    end
  end
  ...
  describe "#to_plist" do
    it "returns valid plist" do
      ...
      plist_expect = <<~EOS
        ...
        \t<dict>
        \t\t<key>ETC_DIR</key>
        \t\t<string>#{DRXHUB_PREFIX}/etc/beanstalkd</string>
        \t\t<key>FOO</key>
        \t\t<string>BAR</string>
        \t\t<key>PATH</key>
        \t\t<string>#{DRXHUB_PREFIX}/bin:#{DRXHUB_PREFIX}/sbin:/usr/bin:/bin:/usr/sbin:/sbin</string>
        \t</dict>
        \t<key>KeepAlive</key>
        \t<true/>
        \t<key>Label</key>
        ...
        \t<array>
        \t\t<string>#{DRXHUB_PREFIX}/opt/formula_name/bin/beanstalkd</string>
        \t\t<string>test</string>
        \t</array>
        \t<key>RootDirectory</key>
        \t<string>#{DRXHUB_PREFIX}/var</string>
        \t<key>RunAtLoad</key>
        \t<true/>
        \t<key>StandardErrorPath</key>
        \t<string>#{DRXHUB_PREFIX}/var/log/beanstalkd.error.log</string>
        \t<key>StandardInPath</key>
        \t<string>#{DRXHUB_PREFIX}/var/in/beanstalkd</string>
        \t<key>StandardOutPath</key>
        \t<string>#{DRXHUB_PREFIX}/var/log/beanstalkd.log</string>
        \t<key>TimeOut</key>
        \t<integer>30</integer>
    ...
    it "returns valid keepalive-path plist" do
      ...
      plist_expect = <<~EOS
        ...
        \t<dict>
        \t\t<key>PathState</key>
        \t\t<string>#{DRXHUB_PREFIX}/opt/formula_name/share/formula_name/test-path</string>
        \t</dict>
        \t<key>Label</key>
        \t<string>homebrew.mxcl.formula_name</string>
        \t<key>LimitLoadToSessionType</key>
    ...
    it "expands paths" do
      ...
      plist_expect = <<~EOS
        ...
        \t<key>ProgramArguments</key>
        \t<array>
        \t\t<string>#{DRXHUB_PREFIX}/opt/formula_name/sbin/sleepwatcher</string>
        \t\t<string>-V</string>
        \t\t<string>-s</string>
  ...
  describe "#to_systemd_unit" do
    it "returns valid unit" do
      ...
      std_path = "#{DRXHUB_PREFIX}/bin:#{DRXHUB_PREFIX}/sbin:/usr/bin:/bin:/usr/sbin:/sbin"
      unit_expect = <<~SYSTEMD
        [Unit]
        Description=DinrusHub generated unit for formula_name
        ...
        [Service]
        Type=simple
        ExecStart="#{DRXHUB_PREFIX}/opt/#{name}/bin/beanstalkd" "test"
        Restart=always
        RestartSec=30
        WorkingDirectory=#{DRXHUB_PREFIX}/var
        RootDirectory=#{DRXHUB_PREFIX}/var
        StandardInput=file:#{DRXHUB_PREFIX}/var/in/beanstalkd
        StandardOutput=append:#{DRXHUB_PREFIX}/var/log/beanstalkd.log
        StandardError=append:#{DRXHUB_PREFIX}/var/log/beanstalkd.error.log
    ...
    it "returns valid partial oneshot unit" do
      ...
      unit_expect = <<~SYSTEMD
        ...
        [Service]
        Type=oneshot
        ExecStart="#{DRXHUB_PREFIX}/opt/#{name}/bin/beanstalkd"
      SYSTEMD
      expect(unit).to eq(unit_expect)
    end
  ...
  describe "#to_systemd_timer" do
    it "returns valid timer" do
      ...
      unit_expect = <<~SYSTEMD
        [Unit]
        Description=DinrusHub generated timer for formula_name
  ...
  describe "#command" do
    it "returns @run data" do
      ...
      expect(command).to eq(["#{DRXHUB_PREFIX}/opt/#{name}/bin/beanstalkd", "test"])
    end
    ...
    it "returns appropriate @run data on Linux", :needs_linux do
      ...
      command = f.service.command
      expect(command).to eq(["#{DRXHUB_PREFIX}/opt/#{name}/bin/beanstalkd", "test", "linux"])
    end
    ...
    it "returns appropriate @run data on macOS", :needs_macos do
      ...
      command = f.service.command
      expect(command).to eq(["#{DRXHUB_PREFIX}/opt/#{name}/bin/beanstalkd", "test", "macos"])
    end
  end
    ...
    let(:serialized_hash) do
      {
        environment_variables: {
          PATH: "$DRXHUB_PREFIX/bin:$DRXHUB_PREFIX/sbin:/usr/bin:/bin:/usr/sbin:/sbin",
        },
        run:                   [Pathname("$DRXHUB_PREFIX/opt/formula_name/bin/beanstalkd"), "test"],
        run_type:              :immediate,
        working_dir:           "/$HOME",
        cron:                  "0 0 * * 0",
    ...
    let(:serialized_hash) do
      {
        ...
        "run"                   => ["$DRXHUB_PREFIX/opt/formula_name/bin/beanstalkd", "test"],
        "run_type"              => "immediate",
        "working_dir"           => DRXHUB_HOME_PLACEHOLDER,
        "keep_alive"            => { "successful_exit" => false },
      }
    end
    ...
    describe "run command" do
      it "handles String argument correctly" do
        expect(described_class.from_hash({
          "run" => "$DRXHUB_PREFIX/opt/formula_name/bin/beanstalkd",
        })).to eq({
          run: "#{DRXHUB_PREFIX}/opt/formula_name/bin/beanstalkd",
        })
      end
      ...
      it "handles Array argument correctly" do
        expect(described_class.from_hash({
          "run" => ["$DRXHUB_PREFIX/opt/formula_name/bin/beanstalkd", "--option"],
        })).to eq({
          run: ["#{DRXHUB_PREFIX}/opt/formula_name/bin/beanstalkd", "--option"],
        })
      end
      ...
      it "handles Hash argument correctly" do
        expect(described_class.from_hash({
          "run" => {
            "linux" => "$DRXHUB_PREFIX/opt/formula_name/bin/beanstalkd",
            "macos" => ["$DRXHUB_PREFIX/opt/formula_name/bin/beanstalkd", "--option"],
          },
        })).to eq({
          run: {
            linux: "#{DRXHUB_PREFIX}/opt/formula_name/bin/beanstalkd",
