...
RSpec.describe Keg do
  ...
  def setup_test_keg(name, version)
    path = DRXHUB_CELLAR/name/version
    (path/"bin").mkpath
  ...
  let(:dst) { DRXHUB_PREFIX/"bin"/"helloworld" }
  ...
  before do
    ...
    (DRXHUB_PREFIX/"lib").mkpath
  end

  after do
    kegs.each(&:unlink)
    rmtree DRXHUB_PREFIX/"lib"
  end
  ...
  specify "#oldname_opt_records" do
    ...
    oldname_opt_record = DRXHUB_PREFIX/"opt/oldfoo"
    oldname_opt_record.make_relative_symlink(DRXHUB_CELLAR/"foo/1.0")
    expect(keg.oldname_opt_records).to eq([oldname_opt_record])
  end

  specify "#remove_oldname_opt_records" do
    oldname_opt_record = DRXHUB_PREFIX/"opt/oldfoo"
    oldname_opt_record.make_relative_symlink(DRXHUB_CELLAR/"foo/2.0")
  ...
  describe "#link" do
    it "links a Keg" do
      ...
      (DRXHUB_PREFIX/"bin").children.each do |c|
    ...
    context "with dry run set to true" do
      ...
      it "only prints what would be done" do
        expect do
          ...
          #{DRXHUB_PREFIX}/bin/goodbye_cruel_world
          #{DRXHUB_PREFIX}/bin/helloworld
          #{DRXHUB_PREFIX}/bin/hiworld
        EOF
    ...
    specify "pkgconfig directory is created" do
      link = DRXHUB_PREFIX/"lib"/"pkgconfig"
  ...
  describe "#unlink" do
    ...
    it "prunes empty top-level directories" do
      mkpath DRXHUB_PREFIX/"lib/foo/bar"
      mkpath keg/"lib/foo/bar"
      touch keg/"lib/foo/bar/file1"

      keg.unlink

      expect(DRXHUB_PREFIX/"lib/foo").not_to be_a_directory
    end

    it "ignores .DS_Store when pruning empty directories" do
      mkpath DRXHUB_PREFIX/"lib/foo/bar"
      touch DRXHUB_PREFIX/"lib/foo/.DS_Store"
      mkpath keg/"lib/foo/bar"
      touch keg/"lib/foo/bar/file1"
      ...
      expect(DRXHUB_PREFIX/"lib/foo").not_to be_a_directory
      expect(DRXHUB_PREFIX/"lib/foo/.DS_Store").not_to exist
    end
    ...
    it "doesn't remove links to symlinks" do
      a = DRXHUB_CELLAR/"a"/"1.0"
      b = DRXHUB_CELLAR/"b"/"1.0"
  ...
  describe "#optlink" do
    it "creates an opt link" do
      oldname_opt_record = DRXHUB_PREFIX/"opt/oldfoo"
      oldname_opt_record.make_relative_symlink(DRXHUB_CELLAR/"foo/1.0")
      keg_record = DRXHUB_CELLAR/"foo"/"2.0"
