...
RSpec.shared_context "integration test" do # rubocop:disable RSpec/ContextWording
  ...
  around do |example|
    ENV["DRXHUB_INTEGRATION_TEST"] = "1"
    (DRXHUB_PREFIX/"bin").mkpath
    FileUtils.touch DRXHUB_PREFIX/"bin/dhub"

    example.run
  ensure
    FileUtils.rm_rf DRXHUB_PREFIX/"bin"
    ENV.delete("DRXHUB_INTEGRATION_TEST")
  end
  ...
  # Runs a `dhub` command with the test configuration
  # and with coverage reporting enabled.
  def dhub(*args)
    env = args.last.is_a?(Hash) ? args.pop : {}

    # Avoid warnings when DRXHUB_PREFIX/bin is not in PATH.
    # Also include our extra commands directory.
    path = [
      env["PATH"],
      (DRXHUB_LIBRARY_PATH/"test/support/helper/cmd").realpath.to_s,
      (DRXHUB_PREFIX/"bin").realpath.to_s,
      ENV.fetch("PATH"),
    ...
    env.merge!(
      "PATH"                        => path,
      "DRXHUB_PATH"               => path,
      "DRXHUB_BREW_FILE"          => DRXHUB_PREFIX/"bin/dhub",
      "DRXHUB_INTEGRATION_TEST"   => command_id,
      "DRXHUB_TEST_TMPDIR"        => TEST_TMPDIR,
      "DRXHUB_DEV_CMD_RUN"        => "true",
      "DRXHUB_USE_RUBY_FROM_PATH" => ENV.fetch("DRXHUB_USE_RUBY_FROM_PATH", nil),
      "GEM_HOME"                    => nil,
    )

    @ruby_args ||= begin
      ruby_args = DRXHUB_RUBY_EXEC_ARGS.dup
      if ENV["DRXHUB_TESTS_COVERAGE"]
      ...
      ruby_args << "-r#{DRXHUB_LIBRARY_PATH}/test/support/helper/integration_mocks"
      ruby_args << "-e" << "$0 = ARGV.shift; load($0)"
      ruby_args << (DRXHUB_LIBRARY_PATH/"dhub.rb").resolved_path.to_s
    end
  ...
  def brew_sh(*args)
    ...
    Bundler.with_unbundled_env do
      stdout, stderr, status = Open3.capture3(env, "#{ENV.fetch("DRXHUB_PREFIX")}/bin/dhub", *args)
      $stdout.print stdout
      $stderr.print stderr
      status
    end
  ...
  def setup_test_tap
    path = DRXHUB_TAP_DIRECTORY/"homebrew/homebrew-foo"
    path.mkpath
    path.cd do
      system "git", "init"
  ...
  def setup_remote_tap(name)
    Tap.fetch(name).tap do |tap|
      ...
      # Check to see if the original DinrusHub process has taps we can use.
      system_tap_path = Pathname("#{ENV.fetch("DRXHUB_LIBRARY")}/Taps/#{full_name}")
