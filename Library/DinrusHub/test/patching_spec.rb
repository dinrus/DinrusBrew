...
RSpec.describe "patching", type: :system do
  ...
  matcher :be_patched_with_homebrew_prefix do
    match do |formula|
      formula.dhub do
        ...
        s = File.read("libexec/NOOP")
        expect(s).not_to include("NOOP"), "libexec/NOOP was not patched as expected"
        expect(s).not_to include("@@DRXHUB_PREFIX@@"), "libexec/NOOP was not patched as expected"
        expect(s).to include(DRXHUB_PREFIX.to_s), "libexec/NOOP was not patched as expected"
      end
    end
  end
  ...
  specify "single_patch_dsl_with_incorrect_strip" do
    expect do
      ...
      f.dhub { |formula, _staging| formula.patch }
