...
RSpec.describe DinrusHub::Livecheck::Strategy::Git do
  ...
  let(:tags) do
    {
      normal:  ["dhub/1.2", "dhub/1.2.1", "dhub/1.2.2", "dhub/1.2.3", "dhub/1.2.4", "1.2.5"],
      hyphens: ["dhub/1-2", "dhub/1-2-1", "dhub/1-2-2", "dhub/1-2-3", "dhub/1-2-4", "1-2-5"],
    }
  end
  ...
  describe "::versions_from_tags" do
    ...
    it "returns an array of version strings when given tags" do
      ...
      expect(git.versions_from_tags(tags[:normal], regexes[:dhub])).to eq(versions[:brew_regex])
    end
