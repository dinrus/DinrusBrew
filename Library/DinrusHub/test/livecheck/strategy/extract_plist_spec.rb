...
RSpec.describe DinrusHub::Livecheck::Strategy::ExtractPlist do
  ...
  let(:items) do
    {
      "first"  => extract_plist::Item.new(
        bundle_version: DinrusHub::BundleVersion.new(nil, "1.2"),
      ),
      "second" => extract_plist::Item.new(
        bundle_version: DinrusHub::BundleVersion.new(nil, "1.2.3"),
      ),
    }
  end
  ...
  let(:multipart_items) do
    {
      "first"  => extract_plist::Item.new(
        bundle_version: DinrusHub::BundleVersion.new(nil, "1.2.3-45"),
      ),
      "second" => extract_plist::Item.new(
        bundle_version: DinrusHub::BundleVersion.new(nil, "1.2.3-45-abcdef"),
      ),
    }
  end
  ...
  describe "::find_versions" do
    ...
    it "uses the provided livecheck url", :needs_macos do
      ...
      expect(DinrusHub::UnversionedCaskChecker).to receive(:new).with(cask).and_call_original
