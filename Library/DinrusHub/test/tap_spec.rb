...
RSpec.describe Tap do
  ...
  subject(:homebrew_foo_tap) { described_class.fetch("DinrusHub", "foo") }

  let(:path) { DRXHUB_TAP_DIRECTORY/"homebrew/homebrew-foo" }
  ...
  specify "::fetch" do
    expect(described_class.fetch("DinrusHub", "core")).to be_a(CoreTap)
    expect(described_class.fetch("DinrusHub", "homebrew")).to be_a(CoreTap)
    tap = described_class.fetch("DinrusHub", "foo")
  ...
  describe "::from_path" do
    let(:tap) { described_class.fetch("DinrusHub", "core") }
    let(:path) { tap.path }
  ...
  describe "::allowed_taps" do
    before { allow(DinrusHub::EnvConfig).to receive(:allowed_taps).and_return("homebrew/allowed") }
  ...
  describe "::forbidden_taps" do
    before { allow(DinrusHub::EnvConfig).to receive(:forbidden_taps).and_return("homebrew/forbidden") }
  ...
  specify "attributes" do
    expect(homebrew_foo_tap.user).to eq("DinrusHub")
  ...
  specify "#issues_url" do
    ...
    path = DRXHUB_TAP_DIRECTORY/"someone/homebrew-foo"
    path.mkpath
    cd path do
      system "git", "init"
    ...
    (DRXHUB_TAP_DIRECTORY/"someone/homebrew-no-git").mkpath
  ...
  describe "#remote" do
    it "returns the remote URL", :needs_network do
      ...
      services_tap = described_class.fetch("DinrusHub", "services")
  ...
  describe "#remote_repo" do
    it "returns the remote https repository" do
      ...
      expect(homebrew_foo_tap.remote_repository).to eq("DinrusHub/homebrew-foo")
      ...
      expect(services_tap.remote_repository).to eq("DinrusHub/homebrew-bar")
    end
    ...
    it "returns the remote ssh repository" do
      ...
      services_tap.path.cd do
        system "git", "init"
        system "git", "remote", "add", "origin", "git@github.com:DinrusHub/homebrew-bar"
      end
      expect(services_tap.remote_repository).to eq("DinrusHub/homebrew-bar")
    end
  ...
  describe "#custom_remote?" do
    subject(:tap) { described_class.fetch("DinrusHub", "services") }
    ...
    context "when using a non-default remote" do
      let(:remote) { "git@github.com:DinrusHub/homebrew-services" }
  ...
  describe "#install" do
    it "raises an error when the Tap is already tapped" do
      setup_git_repo
      already_tapped_tap = described_class.fetch("DinrusHub", "foo")
    ...
    it "raises an error when the remote for DinrusHub/core doesn't match DRXHUB_CORE_GIT_REMOTE" do
      core_tap = described_class.fetch("DinrusHub", "core")
      wrong_remote = "#{DinrusHub::EnvConfig.core_git_remote}-oops"
      expect do
        core_tap.install clone_target: wrong_remote
      end.to raise_error(TapCoreRemoteMismatchError)
    end

    it "raises an error when run `dhub tap --custom-remote` without a custom remote (already installed)" do
      setup_git_repo
      already_tapped_tap = described_class.fetch("DinrusHub", "foo")
      expect(already_tapped_tap).to be_installed
    ...
    it "raises an error when run `dhub tap --custom-remote` without a custom remote (not installed)" do
      not_tapped_tap = described_class.fetch("DinrusHub", "bar")
    ...
    specify "Git error" do
      ...
      expect(DRXHUB_TAP_DIRECTORY/"user").not_to exist
    end
  end

  describe "#uninstall" do
    it "raises an error if the Tap is not available" do
      tap = described_class.fetch("DinrusHub", "bar")
  ...
  specify "#install and #uninstall" do
    ...
    expect(DRXHUB_PREFIX/"share/man/man1/brew-tap-cmd.1").to be_a_file
    expect(DRXHUB_PREFIX/"etc/bash_completion.d/brew-tap-cmd").to be_a_file
    expect(DRXHUB_PREFIX/"share/zsh/site-functions/_brew-tap-cmd").to be_a_file
    expect(DRXHUB_PREFIX/"share/fish/vendor_completions.d/brew-tap-cmd.fish").to be_a_file
    tap.uninstall

    expect(tap).not_to be_installed
    expect(DRXHUB_PREFIX/"share/man/man1/brew-tap-cmd.1").not_to exist
    expect(DRXHUB_PREFIX/"share/man/man1").not_to exist
    expect(DRXHUB_PREFIX/"etc/bash_completion.d/brew-tap-cmd").not_to exist
    expect(DRXHUB_PREFIX/"share/zsh/site-functions/_brew-tap-cmd").not_to exist
    expect(DRXHUB_PREFIX/"share/fish/vendor_completions.d/brew-tap-cmd.fish").not_to exist
  ensure
    FileUtils.rm_r(DRXHUB_PREFIX/"etc") if (DRXHUB_PREFIX/"etc").exist?
    FileUtils.rm_r(DRXHUB_PREFIX/"share") if (DRXHUB_PREFIX/"share").exist?
  end
  ...
  specify "#link_completions_and_manpages when completions are enabled for non-official tap" do
    ...
    tap.install clone_target: homebrew_foo_tap.path/".git"
    (DRXHUB_PREFIX/"share/man/man1/brew-tap-cmd.1").delete
    (DRXHUB_PREFIX/"etc/bash_completion.d/brew-tap-cmd").delete
    (DRXHUB_PREFIX/"share/zsh/site-functions/_brew-tap-cmd").delete
    (DRXHUB_PREFIX/"share/fish/vendor_completions.d/brew-tap-cmd.fish").delete
  ...
  specify "#link_completions_and_manpages when completions are disabled for non-official tap" do
    ...
    tap.link_completions_and_manpages
    expect(DRXHUB_PREFIX/"share/man/man1/brew-tap-cmd.1").to be_a_file
    expect(DRXHUB_PREFIX/"etc/bash_completion.d/brew-tap-cmd").not_to be_a_file
    expect(DRXHUB_PREFIX/"share/zsh/site-functions/_brew-tap-cmd").not_to be_a_file
    expect(DRXHUB_PREFIX/"share/fish/vendor_completions.d/brew-tap-cmd.fish").not_to be_a_file
    tap.uninstall
  ensure
    FileUtils.rm_r(DRXHUB_PREFIX/"etc") if (DRXHUB_PREFIX/"etc").exist?
    FileUtils.rm_r(DRXHUB_PREFIX/"share") if (DRXHUB_PREFIX/"share").exist?
  ...
  specify "#link_completions_and_manpages when completions are enabled for official tap" do
    ...
    setup_git_repo
    setup_completion link: false
    tap = described_class.fetch("DinrusHub", "baz")
    tap.install clone_target: homebrew_foo_tap.path/".git"
    (DRXHUB_PREFIX/"share/man/man1/brew-tap-cmd.1").delete
    (DRXHUB_PREFIX/"etc/bash_completion.d/brew-tap-cmd").delete
    (DRXHUB_PREFIX/"share/zsh/site-functions/_brew-tap-cmd").delete
  ...
  describe ".each" do
    ...
    context "when the core tap is not installed" do
      ...
      it "includes the core tap with the api" do
        ENV.delete("DRXHUB_NO_INSTALL_FROM_API")
  ...
  describe CoreTap do
    ...
    specify "attributes" do
      expect(core_tap.user).to eq("DinrusHub")
    ...
    specify "files" do
      path = DRXHUB_TAP_DIRECTORY/"homebrew/homebrew-core"
  ...
  describe "#repository_var_suffix" do
    it "converts the repo directory to an environment variable suffix" do
      expect(CoreTap.instance.repository_var_suffix).to eq "_DRXHUB_DRXHUB_CORE"
    end

    it "converts non-alphanumeric characters to underscores" do
      expect(described_class.fetch("my",
                                   "tap-with-dashes").repository_var_suffix).to eq "_MY_DRXHUB_TAP_WITH_DASHES"
      expect(described_class.fetch("my",
                                   "tap-with-@-symbol").repository_var_suffix).to eq "_MY_DRXHUB_TAP_WITH___SYMBOL"
    end
  end
