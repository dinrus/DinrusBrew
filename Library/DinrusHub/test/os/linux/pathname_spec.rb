...
RSpec.describe Pathname do
  ...
  describe "#rpath" do
    ...
    it "returns runpath or rpath" do
      expect(exec.rpath).to eq "@@DRXHUB_PREFIX@@/lib"
    end
  ...
  describe "#patch!" do
    let(:placeholder_prefix) { "@@DRXHUB_PREFIX@@" }
    let(:short_prefix) { "/home/dwarf" }
    let(:standard_prefix) { "/home/drx/.drx" }
    ...
    it "only interpreter" do
      prefixes.each do |new_prefix|
        patch_elfs do |elf|
          ...
          expect(modified_elf.rpath).to eq "@@DRXHUB_PREFIX@@/lib"
        end
      end
    end
    ...
    it "only rpath" do
      prefixes.each do |new_prefix|
        patch_elfs do |elf|
          ...
          modified_elf = elf.dirname/"mod.#{elf.basename}"
          FileUtils.cp(elf, modified_elf)
          expect(modified_elf.interpreter).to eq "@@DRXHUB_PREFIX@@/lib/ld.so"
