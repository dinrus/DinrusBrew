...
RSpec.describe DinrusHub::CLI::Parser do
  before { stub_const("Cmd", Class.new(DinrusHub::AbstractCommand)) }
  ...
  describe "test switch options" do
    ...
    before do
      allow(DinrusHub::EnvConfig).to receive(:pry?).and_return(true)
    end
    ...
    it "raises an exception and outputs help text when an invalid option is passed" do
      expect { parser.parse(["--random"]) }.to raise_error(OptionParser::InvalidOption, /--random/)
                                           .and output(/Usage: dhub/).to_stderr
    end
  ...
  describe "test constraints for flag options" do
    ...
    it "raises exception on depends_on constraint violation" do
      expect { parser.parse(["--flag2=flag2"]) }.to raise_error(DinrusHub::CLI::OptionConstraintError)
    end

    it "raises exception for conflict violation" do
      expect { parser.parse(["--flag1=flag1", "--flag3=flag3"]) }.to raise_error(DinrusHub::CLI::OptionConflictError)
    end
  ...
  describe "test invalid constraints" do
    ...
    it "raises exception due to invalid constraints" do
      expect { parser.parse([]) }.to raise_error(DinrusHub::CLI::InvalidConstraintError)
    end
  end
  ...
  describe "test constraints for switch options" do
    ...
    it "raises exception on depends_on constraint violation" do
      expect { parser.parse(["--switch-c"]) }.to raise_error(DinrusHub::CLI::OptionConstraintError)
    end

    it "raises exception for conflict violation" do
      expect { parser.parse(["-ab"]) }.to raise_error(DinrusHub::CLI::OptionConflictError)
    end
    ...
    it "prioritizes cli arguments over env vars when they conflict" do
      without_partial_double_verification do
        allow(DinrusHub::EnvConfig).to receive_messages(switch_a?: true, switch_b?: false)
      end
      args = parser.parse(["--switch-b"])
    ...
    it "raises an exception on constraint violation when both are env vars" do
      without_partial_double_verification do
        allow(DinrusHub::EnvConfig).to receive_messages(switch_a?: true, switch_b?: true)
      end
      expect { parser.parse([]) }.to raise_error(DinrusHub::CLI::OptionConflictError)
    end
  end
  ...
  describe "named_args" do
    ...
    it "doesn't accept fewer than the passed number of arguments" do
      expect { parser_number.parse([]) }.to raise_error(DinrusHub::CLI::NumberOfNamedArgumentsError)
    end

    it "doesn't accept more than the passed number of arguments" do
      expect { parser_number.parse(["foo", "bar"]) }.to raise_error(DinrusHub::CLI::NumberOfNamedArgumentsError)
    end
    ...
    it "doesn't accept any arguments with :none" do
      expect { parser_none.parse(["foo"]) }
        .to raise_error(DinrusHub::CLI::MaxNamedArgumentsError, /This command does not take named arguments/)
    end
    ...
    it "displays the correct error message with no arg types and min" do
      ...
      expect { parser.parse([]) }.to raise_error(
        DinrusHub::CLI::MinNamedArgumentsError, /This command requires at least 2 named arguments/
      )
    end
    ...
    it "displays the correct error message with no arg types and number" do
      ...
      expect { parser.parse([]) }.to raise_error(
        DinrusHub::CLI::NumberOfNamedArgumentsError, /This command requires exactly 2 named arguments/
      )
    end
    ...
    it "displays the correct error message with no arg types and max" do
      ...
      expect { parser.parse(%w[foo bar]) }.to raise_error(
        DinrusHub::CLI::MaxNamedArgumentsError, /This command does not take more than 1 named argument/
      )
    end
    ...
    it "displays the correct error message with an array of strings" do
      ...
      expect { parser.parse([]) }.to raise_error(
        DinrusHub::CLI::NumberOfNamedArgumentsError, /This command requires exactly 1 subcommand/
      )
    end
    ...
    it "displays the correct error message with an array of symbols" do
      ...
      expect { parser.parse([]) }.to raise_error(
        DinrusHub::CLI::MinNamedArgumentsError, /This command requires at least 1 formula or cask argument/
      )
    end
    ...
    it "displays the correct error message with an array of symbols and max" do
      ...
      expect { parser.parse(%w[foo bar]) }.to raise_error(
        DinrusHub::CLI::MaxNamedArgumentsError, /This command does not take more than 1 formula or cask argument/
      )
    end
  ...
  describe "--cask on linux", :needs_linux do
    context "without --formula switch" do
      ...
      it "succeeds for developer commands" do
        require "dev-cmd/cat"
        args = DinrusHub::DevCmd::Cat.new(["--cask", "cask_name"]).args
    ...
    context "with conflicting --formula switch" do
      ...
      it "throws an error when both defined" do
        expect { parser.parse(["--cask", "--formula"]) }
          .to raise_exception DinrusHub::CLI::OptionConflictError
      end
    end
  end
