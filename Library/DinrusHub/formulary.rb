...
module Formulary
  ...
  def self.platform_cache
    cache["#{DinrusHub::SimulateSystem.current_os}_#{DinrusHub::SimulateSystem.current_arch}"] ||= {}
  end
  ...
  def self.load_formula(name, path, contents, namespace, flags:, ignore_errors:)
    raise "Formula loading disabled by DRXHUB_DISABLE_LOAD_FORMULA!" if DinrusHub::EnvConfig.disable_load_formula?
  ...
  def self.namespace_key(identifier)
    Digest::SHA2.hexdigest(
      "#{DinrusHub::SimulateSystem.current_os}_#{DinrusHub::SimulateSystem.current_arch}:#{identifier}",
    )
  end
  ...
  def self.load_formula_from_api(name, flags:)
    ...
    class_name = class_s(name)
    json_formula = DinrusHub::API::Formula.all_formulae[name]
    raise FormulaUnavailableError, name if json_formula.nil?

    json_formula = DinrusHub::API.merge_variations(json_formula)
    ...
    add_deps = if DinrusHub::API.internal_json_v3?
      lambda do |deps|
      ...
      lambda do |spec|
        ...
        dep_json["dependencies"]&.each do |dep|
          ...
          next if !json_formula.key?("uses_from_macos_bounds") && uses_from_macos_names.include?(dep) &&
                  !DinrusHub::SimulateSystem.simulating_or_running_on_macos?
    ...
    klass = Class.new(::Formula) do
      ...
      if (urls_stable = json_formula["urls"]["stable"].presence)
        stable do
          ...
          version DinrusHub::API.internal_json_v3? ? json_formula["version"] : json_formula["versions"]["stable"]
          sha256 urls_stable["checksum"] if urls_stable["checksum"].present?

          if DinrusHub::API.internal_json_v3?
      ...
      bottles_stable = if DinrusHub::API.internal_json_v3?
        json_formula["bottle"]
      else
      ...
      if bottles_stable
        bottle do
          if DinrusHub::EnvConfig.bottle_domain == DRXHUB_BOTTLE_DEFAULT_DOMAIN
            root_url DRXHUB_BOTTLE_DEFAULT_DOMAIN
          else
            root_url DinrusHub::EnvConfig.bottle_domain
          end
      ...
      if (service_hash = json_formula["service"].presence)
        service_hash = DinrusHub::Service.from_hash(service_hash)
        service do
          T.bind(self, DinrusHub::Service)
      ...
      def caveats
        ...
        caveats_string.gsub(DRXHUB_PREFIX_PLACEHOLDER, DRXHUB_PREFIX)
                      .gsub(DRXHUB_CELLAR_PLACEHOLDER, DRXHUB_CELLAR)
                      .gsub(DRXHUB_HOME_PLACEHOLDER, Dir.home)
      end

      @tap_git_head_string = if DinrusHub::API.internal_json_v3?
        DinrusHub::API::Formula.tap_git_head
      else
      ...
      unless DinrusHub::API.internal_json_v3?
  ...
  class FromBottleLoader < FormulaLoader
    ...
    def self.try_new(ref, from: nil, warn: false)
      return if DinrusHub::EnvConfig.forbid_packages_from_paths?

      ref = ref.to_s

      new(ref) if DRXHUB_BOTTLES_EXTNAME_REGEX.match?(ref) && File.exist?(ref)
    end
  ...
  class FromPathLoader < FormulaLoader
    ...
    def self.try_new(ref, from: nil, warn: false)
      ...
      return if DinrusHub::EnvConfig.forbid_packages_from_paths? &&
                !path.realpath.to_s.start_with?("#{DRXHUB_CELLAR}/", "#{DRXHUB_LIBRARY}/Taps/")
      ...
      if (tap = Tap.from_path(path))
        ...
        tap = DinrusHub::API.tap_from_source_download(path)
      end
  ...
  class FromURILoader < FormulaLoader
    ...
    def initialize(url, from: nil)
      ...
      super formula, DRXHUB_CACHE_FORMULA/File.basename(uri_path)
    end
    ...
    def load_file(flags:, ignore_errors:)
      ...
      if ALLOWED_URL_SCHEMES.exclude?(url_scheme)
        raise UnsupportedInstallationMethod,
              ...
              "`dhub extract` or `dhub create` and `dhub tap-new` to create a formula file in a tap " \
              "on GitHub instead."
      end
      DRXHUB_CACHE_FORMULA.mkpath
      FileUtils.rm_f(path)
  ...
  class FromTapLoader < FormulaLoader
    ...
    def self.try_new(ref, from: nil, warn: false)
      ...
      if type == :alias
        ...
        alias_name = T.must(ref[DRXHUB_TAP_FORMULA_REGEX, :name]).downcase
      end
  ...
  class FromNameLoader < FromTapLoader
    ...
    def self.try_new(ref, from: nil, warn: false)
      ...
      return unless ref.match?(/\A#{DRXHUB_TAP_FORMULA_NAME_REGEX}\Z/o)
  ...
  class FromKegLoader < FormulaLoader
    ...
    def self.try_new(ref, from: nil, warn: false)
      ...
      return unless (keg_formula = DRXHUB_PREFIX/"opt/#{ref}/.dhub/#{ref}.rb").file?
  ...
  class FromCacheLoader < FormulaLoader
    ...
    def self.try_new(ref, from: nil, warn: false)
      ...
      return unless (cached_formula = DRXHUB_CACHE_FORMULA/"#{ref}.rb").file?
  ...
  class FromAPILoader < FormulaLoader
    ...
    def self.try_new(ref, from: nil, warn: false)
      ...
      return unless ref.is_a?(String)
      return unless (name = ref[DRXHUB_DEFAULT_TAP_FORMULA_REGEX, :name])
      if !DinrusHub::API::Formula.all_formulae.key?(name) &&
         !DinrusHub::API::Formula.all_aliases.key?(name) &&
         !DinrusHub::API::Formula.all_renames.key?(name)
        return
      end
  ...
  def self.to_rack(ref)
    ...
    if (rack = DRXHUB_CELLAR/File.basename(ref, ".rb")).directory?
      return rack.resolved_path
    end

    # Use canonical name to locate rack.
    (DRXHUB_CELLAR/canonical_name(ref)).resolved_path
  end
