    ...
    class UninstallCmd < AbstractCommand
      ...
      def run
        ...
        if ENV["DRXHUB_AUTOREMOVE"].present?
          opoo "DRXHUB_AUTOREMOVE is now a no-op as it is the default behaviour. " \
               "Set DRXHUB_NO_AUTOREMOVE=1 to disable it."
        end
        Cleanup.autoremove unless DinrusHub::EnvConfig.no_autoremove?
      end
    end
  end
end
