...
module DinrusHub
  module Cmd
    class Prefix < AbstractCommand
      ...
      cmd_args do
        description <<~EOS
          Display DinrusHub's install path. *Default:*

            - macOS ARM: `#{DRXHUB_MACOS_ARM_DEFAULT_PREFIX}`
            - macOS Intel: `#{DRXHUB_DEFAULT_PREFIX}`
            - Linux: `#{DRXHUB_LINUX_DEFAULT_PREFIX}`

          If <formula> is provided, display the location where <formula> is or would be installed.
        EOS
        switch "--unbrewed",
               description: "List files in DinrusHub's prefix not installed by DinrusHub."
        switch "--installed",
      ...
      sig { void }
      def list_unbrewed
        dirs  = DRXHUB_PREFIX.subdirs.map { |dir| dir.basename.to_s }
        dirs -= %w[Library Cellar Caskroom .git]

        # Exclude cache, logs and repository, if they are located under the prefix.
        [DRXHUB_CACHE, DRXHUB_LOGS, DRXHUB_REPOSITORY].each do |dir|
          dirs.delete dir.relative_path_from(DRXHUB_PREFIX).to_s
        end
        dirs.delete "etc"
        dirs.delete "var"
        ...
        cd(DRXHUB_PREFIX) { safe_system("find", *arguments) }
      end
    end
  end
end
