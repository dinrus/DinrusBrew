...
module DinrusHub
  module Cmd
    class Update < AbstractCommand
      ...
      cmd_args do
        description <<~EOS
          Fetch the newest version of DinrusHub and all formulae from GitHub using `git`(1) and perform any necessary migrations.
        EOS
        switch "--merge",
               description: "Use `git merge` to apply updates (rather than `git rebase`)."
        switch "--auto-update",
               description: "Run on auto-updates (e.g. before `dhub install`). Skips some slower steps."
        switch "-f", "--force",
