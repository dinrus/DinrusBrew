...
module DinrusHub
  module Cmd
    class RbenvSync < AbstractCommand
      cmd_args do
        description <<~EOS
          Create symlinks for DinrusHub's installed Ruby versions in `~/.rbenv/versions`.
      ...
      sig { override.void }
      def run
        rbenv_root = Pathname(ENV.fetch("DRXHUB_RBENV_ROOT", Pathname(Dir.home)/".rbenv"))
