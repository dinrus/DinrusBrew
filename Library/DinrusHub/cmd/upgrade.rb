    ...
    class UpgradeCmd < AbstractCommand
      cmd_args do
        description <<~EOS
          ...
          installed with, plus any appended dhub formula options. If <cask> or <formula> are specified,
          upgrade only the given <cask> or <formula> kegs (unless they are pinned; see `pin`, `unpin`).

          Unless `$DRXHUB_NO_INSTALLED_DEPENDENTS_CHECK` is set, `dhub upgrade` or `dhub reinstall` will be run for
          outdated dependents and dependents with broken linkage, respectively.

          Unless `$DRXHUB_NO_INSTALL_CLEANUP` is set, `dhub cleanup` will then be run for the
        ...
        [
          ...
          [:switch, "-i", "--interactive", {
            description: "Download and patch <formula>, then open a shell. This allows the user to " \
                         ...
                         "package into a DinrusHub package.",
          }],
          [:switch, "--force-bottle", {
      ...
      def upgrade_outdated_formulae(formulae)
        ...
        if args.build_from_source?
          ...
          unless DinrusHub::EnvConfig.developer?
