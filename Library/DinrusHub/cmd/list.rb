    ...
    class List < AbstractCommand
      ...
      def run
        if args.full_name? &&
          ...
          if !args.cask? && DRXHUB_CELLAR.exist? && DRXHUB_CELLAR.children.any?
            ohai "Formulae" if $stdout.tty? && !args.formula?
            safe_system "ls", *ls_args, DRXHUB_CELLAR
      ...
      def filtered_list
        names = if args.no_named?
          ...
          racks.select do |rack|
            DinrusHub.failed = true unless rack.exist?
            rack.exist?
          end
        end
        if args.pinned?
          ...
          names.sort.each do |d|
            keg_pin = (DRXHUB_PINNED_KEGS/d.basename.to_s)
      ...
      def list_casks
        casks = if args.no_named?
          ...
          filtered_args = args.named.dup.delete_if do |n|
            DinrusHub.failed = true unless Cask::Caskroom.path.join(n).exist?
            !Cask::Caskroom.path.join(n).exist?
          end
          # NamedAargs subclasses array
          T.cast(filtered_args, DinrusHub::CLI::NamedArgs).to_formulae_and_casks(only: :cask)
        end
