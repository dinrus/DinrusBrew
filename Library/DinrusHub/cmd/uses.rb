...
module DinrusHub
  module Cmd
    # `dhub uses foo bar` returns formulae that use both foo and bar
    ...
    class Uses < AbstractCommand
      ...
      def intersection_of_dependents(use_runtime_dependents, used_formulae)
        ...
        if use_runtime_dependents
          ...
          if !args.installed? && !(all || DinrusHub::EnvConfig.eval_all?)
            raise UsageError, "`dhub uses` needs `--installed` or `--eval-all` passed or `$DRXHUB_EVAL_ALL` set!"
          end
