...
module DinrusHub
  module Cmd
    class UpdateReport < AbstractCommand
      cmd_args do
        description <<~EOS
          The Ruby implementation of `dhub update`. Never called manually.
        EOS
      ...
      def auto_update_header
        @auto_update_header ||= begin
          ohai "Auto-updated DinrusHub!" if args.auto_update?
          true
        end
      end

      def output_update_report
        # Run `dhub update` (again) if we've got a linuxbrew-core CoreTap
        if CoreTap.instance.installed? && CoreTap.instance.linuxbrew_core? &&
           ENV["DRXHUB_LINUXBREW_CORE_MIGRATION"].blank?
          ohai "Re-running `dhub update` for linuxbrew-core migration"

          if DinrusHub::EnvConfig.core_git_remote != DRXHUB_CORE_DEFAULT_GIT_REMOTE
            opoo <<~EOS
              DRXHUB_CORE_GIT_REMOTE was set: #{DinrusHub::EnvConfig.core_git_remote}.
              It has been unset for the migration.
              You may need to change this from a linuxbrew-core mirror to a homebrew-core one.

            EOS
          end
          ENV.delete("DRXHUB_CORE_GIT_REMOTE")

          if DinrusHub::EnvConfig.bottle_domain != DRXHUB_BOTTLE_DEFAULT_DOMAIN
            opoo <<~EOS
              DRXHUB_BOTTLE_DOMAIN was set: #{DinrusHub::EnvConfig.bottle_domain}.
              It has been unset for the migration.
              You may need to change this from a Linuxbrew package mirror to a DinrusHub one.

            EOS
          end
          ENV.delete("DRXHUB_BOTTLE_DOMAIN")

          ENV["DRXHUB_LINUXBREW_CORE_MIGRATION"] = "1"
          FileUtils.rm_f DRXHUB_LOCKS/"update"

          update_args = []
          update_args << "--auto-update" if args.auto_update?
          update_args << "--force" if args.force?
          exec DRXHUB_BREW_FILE, "update", *update_args
        end

        if ENV["DRXHUB_ADDITIONAL_GOOGLE_ANALYTICS_ID"].present?
          opoo "DRXHUB_ADDITIONAL_GOOGLE_ANALYTICS_ID is now a no-op so can be unset."
          puts "All DinrusHub Google Analytics code and data was destroyed."
        end

        if ENV["DRXHUB_NO_GOOGLE_ANALYTICS"].present?
          opoo "DRXHUB_NO_GOOGLE_ANALYTICS is now a no-op so can be unset."
          puts "All DinrusHub Google Analytics code and data was destroyed."
        end
        ...
        initial_revision = ENV["DRXHUB_UPDATE_BEFORE"].to_s
        current_revision = ENV["DRXHUB_UPDATE_AFTER"].to_s
        ...
        if initial_revision != current_revision
          ...
          new_tag = Utils.popen_read(
            "git", "-C", DRXHUB_REPOSITORY, "tag", "--list", "--sort=-version:refname", "*.*"
          ).lines.first.chomp

          Settings.write "latesttag", new_tag if new_tag != old_tag

          if new_tag == old_tag
            ohai "Updated DinrusHub from #{shorten_revision(initial_revision)} " \
                 "to #{shorten_revision(current_revision)}."
          elsif old_tag.blank?
            ohai "Updated DinrusHub from #{shorten_revision(initial_revision)} " \
                 "to #{new_tag} (#{shorten_revision(current_revision)})."
          else
            ohai "Updated DinrusHub from #{old_tag} (#{shorten_revision(initial_revision)}) " \
                 "to #{new_tag} (#{shorten_revision(current_revision)})."
          end
        end

        # Check if we can parse the JSON and do any Ruby-side follow-up.
        unless DinrusHub::EnvConfig.no_install_from_api?
          DinrusHub::API::Formula.write_names_and_aliases
          DinrusHub::API::Cask.write_names
        end

        DinrusHub.failed = true if ENV["DRXHUB_UPDATE_FAILED"]
        return if DinrusHub::EnvConfig.disable_load_formula?
        ...
        Tap.installed.each do |tap|
          ...
          next if (tap.core_tap? || tap.core_cask_tap?) && !DinrusHub::EnvConfig.no_install_from_api?

          if ENV["DRXHUB_MIGRATE_LINUXBREW_FORMULAE"].present? && tap.core_tap? &&
            ...
            LINUXBREW_CORE_MIGRATION_LIST.each do |name|
              ...
              tab = keg.tab
              # force a `dhub upgrade` from the linuxbrew-core version to the homebrew-core version (even if lower)
        ...
        unless DinrusHub::EnvConfig.no_install_from_api?
          api_cache = DinrusHub::API::DRXHUB_CACHE_API
        ...
        if updated
          if hub.empty?
            ...
            if ENV.fetch("DRXHUB_UPDATE_REPORT_ONLY_INSTALLED", false)
              opoo "DRXHUB_UPDATE_REPORT_ONLY_INSTALLED is now the default behaviour, " \
        ...
        elsif !args.auto_update? && !ENV["DRXHUB_UPDATE_FAILED"] && !ENV["DRXHUB_MIGRATE_LINUXBREW_FORMULAE"]
        ...
        failed_fetch_dirs = ENV["DRXHUB_MISSING_REMOTE_REF_DIRS"]&.split("\n")
        if failed_fetch_dirs.present?
          ...
          ofail <<~EOS
            ...
            Reset taps to point to the correct remote branches by running `dhub tap --repair`
          EOS
        end
        ...
        if old_tag.blank? || new_major_version > old_major_version || new_minor_version > old_minor_version
          puts <<~EOS
            The #{new_major_version}.#{new_minor_version}.0 release notes are available on the DinrusHub Blog:
      ...
      def shorten_revision(revision)
        Utils.popen_read("git", "-C", DRXHUB_REPOSITORY, "rev-parse", "--short", revision).chomp
      end

      def tap_or_untap_core_taps_if_necessary
        return if ENV["DRXHUB_UPDATE_TEST"]
        ...
        if DinrusHub::EnvConfig.no_install_from_api?
          ...
          ENV["DRXHUB_UPDATE_BEFORE_DRXHUB_DRXHUB_CORE"] = revision
          ENV["DRXHUB_UPDATE_AFTER_DRXHUB_DRXHUB_CORE"] = revision
        else
          return if DinrusHub::EnvConfig.developer? || ENV["DRXHUB_DEV_CMD_RUN"]
          return if ENV["DRXHUB_GITHUB_HOSTED_RUNNER"] || ENV["GITHUB_ACTIONS_DRXHUB_SELF_HOSTED"]
          return if (DRXHUB_PREFIX/".homebrewdocker").exist?
          ...
          [CoreTap.instance, CoreCaskTap.instance].each do |tap|
            ...
            if tap.git_branch == "master" &&
              ...
              puts "  dhub untap #{tap.name}"
            end
          end
        end
      end

      def link_completions_manpages_and_docs(repository = DRXHUB_REPOSITORY)
        command = "dhub update"
      ...
      def analytics_message
        ...
        if Utils::Analytics.disabled? && !Utils::Analytics.influx_message_displayed?
          ohai "DinrusHub's analytics have entirely moved to our InfluxDB instance in the EU."
          ...
          puts "  dhub analytics on"
        elsif !Utils::Analytics.disabled?
          ENV["DRXHUB_NO_ANALYTICS_THIS_RUN"] = "1"
          # Use the shell's audible bell.
          print "\a"

          # Use an extra newline and bold to avoid this being missed.
          ohai "DinrusHub collects anonymous analytics."
          puts <<~EOS
            #{Tty.bold}Read the analytics documentation (and how to opt-out) here:
              #{Formatter.url("https://docs.brew.sh/Analytics")}#{Tty.reset}
            No analytics have been recorded yet (nor will be during this `dhub` run).
      ...
      def donation_message
        ...
        ohai "DinrusHub is run entirely by unpaid volunteers. Please consider donating:"
      ...
      def install_from_api_message
        ...
        no_install_from_api_set = DinrusHub::EnvConfig.no_install_from_api? &&
                                  !DinrusHub::EnvConfig.automatically_set_no_install_from_api?
        return unless no_install_from_api_set

        ohai "You have DRXHUB_NO_INSTALL_FROM_API set"
        puts "DinrusHub >=4.1.0 is dramatically faster and less error-prone when installing"
        puts "from the JSON API. Please consider unsetting DRXHUB_NO_INSTALL_FROM_API."
...
class Reporter
  ...
  def initialize(tap, api_names_txt: nil, api_names_before_txt: nil, api_dir_prefix: nil)
    ...
    if installed_from_api?(api_names_txt, api_names_before_txt, api_dir_prefix)
      ...
      initial_revision_var = "DRXHUB_UPDATE_BEFORE#{tap.repository_var_suffix}"
      @initial_revision = ENV[initial_revision_var].to_s
      raise ReporterRevisionUnsetError, initial_revision_var if @initial_revision.empty?

      current_revision_var = "DRXHUB_UPDATE_AFTER#{tap.repository_var_suffix}"
  ...
  def migrate_tap_migration
    (report[:D] + report[:DC]).each do |full_name|
      ...
      if report[:DC].include? full_name
        next unless (DRXHUB_PREFIX/"Caskroom"/new_name).exist?

        new_tap = Tap.fetch(new_tap_name)
        new_tap.ensure_installed!
        ohai "#{name} has been moved to DinrusHub.", <<~EOS
          To uninstall the cask, run:
            dhub uninstall --cask --force #{name}
        EOS
        next if (DRXHUB_CELLAR/new_name.split("/").last).directory?

        ohai "Installing #{new_name}..."
        system DRXHUB_BREW_FILE, "install", new_full_name
        begin
          unless Formulary.factory(new_full_name).keg_only?
            system DRXHUB_BREW_FILE, "link", new_full_name, "--overwrite"
          end
      ...
      next unless (dir = DRXHUB_CELLAR/name).exist? # skip if formula is not installed.
      ...
      if new_tap_name.start_with?("homebrew/cask")
        if new_tap.installed? && (DRXHUB_PREFIX/"Caskroom").directory?
          ohai "#{name} has been moved to DinrusHub Cask."
          ohai "dhub unlink #{name}"
          system DRXHUB_BREW_FILE, "unlink", name
          ohai "dhub cleanup"
          system DRXHUB_BREW_FILE, "cleanup"
          ohai "dhub install --cask #{new_name}"
          system DRXHUB_BREW_FILE, "install", "--cask", new_name
          ohai <<~EOS
            #{name} has been moved to DinrusHub Cask.
            The existing keg has been unlinked.
            Please uninstall the formula when convenient by running:
              dhub uninstall --force #{name}
          EOS
        else
          ohai "#{name} has been moved to DinrusHub Cask.", <<~EOS
            To uninstall the formula and install the cask, run:
              dhub uninstall --force #{name}
              dhub tap #{new_tap_name}
              dhub install --cask #{new_name}
          EOS
        end
      else
        new_tap.ensure_installed!
  ...
  def migrate_formula_rename(force:, verbose:)
    Formula.installed.each do |formula|
      ...
      oldnames_to_migrate = formula.oldnames.select do |oldname|
        oldname_rack = DRXHUB_CELLAR/oldname
...
class ReporterHub
  ...
  def dump(auto_update: false)
    unless DinrusHub::EnvConfig.no_update_report_new?
    ...
    puts <<~EOS
      You can upgrade #{update_pronoun} with #{Tty.bold}dhub upgrade#{Tty.reset}
      or list #{update_pronoun} with #{Tty.bold}dhub outdated#{Tty.reset}.
    EOS
  end
  ...
  def dump_new_cask_report
    return if DinrusHub::SimulateSystem.simulating_or_running_on_linux?
  ...
  def installed?(formula)
    (DRXHUB_CELLAR/formula.split("/").last).directory?
  end
