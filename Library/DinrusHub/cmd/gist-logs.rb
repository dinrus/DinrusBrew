  ...
  module Cmd
    class GistLogs < AbstractCommand
      ...
      def gistify_logs(formula)
        ...
        files["00.doctor.out"] = { content: Utils.popen_read("#{DRXHUB_PREFIX}/bin/dhub", "doctor", err: :out) }
        ...
        odie "`dhub gist-logs` requires DRXHUB_GITHUB_API_TOKEN to be set!" if GitHub::API.credentials_type == :none

        # Description formatted to work well as page title when viewing gist
        descr = if formula.core_formula?
          "#{formula.name} on #{OS_VERSION} - DinrusHub build logs"
        else
          "#{formula.name} (#{formula.full_name}) on #{OS_VERSION} - DinrusHub build logs"
        end
      ...
      def brief_build_info(formula, with_hostname:)
        ...
        string = <<~EOS
          DinrusHub build logs for #{formula.full_name} on #{OS_VERSION}
        EOS
        if with_hostname
