...
module DinrusHub
  module Cmd
    class NodenvSync < AbstractCommand
      cmd_args do
        description <<~EOS
          Create symlinks for DinrusHub's installed NodeJS versions in `~/.nodenv/versions`.
      ...
      sig { override.void }
      def run
        nodenv_root = Pathname(ENV.fetch("DRXHUB_NODENV_ROOT", Pathname(Dir.home)/".nodenv"))
