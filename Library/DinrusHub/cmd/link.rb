...
module DinrusHub
  module Cmd
    class Link < AbstractCommand
      cmd_args do
        description <<~EOS
          Symlink all of <formula>'s installed files into DinrusHub's prefix.
        ...
        switch "-n", "--dry-run",
               description: "List files which would be linked or deleted by " \
                            "`dhub link --overwrite` without actually linking or deleting any files."
        switch "-f", "--force",
      ...
      def run
        ...
        kegs.freeze.each do |keg|
          ...
          if keg.linked?
            ...
            puts <<~EOS
              Для перекомпоновки, выполните:
                dhub unlink #{keg.name} && dhub link #{name_and_flag}
            EOS
            next
          end
          ...
          if keg_only
            if DRXHUB_PREFIX.to_s == DRXHUB_DEFAULT_PREFIX && formula.present? &&
          ...
          keg.lock do
            ...
            puts_keg_only_path_message(keg) if keg_only && !DinrusHub::EnvConfig.developer?
          end
        end
      end
      ...
      def puts_keg_only_path_message(keg)
        ...
        opt = DRXHUB_PREFIX/"opt/#{keg.name}"
