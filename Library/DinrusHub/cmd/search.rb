  ...
  module Cmd
    class SearchCmd < AbstractCommand
      ...
      cmd_args do
        ...
        switch "--eval-all",
               ...
               description: "Evaluate all available formulae and casks, whether installed or not, to search their " \
                            "descriptions. Implied if `$DRXHUB_EVAL_ALL` is set."
        switch "--pull-request",
      ...
      def run
        ...
        if args.desc?
          if !args.eval_all? && !DinrusHub::EnvConfig.eval_all? && DinrusHub::EnvConfig.no_install_from_api?
            raise UsageError, "`dhub search --desc` needs `--eval-all` passed or `$DRXHUB_EVAL_ALL` set!"
          end
        ...
        puts "Use `dhub desc` to list packages with a short description." if args.verbose?
