...
module DinrusHub
  module Cmd
    class CleanupCmd < AbstractCommand
      cmd_args do
        days = DinrusHub::EnvConfig::ENVS[:DRXHUB_CLEANUP_MAX_AGE_DAYS]&.dig(:default)
        description <<~EOS
          Remove stale lock files and outdated downloads for all formulae and casks,
          and remove old versions of installed formulae. If arguments are specified,
          only do this for the given formulae and casks. Removes all downloads more than
          #{days} days old. This can be adjusted with `$DRXHUB_CLEANUP_MAX_AGE_DAYS`.
        EOS
        flag   "--prune=",
        ...
        switch "-s", "--scrub",
               description: "Scrub the cache, including downloads for even the latest versions. " \
                            ...
                            "If you want to delete those too: `rm -rf \"$(dhub --cache)\"`"
        switch "--prune-prefix",
