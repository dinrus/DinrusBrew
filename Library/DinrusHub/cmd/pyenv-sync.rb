...
module DinrusHub
  module Cmd
    class PyenvSync < AbstractCommand
      cmd_args do
        description <<~EOS
          Create symlinks for DinrusHub's installed Python versions in `~/.pyenv/versions`.
      ...
      sig { override.void }
      def run
        pyenv_root = Pathname(ENV.fetch("DRXHUB_PYENV_ROOT", Pathname(Dir.home)/".pyenv"))
