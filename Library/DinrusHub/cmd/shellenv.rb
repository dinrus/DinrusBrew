    ...
    class Shellenv < AbstractCommand
      ...
      cmd_args do
        description <<~EOS
          ...
          Print export statements. When run in a shell, this installation of DinrusHub will be added to your `PATH`, `MANPATH`, and `INFOPATH`.

          The variables `$DRXHUB_PREFIX`, `$DRXHUB_CELLAR` and `$DRXHUB_REPOSITORY` are also exported to avoid
          querying them multiple times.
          To help guarantee idempotence, this command produces no output when DinrusHub's `bin` and `sbin` directories
