    ...
    class Deps < AbstractCommand
      ...
      cmd_args do
        description <<~EOS
          ...
          to `dhub linkage`), which may differ from the current versions' stated
      ...
      sig { override.void }
      def run
        raise UsageError, "`dhub deps --os=all` is not supported" if args.os == "all"
        raise UsageError, "`dhub deps --arch=all` is not supported" if args.arch == "all"
