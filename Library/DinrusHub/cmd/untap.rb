    ...
    class Untap < AbstractCommand
      ...
      def run
        args.named.to_installed_taps.each do |tap|
          odie "Untapping #{tap} is not allowed" if tap.core_tap? && DinrusHub::EnvConfig.no_install_from_api?

          if DinrusHub::EnvConfig.no_install_from_api? || (!tap.core_tap? && !tap.core_cask_tap?)
            ...
            if installed_tap_formulae.present? || installed_tap_casks.present?
              ...
              if args.force? || DinrusHub::EnvConfig.developer?
                opoo <<~EOS
