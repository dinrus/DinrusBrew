  ...
  module Cmd
    class ReadallCmd < AbstractCommand
      cmd_args do
        ...
        switch "--syntax",
               description: "Syntax-check all of DinrusHub's Ruby files (if no <tap> is passed)."
        switch "--eval-all",
      ...
      def run
        DinrusHub.with_no_api_env do
          if args.syntax? && args.no_named?
            scan_files = "#{DRXHUB_LIBRARY_PATH}/**/*.rb"
            ruby_files = Dir.glob(scan_files).grep_v(%r{/(vendor)/})

            DinrusHub.failed = true unless Readall.valid_ruby_syntax?(ruby_files)
          end
          ...
          taps = if args.no_named?
            if !args.eval_all? && !DinrusHub::EnvConfig.eval_all?
              raise UsageError, "`dhub readall` needs a tap or `--eval-all` passed or `$DRXHUB_EVAL_ALL` set!"
            end
          ...
          taps.each do |tap|
            DinrusHub.failed = true unless Readall.valid_tap?(tap, **options)
          end
        end
      end
    end
