...
module DinrusHub
  module Cmd
    class Log < AbstractCommand
      ...
      cmd_args do
        description <<~EOS
          Show the `git log` for <formula> or <cask>, or show the log for the DinrusHub repository
      ...
      def git_log(cd_dir, path = nil, tap = nil)
        cd cd_dir do
          ...
          if tap
            name = tap.to_s
            git_cd = "$(dhub --repo #{tap})"
          elsif cd_dir == DRXHUB_REPOSITORY
            name = "Homebrew/brew"
            git_cd = "$(dhub --repo)"
          else
            name, git_cd = cd_dir
