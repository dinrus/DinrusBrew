# Documentation defined in Library/DinrusHub/cmd/update-reset.rb

# Replaces the function in Library/DinrusHub/brew.sh to cache the Git executable to provide
...
homebrew-update-reset() {
  ...
  if [[ -z "${REPOS[*]}" ]]
  then
    REPOS+=("${DRXHUB_REPOSITORY}" "${DRXHUB_LIBRARY}"/Taps/*/*)
  fi
  ...
  do
    ...
    ohai "Resetting ${DIR}..."
    # DRXHUB_* variables here may all set by bin/dhub or the user
    # shellcheck disable=SC2154
    if [[ "${DIR}" == "${DRXHUB_REPOSITORY}" &&
       (-n "${DRXHUB_UPDATE_TO_TAG}" ||
       (-z "${DRXHUB_DEVELOPER}" && -z "${DRXHUB_DEV_CMD_RUN}")) ]]
    then
