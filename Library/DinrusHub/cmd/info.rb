...
module DinrusHub
  module Cmd
    class Info < AbstractCommand
      ...
      cmd_args do
        description <<~EOS
          Display brief statistics for your DinrusHub installation.
          If a <formula> or <cask> is provided, show summary of information about it.
        EOS
        switch "--analytics",
               description: "List global DinrusHub analytics data or, if specified, installation and " \
                            "build error data for <formula> (provided neither `$DRXHUB_NO_ANALYTICS` " \
                            "nor `$DRXHUB_NO_GITHUB_API` are set)."
        flag   "--days=",
        ...
        switch "--github",
               description: "Open the GitHub source page for <formula> and <cask> in a browser. " \
                            "To view the history locally: `dhub log -p` <formula> or <cask>"
        ...
        switch "--eval-all",
               ...
               description: "Evaluate all available formulae and casks, whether installed or not, to print their " \
                            "JSON. Implied if `$DRXHUB_EVAL_ALL` is set."
        switch "--variations",
               depends_on:  "--json",
      ...
      def print_statistics
        ...
        puts "#{Utils.pluralize("keg", count, include_count: true)}, #{DRXHUB_CELLAR.dup.abv}"
      end
