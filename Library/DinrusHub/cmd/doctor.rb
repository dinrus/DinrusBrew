    ...
    class Doctor < AbstractCommand
      cmd_args do
        description <<~EOS
          ...
          Please note that these warnings are just used to help the DinrusHub maintainers
          with debugging if you file an issue. If everything you use DinrusHub for
      ...
      sig { override.void }
      def run
        DinrusHub.inject_dump_stats!(Diagnostic::Checks, /^check_*/) if args.audit_debug?
        ...
        methods.each do |method|
          ...
          if first_warning
            $stderr.puts <<~EOS
              #{Tty.bold}Please note that these warnings are just used to help the DinrusHub maintainers
              with debugging if you file an issue. If everything you use DinrusHub for is
        ...
        puts "Your system is ready to dhub." if !DinrusHub.failed? && !args.quiet?
      end
    end
  end
end
