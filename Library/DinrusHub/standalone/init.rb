...
required_ruby_major, required_ruby_minor, = ENV.fetch("DRXHUB_REQUIRED_RUBY_VERSION", "").split(".").map(&:to_i)
gems_vendored = if required_ruby_minor.nil?
  ...
  if ruby_major < required_ruby_major || (ruby_major == required_ruby_major && ruby_minor < required_ruby_minor)
    raise "DinrusHub must be run under Ruby #{required_ruby_major}.#{required_ruby_minor}! " \
...
DRXHUB_LIBRARY_PATH = Pathname(dir).parent.realpath.freeze
DRXHUB_USING_PORTABLE_RUBY = RbConfig.ruby.include?("/vendor/portable-ruby/").freeze

require_relative "../utils/gems"
DinrusHub.setup_gem_environment!(setup_path: false)

# Install gems for Rubies we don't vendor for.
if !gems_vendored && !ENV["DRXHUB_SKIP_INITIAL_GEM_INSTALL"]
  DinrusHub.install_bundler_gems!(setup_path: false)
  ENV["DRXHUB_SKIP_INITIAL_GEM_INSTALL"] = "1"
end

unless $LOAD_PATH.include?(DRXHUB_LIBRARY_PATH.to_s)
  # Insert the path after any existing DinrusHub paths (e.g. those inserted by tests and parent processes)
  last_homebrew_path_idx = $LOAD_PATH.rindex do |path|
    path.start_with?(DRXHUB_LIBRARY_PATH.to_s) && !path.include?("vendor/portable-ruby")
  end || -1
  $LOAD_PATH.insert(last_homebrew_path_idx + 1, DRXHUB_LIBRARY_PATH.to_s)
end
require_relative "../vendor/bundle/bundler/setup"
require "portable_ruby_gems" if DRXHUB_USING_PORTABLE_RUBY
$LOAD_PATH.unshift "#{DRXHUB_LIBRARY_PATH}/vendor/bundle/#{RUBY_ENGINE}/#{Gem.ruby_api_version}/gems/" \
                   "bundler-#{DinrusHub::DRXHUB_BUNDLER_VERSION}/lib"
$LOAD_PATH.uniq!
