...
# A {Tap} is used to extend the formulae provided by DinrusHub core.
...
class Tap
  ...
  DRXHUB_TAP_CASK_RENAMES_FILE = "cask_renames.json"
  private_constant :DRXHUB_TAP_CASK_RENAMES_FILE
  DRXHUB_TAP_FORMULA_RENAMES_FILE = "formula_renames.json"
  private_constant :DRXHUB_TAP_FORMULA_RENAMES_FILE
  DRXHUB_TAP_MIGRATIONS_FILE = "tap_migrations.json"
  private_constant :DRXHUB_TAP_MIGRATIONS_FILE
  DRXHUB_TAP_AUTOBUMP_FILE = ".github/autobump.txt"
  private_constant :DRXHUB_TAP_AUTOBUMP_FILE
  DRXHUB_TAP_PYPI_FORMULA_MAPPINGS_FILE = "pypi_formula_mappings.json"
  private_constant :DRXHUB_TAP_PYPI_FORMULA_MAPPINGS_FILE
  DRXHUB_TAP_SYNCED_VERSIONS_FORMULAE_FILE = "synced_versions_formulae.json"
  private_constant :DRXHUB_TAP_SYNCED_VERSIONS_FORMULAE_FILE
  DRXHUB_TAP_AUDIT_EXCEPTIONS_DIR = "audit_exceptions"
  private_constant :DRXHUB_TAP_AUDIT_EXCEPTIONS_DIR
  DRXHUB_TAP_STYLE_EXCEPTIONS_DIR = "style_exceptions"
  private_constant :DRXHUB_TAP_STYLE_EXCEPTIONS_DIR

  TAP_MIGRATIONS_STALE_SECONDS = 86400 # 1 day
  private_constant :TAP_MIGRATIONS_STALE_SECONDS

  DRXHUB_TAP_JSON_FILES = %W[
    #{DRXHUB_TAP_FORMULA_RENAMES_FILE}
    #{DRXHUB_TAP_CASK_RENAMES_FILE}
    #{DRXHUB_TAP_MIGRATIONS_FILE}
    #{DRXHUB_TAP_PYPI_FORMULA_MAPPINGS_FILE}
    #{DRXHUB_TAP_SYNCED_VERSIONS_FORMULAE_FILE}
    #{DRXHUB_TAP_AUDIT_EXCEPTIONS_DIR}/*.json
    #{DRXHUB_TAP_STYLE_EXCEPTIONS_DIR}/*.json
  ].freeze
  ...
  def self.fetch(user, repository = T.unsafe(nil))
    ...
    repository = repository.sub(DRXHUB_OFFICIAL_REPO_PREFIXES_REGEX, "")
  ...
  def self.from_path(path)
    match = File.expand_path(path).match(DRXHUB_TAP_PATH_REGEX)
  ...
  def self.with_formula_name(name)
    return unless (match = name.match(DRXHUB_TAP_FORMULA_REGEX))
  ...
  def self.with_cask_token(token)
    return unless (match = token.match(DRXHUB_TAP_CASK_REGEX))
  ...
  def self.allowed_taps
    cache_key = :"allowed_taps_#{DinrusHub::EnvConfig.allowed_taps.to_s.tr(" ", "_")}"
    cache[cache_key] ||= begin
      allowed_tap_list = DinrusHub::EnvConfig.allowed_taps.to_s.split

      Set.new(allowed_tap_list.filter_map do |tap|
        Tap.fetch(tap)
      rescue Tap::InvalidNameError
        opoo "Неполноценное имя тапа в `DRXHUB_ALLOWED_TAPS`: #{tap}"
        nil
      end).freeze
    end
  end
  ...
  def self.forbidden_taps
    cache_key = :"forbidden_taps_#{DinrusHub::EnvConfig.forbidden_taps.to_s.tr(" ", "_")}"
    cache[cache_key] ||= begin
      forbidden_tap_list = DinrusHub::EnvConfig.forbidden_taps.to_s.split

      Set.new(forbidden_tap_list.filter_map do |tap|
        Tap.fetch(tap)
      rescue Tap::InvalidNameError
        opoo "Неполноценное имя тапа в `DRXHUB_FORBIDDEN_TAPS`: #{tap}"
        nil
      end).freeze
    end
  end
  ...
  def initialize(user, repository)
    ...
    @path = DRXHUB_TAP_DIRECTORY/@full_name.downcase
  ...
  def remote_repository
    ...
    return unless (match = remote.match(DRXHUB_TAP_REPOSITORY_REGEX))
  ...
  def repository_var_suffix
    @repository_var_suffix ||= path.to_s
                                   .delete_prefix(DRXHUB_TAP_DIRECTORY.to_s)
                                   .tr("^A-Za-z0-9", "_")
                                   .upcase
  end
  ...
  # Check whether this {Tap} is an official DinrusHub tap.
  #
  # @api public
  def official?
    user == "Homebrew"
  ...
  def install(quiet: false, clone_target: nil,
    ...
    if !allowed_by_env? || forbidden_by_env?
      owner = DinrusHub::EnvConfig.forbidden_owner
      owner_contact = if (contact = DinrusHub::EnvConfig.forbidden_owner_contact.presence)
        "\n#{contact}"
      end

      error_message = "The installation of the #{full_name} was requested but #{owner}\n"
      error_message << "has not allowed this tap in `DRXHUB_ALLOWED_TAPS`" unless allowed_by_env?
      error_message << " and\n" if !allowed_by_env? && forbidden_by_env?
      error_message << "has forbidden this tap in `DRXHUB_FORBIDDEN_TAPS`" if forbidden_by_env?
    ...
    if installed?
      ...
      return
    elsif (core_tap? || core_cask_tap?) && !DinrusHub::EnvConfig.no_install_from_api? && !force
      odie "Таппирование #{name}, как правило, больее не требуется.\n" \
           "Add #{Formatter.option("--force")} if you are sure you need it for contributing to DinrusHub."
    end

    clear_cache
    Tap.clear_cache

    $stderr.ohai "Таппируется #{name}" unless quiet
    ...
    begin
      ...
      if verify && !DinrusHub::EnvConfig.developer? && !Readall.valid_tap?(self, aliases: true)
    ...
    if official?
      ...
      if untapped.empty?
        DinrusHub::Settings.delete :untapped
      else
        DinrusHub::Settings.write :untapped, untapped.join(";")
      end
    end
  ...
  def link_completions_and_manpages
    ...
    command = "dhub tap --repair"
    Utils::Link.link_manpages(path, command)

    require "completions"
    DinrusHub::Completions.show_completions_message_if_needed
    if official? || DinrusHub::Completions.link_completions?
  ...
  sig { returns(T::Hash[String, String]) }
  def cask_renames
    @cask_renames ||= if (rename_file = path/DRXHUB_TAP_CASK_RENAMES_FILE).file?
  ...
  sig { returns(T::Hash[String, String]) }
  def formula_renames
    @formula_renames ||= if (rename_file = path/DRXHUB_TAP_FORMULA_RENAMES_FILE).file?
      JSON.parse(rename_file.read)
    else
      {}
    end
  ...
  sig { returns(T::Hash[String, String]) }
  def tap_migrations
    @tap_migrations ||= if (migration_file = path/DRXHUB_TAP_MIGRATIONS_FILE).file?
  ...
  sig { returns(T::Array[String]) }
  def autobump
    @autobump ||= if (autobump_file = path/DRXHUB_TAP_AUTOBUMP_FILE).file?
  ...
  def allow_bump?(formula_or_cask_name)
    ENV["DRXHUB_TEST_BOT_AUTOBUMP"].present? || !official? || autobump.exclude?(formula_or_cask_name)
  end

  # Hash with audit exceptions
  sig { returns(Hash) }
  def audit_exceptions
    @audit_exceptions ||= read_formula_list_directory("#{DRXHUB_TAP_AUDIT_EXCEPTIONS_DIR}/*")
  end

  # Hash with style exceptions
  sig { returns(Hash) }
  def style_exceptions
    @style_exceptions ||= read_formula_list_directory("#{DRXHUB_TAP_STYLE_EXCEPTIONS_DIR}/*")
  end

  # Hash with pypi formula mappings
  sig { returns(Hash) }
  def pypi_formula_mappings
    @pypi_formula_mappings ||= read_formula_list(path/DRXHUB_TAP_PYPI_FORMULA_MAPPINGS_FILE)
  end

  # Array with synced versions formulae
  sig { returns(T::Array[T::Array[String]]) }
  def synced_versions_formulae
    @synced_versions_formulae ||= if (synced_file = path/DRXHUB_TAP_SYNCED_VERSIONS_FORMULAE_FILE).file?
  ...
  def self.installed
    cache[:installed] ||= if DRXHUB_TAP_DIRECTORY.directory?
      DRXHUB_TAP_DIRECTORY.subdirs.flat_map(&:subdirs).map { from_path(_1) }
    else
      []
    end
  end
  ...
  def self.all
    cache[:all] ||= begin
      core_taps = [
        ...
        (CoreCaskTap.instance if OS.mac?), # rubocop:disable DinrusHub/MoveToExtendOS
      ].compact
  ...
  # @api public
  def self.each(&block)
    if DinrusHub::EnvConfig.no_install_from_api?
  ...
  def self.untapped_official_taps
    DinrusHub::Settings.read(:untapped)&.split(";") || []
  end
...
class AbstractCoreTap < Tap
  ...
  def ensure_installed!
    ...
    return if DinrusHub::EnvConfig.automatically_set_no_install_from_api?
  ...
  def should_report_analytics?
    return super if DinrusHub::EnvConfig.no_install_from_api?
...
class CoreTap < AbstractCoreTap
  ...
  def remote
    ...
    DinrusHub::EnvConfig.core_git_remote
  end

  # CoreTap never allows shallow clones (on request from GitHub).
  def install(quiet: false, clone_target: nil,
              custom_remote: false, verify: false, force: false)
    remote = DinrusHub::EnvConfig.core_git_remote # set by DRXHUB_CORE_GIT_REMOTE
    requested_remote = clone_target || remote

    # The remote will changed again on `dhub update` since remotes for homebrew/core are mismatched
    raise TapCoreRemoteMismatchError.new(name, remote, requested_remote) if requested_remote != remote

    if remote != default_remote
      $stderr.puts "DRXHUB_CORE_GIT_REMOTE set: using #{remote} as the DinrusHub/homebrew-core Git remote."
    end
  ...
  def uninstall(manual: false)
    raise "Tap#uninstall is not available for CoreTap" if DinrusHub::EnvConfig.no_install_from_api?
  ...
  sig { returns(T::Hash[String, String]) }
  def formula_renames
    @formula_renames ||= if DinrusHub::EnvConfig.no_install_from_api?
      ensure_installed!
      super
    else
      DinrusHub::API::Formula.all_renames
    end
  end

  sig { returns(Hash) }
  def tap_migrations
    @tap_migrations ||= if DinrusHub::EnvConfig.no_install_from_api?
      ensure_installed!
      super
    elsif DinrusHub::API.internal_json_v3?
      DinrusHub::API::Formula.tap_migrations
    else
      migrations, = DinrusHub::API.fetch_json_api_file "formula_tap_migrations.jws.json",
  ...
  sig { returns(T::Hash[String, String]) }
  def alias_table
    @alias_table ||= if DinrusHub::EnvConfig.no_install_from_api?
      super
    else
      DinrusHub::API::Formula.all_aliases
    end
  end
  ...
  def formula_names
    ...
    DinrusHub::API::Formula.all_formulae.keys
  end
  ...
  def formula_files_by_name
    ...
    @formula_files_by_name ||= begin
      tap_path = path.to_s
      DinrusHub::API::Formula.all_formulae.each_with_object({}) do |item, hash|
...
class CoreCaskTap < AbstractCoreTap
  ...
  def cask_tokens
    ...
    DinrusHub::API::Cask.all_casks.keys
  end
  ...
  def cask_files_by_name
    ...
    @cask_files_by_name ||= begin
      tap_path = path.to_s
      DinrusHub::API::Cask.all_casks.each_with_object({}) do |item, hash|
        name, cask_hash = item
        # If there's more than one item with the same path: use the longer one to prioritise more specific results.
        existing_path = hash[name]
        # Pathname equivalent is slow in a tight loop
  ...
  def cask_renames
    @cask_renames ||= if DinrusHub::EnvConfig.no_install_from_api?
      super
    else
      DinrusHub::API::Cask.all_renames
    end
  end
  ...
  def tap_migrations
    @tap_migrations ||= if DinrusHub::EnvConfig.no_install_from_api?
      ...
      migrations, = DinrusHub::API.fetch_json_api_file "cask_tap_migrations.jws.json",
                                                      stale_seconds: TAP_MIGRATIONS_STALE_SECONDS
      migrations
    end
  end
...
class TapConfig
  ...
  def [](key)
    ...
    case DinrusHub::Settings.read(key, repo: tap.path)
    when "true" then true
  ...
  def []=(key, value)
    ...
    DinrusHub::Settings.write key, value.to_s, repo: tap.path
  end
  ...
  def delete(key)
    ...
    DinrusHub::Settings.delete key, repo: tap.path
  end
end