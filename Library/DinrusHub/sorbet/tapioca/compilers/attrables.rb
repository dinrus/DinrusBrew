...
module Tapioca
  module Compilers
    class Attrables < Tapioca::Dsl::Compiler
      ...
      def self.gather_constants = DinrusHub::Tapioca::Utils.named_objects_with_module(Attrable)

      sig { override.void }
      def decorate
        root.create_path(constant) do |klass|
          DinrusHub::Tapioca::Utils.methods_from_file(constant, ATTRABLE_FILENAME)
                                  .each { |method| compile_attrable_method(klass, method) }
          DinrusHub::Tapioca::Utils.methods_from_file(constant, ATTRABLE_FILENAME, class_methods: true)
