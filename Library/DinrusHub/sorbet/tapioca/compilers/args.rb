  ...
  module Compilers
    class Args < Tapioca::Dsl::Compiler
      GLOBAL_OPTIONS = T.let(
        DinrusHub::CLI::Parser.global_options.map do |short_option, long_option, _|
          [short_option, long_option].map { "#{DinrusHub::CLI::Parser.option_to_name(_1)}?" }
        end.flatten.freeze, T::Array[String]
      )

      Parsable = T.type_alias { T.any(T.class_of(DinrusHub::CLI::Args), T.class_of(DinrusHub::AbstractCommand)) }
      ...
      def self.gather_constants
        ...
        DinrusHub::AbstractCommand.subclasses
      end

      sig { override.void }
      def decorate
        cmd = T.cast(constant, T.class_of(DinrusHub::AbstractCommand))
        # This is a dummy class to make the `dhub` command parsable
        return if cmd == DinrusHub::Cmd::Brew

        args_class_name = T.must(T.must(cmd.args_class).name)
        root.create_class(args_class_name, superclass_name: "DinrusHub::CLI::Args") do |klass|
      ...
      sig { params(parser: DinrusHub::CLI::Parser).returns(T::Array[Symbol]) }
      ...
      sig { params(klass: RBI::Scope, parser: DinrusHub::CLI::Parser).void }
