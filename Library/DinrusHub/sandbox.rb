...
class Sandbox
  ...
  def allow_write_temp_and_cache
    ...
    allow_write_path DRXHUB_TEMP
    allow_write_path DRXHUB_CACHE
  end
  ...
  def deny_write_homebrew_repository
    deny_write path: DRXHUB_ORIGINAL_BREW_FILE
    if DRXHUB_PREFIX.to_s == DRXHUB_REPOSITORY.to_s
      deny_write_path DRXHUB_LIBRARY
      deny_write_path DRXHUB_REPOSITORY/".git"
    else
      deny_write_path DRXHUB_REPOSITORY
    end
  end
  ...
  def run(*args)
    Dir.mktmpdir("homebrew-sandbox", DRXHUB_TEMP) do |tmpdir|
      ...
      begin
        ...
        unless logs.empty?
          ...
          if @failed && DinrusHub::EnvConfig.verbose?
            ohai "Sandbox Log", logs
            $stdout.flush # without it, dhub test-bot would fail to catch the log
          end
        end
      end
    end
