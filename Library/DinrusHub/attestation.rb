...
module DinrusHub
  module Attestation
    ...
    # @api private
    DRXHUB_CORE_REPO = "Homebrew/homebrew-core"
    ...
    def self.enabled?
      return false if DinrusHub::EnvConfig.no_verify_attestations?
      return true if DinrusHub::EnvConfig.verify_attestations?
      ...
      (DinrusHub::EnvConfig.developer? || DinrusHub::EnvConfig.devcmdrun?) && GitHub::API.credentials.present?
    end
    ...
    def self.gh_executable
      ...
      # NOTE: We set DRXHUB_NO_VERIFY_ATTESTATIONS when installing `gh` itself,
      #       to prevent a cycle during bootstrapping. This can eventually be resolved
      #       by vendoring a pure-Ruby Sigstore verifier client.
      with_env(DRXHUB_NO_VERIFY_ATTESTATIONS: "1") do
    ...
    def self.sort_formulae_for_install(formulae)
      if formulae.include?(Formula["gh"])
        ...
        DinrusHub::Attestation.gh_executable
        formulae
      end
    end
    ...
    def self.check_attestation(bottle, signing_repo, signing_workflow = nil, subject = nil)
      ...
      begin
        result = system_command!(gh_executable, args: cmd,
                                 ...
                                 secrets: [credentials], print_stderr: false, chdir: DRXHUB_TEMP)
      ...
      attestation = if bottle.tag.to_sym == :all
        # :all-tagged bottles are created by `dhub bottle --merge`, and are not directly
    ...
    def self.check_core_attestation(bottle)
      begin
        ...
        attestation = check_attestation bottle, DRXHUB_CORE_REPO
        return attestation
        ...
        url_sha256 = if EnvConfig.bottle_domain == DRXHUB_BOTTLE_DEFAULT_DOMAIN
          ...
          path, = Utils::Bottles.path_resolved_basename DRXHUB_BOTTLE_DEFAULT_DOMAIN, bottle.name,
                                                        bottle.resource.checksum, bottle.filename
          url = "#{DRXHUB_BOTTLE_DEFAULT_DOMAIN}/#{path}"
      ...
      sleep sleep_time if ENV["DRXHUB_TESTS"].blank?
