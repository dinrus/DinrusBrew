...
# A formula provides instructions and metadata for DinrusHub to install a piece
# of software. Every DinrusHub formula is a {Formula}.
...
# `ThisFormula`. DinrusHub does enforce that the name of the file and the class
# correspond.
# Make sure you check with `dhub search` that the name is free!
# @abstract
...
class Formula
  ...
  include DinrusHub::Livecheck::Constants
  extend Forwardable
  extend Cachable
  extend Attrable
  extend APIHashable
  ...
  # Installed when using `dhub install --HEAD`.
  ...
  # Used for creating new DinrusHub versions of software without new upstream
  # versions.
  # @see .revision=
  sig { returns(Integer) }
  ...
  def installed_alias_path
    ...
    return unless path&.match?(%r{#{DRXHUB_TAP_DIR_REGEX}/Aliases}o)
  ...
  def specified_path
    return DinrusHub::API::Formula.cached_json_file_path if loaded_from_api?
  ...
  # @api internal
  def linked_keg
    linked_keg = possible_names.map { |name| DRXHUB_LINKED_KEGS/name }
                               .find(&:directory?)
    return linked_keg if linked_keg.present?

    DRXHUB_LINKED_KEGS/name
  end
  ...
  def installed_prefixes
    possible_names.map { |name| DRXHUB_CELLAR/name }
                  .select(&:directory?)
                  .flat_map(&:subdirs)
                  .sort_by(&:basename)
  end
  ...
  # This is symlinked into `DRXHUB_PREFIX` after installation or with
  # `dhub link` for formulae that are not keg-only.
  #
  # ### Examples
  #
  ...
  # The directory where the formula's binaries should be installed.
  # This is not symlinked into `DRXHUB_PREFIX`.
  # It is commonly used to install files that we do not wish to be
  # symlinked into `DRXHUB_PREFIX` from one of the other directories and
  ...
  # This directory is not inside the `DRXHUB_CELLAR` so it persists
  # across upgrades.
  #
  # @api public
  sig { returns(Pathname) }
  def etc
    (DRXHUB_PREFIX/"etc").extend(InstallRenamed)
  end

  # A subdirectory of `etc` with the formula name suffixed.
  # e.g. `$DRXHUB_PREFIX/etc/openssl@1.1`
  ...
  sig { returns(Pathname) }
  def pkgetc
    (DRXHUB_PREFIX/"etc"/name).extend(InstallRenamed)
  end
  ...
  # installation so, despite not being in `DRXHUB_CELLAR`, they are installed
  ...
  def plist
    odisabled "`Formula#plist`", "`DinrusHub::Service`"
    nil
  end
  ...
  def service
    @service ||= DinrusHub::Service.new(self, &self.class.service)
  end
  ...
  def install_etc_var
    ...
    Find.find(*etc_var_dirs.select(&:directory?)) do |path|
      ...
      path.cp_path_sub(bottle_prefix, DRXHUB_PREFIX)
    end
  end
  ...
  def run_post_install
    ...
    begin
      ...
      new_env = {
        ...
        TEMP:          DRXHUB_TEMP,
        TMP:           DRXHUB_TEMP,
        _JAVA_OPTIONS: "-Djava.io.tmpdir=#{DRXHUB_TEMP}",
        DRXHUB_PATH: nil,
  ...
  # Warn the user about any DinrusHub-specific issues or quirks for this package.
  ...
  # def caveats
  #   s = <<~EOS
  #     Print some important notice to the user when `dhub info [formula]` is
  ...
  def link_overwrite?(path)
    # Don't overwrite files not created by DinrusHub.
    return false if path.stat.uid != DRXHUB_ORIGINAL_BREW_FILE.stat.uid
    ...
    to_check = path.relative_path_from(DRXHUB_PREFIX).to_s
  ...
  def dhub(fetch: true, keep_tmp: false, debug_symbols: false, interactive: false, &_blk)
    @prefix_returns_versioned_prefix = true
  ...
  def lock
    ...
    oldnames.each do |oldname|
      next unless (oldname_rack = DRXHUB_CELLAR/oldname).exist?
  ...
  def oldnames_to_migrate
    oldnames.select do |oldname|
      old_rack = DRXHUB_CELLAR/oldname
  ...
  def std_cmake_args(install_prefix: prefix, install_libdir: "lib", find_framework: "LAST")
    %W[
      ...
      -DCMAKE_PROJECT_TOP_LEVEL_INCLUDES=#{DRXHUB_LIBRARY_PATH}/cmake/trap_fetchcontent_provider.cmake
      -Wno-dev
      -DBUILD_TESTING=OFF
    ]
  end
  ...
  def rpath(source: bin, target: lib)
    unless target.to_s.start_with?(DRXHUB_PREFIX)
      raise "rpath `target` should only be used for paths inside DRXHUB_PREFIX!"
    end
  ...
  def generate_completions_from_executable(*commands,
    ...
    shells.each do |shell|
      ...
      popen_read_options = {}
      popen_read_options[:err] = :err unless ENV["DRXHUB_STDERR"]
  ...
  def self.all(eval_all: false)
    if !eval_all && !DinrusHub::EnvConfig.eval_all?
      raise ArgumentError, "Formula#all without `--eval-all` or DRXHUB_EVAL_ALL"
    end
  ...
  def self.racks
    Formula.cache[:racks] ||= if DRXHUB_CELLAR.directory?
      DRXHUB_CELLAR.subdirs.reject do |rack|
  ...
  # True if this formula is provided by DinrusHub itself
  sig { returns(T::Boolean) }
  def core_formula?
    !!tap&.core_tap?
  end
  ...
  def to_hash_with_variations(hash_method: :to_hash)
    ...
    if loaded_from_api? && !DinrusHub::EnvConfig.no_install_from_api?
      json_formula = DinrusHub::API::Formula.all_formulae.fetch(name).dup
    ...
    if path.exist? && on_system_blocks_exist?
      ...
      OnSystem::ALL_OS_ARCH_COMBINATIONS.each do |os, arch|
        ...
        DinrusHub::SimulateSystem.with(os:, arch:) do
  ...
  def caveats_with_placeholders
    caveats&.gsub(DRXHUB_PREFIX, DRXHUB_PREFIX_PLACEHOLDER)
           &.gsub(DRXHUB_CELLAR, DRXHUB_CELLAR_PLACEHOLDER)
  end
  ...
  def run_test(keep_tmp: false)
    ...
    test_env = {
      ...
      TMP:           DRXHUB_TEMP,
      TERM:          "dumb",
      PATH:          PATH.new(ENV.fetch("PATH"), DRXHUB_PREFIX/"bin"),
      DRXHUB_PATH: nil,
    }.merge(common_stage_test_env)
    test_env[:_JAVA_OPTIONS] += " -Djava.io.tmpdir=#{DRXHUB_TEMP}"
  ...
  def test_fixtures(file)
    DRXHUB_LIBRARY_PATH/"test/support/fixtures"/file
  end
  ...
  # hash-checked and then DinrusHub changes into a temporary directory where the
  ...
  # defined by the formula, as only `DRXHUB_PREFIX` is available
  ...
  #   s.gsub! "/usr/local", DRXHUB_PREFIX.to_s
  # end
  # ```
  #
  ...
  def system(cmd, *args)
    verbose_using_dots = DinrusHub::EnvConfig.verbose_using_dots?
    ...
    File.open(logfn, "w") do |log|
      ...
      unless $CHILD_STATUS.success?
        log_lines = DinrusHub::EnvConfig.fail_log_lines
  ...
  # Runs `xcodebuild` without DinrusHub's compiler environment variables set.
  ...
  def common_stage_test_env
    {
      _JAVA_OPTIONS:           "-Duser.home=#{DRXHUB_CACHE}/java_cache",
      GOCACHE:                 "#{DRXHUB_CACHE}/go_cache",
      GOPATH:                  "#{DRXHUB_CACHE}/go_mod_cache",
      CARGO_HOME:              "#{DRXHUB_CACHE}/cargo_cache",
      PIP_CACHE_DIR:           "#{DRXHUB_CACHE}/pip_cache",
  ...
  class << self
    ...
    # The reason for why this software is not linked (by default) to {::DRXHUB_PREFIX}.
    attr_reader :keg_only_reason

    # A one-line description of the software. Used by users to get an overview
    # of the software and DinrusHub maintainers.
    # Shows when running `dhub info`.
    #
    # ### Example
    #
    # ```ruby
    ...
    #
    # @!attribute [w] license
    # @see https://docs.brew.sh/License-Guidelines DinrusHub License Guidelines
    ...
    def network_access_allowed?(phase)
      ...
      env_var = DinrusHub::EnvConfig.send(:"formula_#{phase}_network")
      env_var.nil? ? @network_access_allowed[phase] : env_var == "allow"
    end

    # The homepage for the software. Used by users to get more information
    # about the software and DinrusHub maintainers as a point of contact for
    # e.g. submitting patches.
    # Can be opened with running `dhub home`.
    #
    # ### Example
    #
    # ```ruby
    ...
    # Used for creating new DinrusHub version schemes. For example, if we want
    ...
    # this value you can leave it blank and run `dhub fetch --force` and it'll
    ...
    # This provides a pre-built binary package built by the DinrusHub maintainers for you.
    ...
    # You can ignore this block entirely if submitting to DinrusHub/homebrew-core.
    ...
    #   # Optionally specify the DRXHUB_CELLAR in which the bottles were built.
    #   sha256 cellar: "/dhub/Cellar", catalina:    "ef65c759c5097a36323fa9c77756468649e8d1980a3a4e05695c05e39568967c"
    ...
    # ```
    #
    # DinrusHub maintainers aim to bottle all formulae.
    #
    # @api public
    ...
    # formulae. DinrusHub provides some `:special` {Requirement}s for stuff
    ...
    # @!attribute [w] option
    # Options can be used as arguments to `dhub install`.
    ...
    # Software that will not be symlinked into the `dhub --prefix` and will
    # only live in its Cellar. Other formulae can depend on it and DinrusHub
    ...
    # if you build your own software outside of DinrusHub. This way, we
    ...
    # Marks the {Formula} as needing a certain standard, so DinrusHub
    ...
    # object accordingly. This is used by `dhub livecheck` to check for newer
    ...
    # object accordingly. This is used by `dhub install` to generate a service file.
    #
    # ### Example
    #
    # ```ruby
    ...
    # Defines whether the {Formula}'s bottle can be used on the given DinrusHub
    # installation.
    #
    # ### Examples
    #
    ...
    def pour_bottle?(only_if: nil, &block)
      ...
      when :default_prefix
        lambda do |_|
          T.bind(self, PourBottleCheck)
          reason(<<~EOS)
            The bottle (and many others) needs to be installed into #{DinrusHub::DEFAULT_PREFIX}.
          EOS
          satisfy { DRXHUB_PREFIX.to_s == DinrusHub::DEFAULT_PREFIX }
        end
      else
