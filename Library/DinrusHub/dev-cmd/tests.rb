...
module DinrusHub
  module DevCmd
    class Tests < AbstractCommand
      ...
      cmd_args do
        description <<~EOS
          Run DinrusHub's unit and integration tests.
        EOS
        switch "--coverage",
      ...
      def run
        ...
        DinrusHub.install_bundler_gems!(groups: DinrusHub.valid_gem_groups - ["sorbet"])

        DRXHUB_LIBRARY_PATH.cd do
          setup_environment!
          ...
          parallel_rspec_log_path = if ENV["CI"]
            ...
            "#{DRXHUB_CACHE}/#{parallel_rspec_log_name}"
          end
          ...
          bundle_args = ["-I", DRXHUB_LIBRARY_PATH/"test"]
          bundle_args += %W[
            --seed #{seed}
            --color
            --require spec_helper
          ...
          # rubocop:disable DinrusHub/MoveToExtendOS
          unless OS.mac?
          ...
          # rubocop:enable DinrusHub/MoveToExtendOS
          ...
          ENV["DRXHUB_DEBUG"] = "1" if args.debug? # Used in spec_helper.rb to require the "debug" gem.
      ...
      def changed_test_files
        ...
        filestub_regex = %r{Library/DinrusHub/([\w/-]+).rb}
      ...
      def setup_environment!
        ...
        allowed_test_env << "DRXHUB_USE_RUBY_FROM_PATH" if DinrusHub::EnvConfig.developer?
        DinrusHub::EnvConfig::ENVS.keys.map(&:to_s).each do |env|
          next if allowed_test_env.include?(env)

          ENV.delete(env)
        end

        # Codespaces DRXHUB_PREFIX and /tmp are mounted 755 which makes Ruby warn constantly.
        if (ENV["DRXHUB_CODESPACES"] == "true") && (DRXHUB_TEMP.to_s == "/tmp")
          # Need to keep this fairly short to avoid socket paths being too long in tests.
          homebrew_prefix_tmp = "/home/linuxbrew/tmp"
          ENV["DRXHUB_TEMP"] = homebrew_prefix_tmp
          FileUtils.mkdir_p homebrew_prefix_tmp
          system "chmod", "-R", "g-w,o-w", DRXHUB_PREFIX, homebrew_prefix_tmp
        end

        ENV["DRXHUB_TESTS"] = "1"
        ENV["DRXHUB_NO_AUTO_UPDATE"] = "1"
        ENV["DRXHUB_NO_ANALYTICS_THIS_RUN"] = "1"
        ENV["DRXHUB_TEST_GENERIC_OS"] = "1" if args.generic?
        ENV["DRXHUB_TEST_ONLINE"] = "1" if args.online?
        ENV["DRXHUB_SORBET_RUNTIME"] = "1"
        ENV["DRXHUB_NO_FORCE_BREW_WRAPPER"] = "1"

        # TODO: remove this and fix tests when possible.
        ENV["DRXHUB_NO_INSTALL_FROM_API"] = "1"
        ENV.delete("DRXHUB_INTERNAL_JSON_V3")

        ENV["USER"] ||= system_command!("id", args: ["-nu"]).stdout.chomp

        # Avoid local configuration messing with tests, e.g. git being configured
        # to use GPG to sign by default
        ENV["HOME"] = "#{DRXHUB_LIBRARY_PATH}/test"

        # Print verbose output when requesting debug or verbose output.
        ENV["DRXHUB_VERBOSE_TESTS"] = "1" if args.debug? || args.verbose?

        if args.coverage?
          ENV["DRXHUB_TESTS_COVERAGE"] = "1"
        ...
        %w[AUTHOR COMMITTER].each do |role|
          ENV["GIT_#{role}_NAME"] = "dhub tests"
