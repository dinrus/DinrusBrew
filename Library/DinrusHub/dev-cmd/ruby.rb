  ...
  module DevCmd
    class Ruby < AbstractCommand
      cmd_args do
        ...
        description <<~EOS
          Run a Ruby instance with DinrusHub's libraries loaded. For example,
          `dhub ruby -e "puts :gcc.f.deps"` or `dhub ruby script.rb`.

          Run e.g. `dhub ruby -- --version` to pass arbitrary arguments to `ruby`.
        EOS
        flag "-r=",
