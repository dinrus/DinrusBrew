...
module DinrusHub
  ...
  bottle do
    <% if [DRXHUB_BOTTLE_DEFAULT_DOMAIN.to_s,
           "#{DRXHUB_BOTTLE_DEFAULT_DOMAIN}/bottles"].exclude?(root_url) %>
    ...
    <% end %>
      ...
      ALLOWABLE_DRXHUB_REPOSITORY_LINKS = T.let([
        %r{#{Regexp.escape(DRXHUB_LIBRARY)}/Homebrew/os/(mac|linux)/pkgconfig},
      ...
      cmd_args do
        ...
        flag   "--root-url=",
               description: "Use the specified <URL> as the root of the bottle's URL instead of DinrusHub's default."
        flag   "--root-url-using=",
      ...
      def run
        if args.merge?
          DinrusHub.install_bundler_gems!(groups: ["ast"])
          return merge
        end

        DinrusHub.install_bundler_gems!(groups: ["bottle"])
      ...
      def cellar_parameter_needed?(cellar)
        default_cellars = [
          DinrusHub::DEFAULT_MACOS_CELLAR,
          DinrusHub::DEFAULT_MACOS_ARM_CELLAR,
          DinrusHub::DEFAULT_LINUX_CELLAR,
        ]
      ...
      def sudo_purge
        return unless ENV["DRXHUB_BOTTLE_SUDO_PURGE"]
      ...
      def formula_ignores(formula)
        ignores = []
        cellar_regex = Regexp.escape(DRXHUB_CELLAR)
        prefix_regex = Regexp.escape(DRXHUB_PREFIX)
      ...
      def bottle_formula(formula)
        ...
        prefix = DRXHUB_PREFIX.to_s
        cellar = DRXHUB_CELLAR.to_s
        ...
        keg&.lock do
          ...
          begin
            ...
            prefix_check = if DinrusHub.default_prefix?(prefix)
            ...
            repository_reference = if DRXHUB_PREFIX == DRXHUB_REPOSITORY
              DRXHUB_LIBRARY
            else
              DRXHUB_REPOSITORY
            end.to_s
            if keg_contain?(repository_reference, keg, ignores + ALLOWABLE_DRXHUB_REPOSITORY_LINKS)
            ...
            if args.skip_relocation?
              ...
              relocatable = false if keg_contain?(DRXHUB_LIBRARY.to_s, keg, ignores, formula_and_runtime_deps_names)
        ...
        json = {
          formula.full_name => {
            "formula" => {
              ...
              "path"             => formula.path.to_s.delete_prefix("#{DRXHUB_REPOSITORY}/"),
      ...
      def merge
        ...
        bottles_hash.each do |formula_name, bottle_hash|
          ...
          path = DRXHUB_REPOSITORY/bottle_hash["formula"]["path"]
