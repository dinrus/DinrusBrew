      ...
      def run
        ...
        if args.lost?
          if args.named.present?
            raise UsageError, "`dhub unbottled --lost` cannot be used with formula arguments!"
          elsif !CoreTap.instance.installed?
            raise UsageError, "`dhub unbottled --lost` requires `homebrew/core` to be tapped locally!"
          else
            output_lost_bottles
            return
          end
        ...
        DinrusHub::SimulateSystem.with(os:, arch:) do
          all = args.eval_all?
          if args.total?
            if !all && !DinrusHub::EnvConfig.eval_all?
              raise UsageError, "`dhub unbottled --total` needs `--eval-all` passed or `DRXHUB_EVAL_ALL` set!"
            end
      ...
      def formulae_all_installs_from_args(all)
        if args.named.present?
          ...
          if !args.eval_all? && !DinrusHub::EnvConfig.eval_all?
            raise UsageError,
                  "`dhub unbottled --dependents` needs `--eval-all` passed or `DRXHUB_EVAL_ALL` set!"
          end
          ...
          analytics = DinrusHub::API::Analytics.fetch "install", 90

          if analytics.blank?
            raise UsageError,
                  "default sort by analytics data requires " \
                  "`DRXHUB_NO_GITHUB_API` and `DRXHUB_NO_ANALYTICS` to be unset"
          end
