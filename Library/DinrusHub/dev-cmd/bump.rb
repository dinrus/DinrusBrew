    ...
    class Bump < AbstractCommand
      ...
      def run
        ...
        DinrusHub.with_no_api_env do
          eval_all = args.eval_all? || DinrusHub::EnvConfig.eval_all?
          ...
          formulae_and_casks = if args.auto?
            ...
            raise UsageError,
                  "`dhub bump` without named arguments needs `--installed` or `--eval-all` passed or " \
                  "`DRXHUB_EVAL_ALL` set!"
          end
          ...
          if args.repology? && !Utils::Curl.curl_supports_tls13?
            begin
              ensure_formula_installed!("curl", reason: "Repology queries") unless DRXHUB_BREWED_CURL_PATH.exist?
      ...
      def handle_formulae_and_casks(formulae_and_casks)
        ...
        formulae_and_casks.each_with_index do |formula_or_cask, i|
          ...
          repository = if formula_or_cask.is_a?(Formula)
            Repology::DRXHUB_CORE
          else
            Repology::DRXHUB_CASK
          end
      ...
      def retrieve_and_display_info_and_open_pr(formula_or_cask, name, repositories, ambiguous_cask: false)
        ...
        bump_cask_pr_args = [
          ...
          *version_args,
          "--no-browse",
          "--message=Created by `dhub bump`",
        ]

        bump_cask_pr_args << "--no-fork" if args.no_fork?

        system DRXHUB_BREW_FILE, *bump_cask_pr_args
      end
