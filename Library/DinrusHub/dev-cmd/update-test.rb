...
module DinrusHub
  module DevCmd
    class UpdateTest < AbstractCommand
      ...
      cmd_args do
        description <<~EOS
          Run a test of `dhub update` with a new repository clone.
          If no options are passed, use `origin/master` as the start commit.
        EOS
        switch "--to-tag",
               description: "Set `DRXHUB_UPDATE_TO_TAG` to test updating between tags."
        switch "--keep-tmp",
      ...
      def run
        ...
        ENV["DRXHUB_UPDATE_TEST"] = "1"

        # Avoid accidentally updating when we don't expect it.
        ENV["DRXHUB_NO_AUTO_UPDATE"] = "1"

        # Use default behaviours
        ENV["DRXHUB_AUTO_UPDATE_SECS"] = nil
        ENV["DRXHUB_DEVELOPER"] = nil
        ENV["DRXHUB_DEV_CMD_RUN"] = nil
        ENV["DRXHUB_MERGE"] = nil
        ENV["DRXHUB_NO_UPDATE_CLEANUP"] = nil
        ENV["DRXHUB_UPDATE_TO_TAG"] = nil

        branch = if args.to_tag?
          ENV["DRXHUB_UPDATE_TO_TAG"] = "1"
          "stable"
        else
          ENV["DRXHUB_DEV_CMD_RUN"] = "1"
          "master"
        end
        ...
        chdir "update-test" do
          ...
          # copy DinrusHub installation
          safe_system "git", "clone", "#{DRXHUB_REPOSITORY}/.git", ".",
                      "--branch", "master", "--single-branch"

          # set git origin to another copy
          safe_system "git", "clone", "#{DRXHUB_REPOSITORY}/.git", "remote.git",
                      "--bare", "--branch", "master", "--single-branch"
          safe_system "git", "config", "remote.origin.url", "#{curdir}/remote.git"
          ENV["DRXHUB_BREW_GIT_REMOTE"] = "#{curdir}/remote.git"
          ...
          # Run `dhub help` to install `portable-ruby` (if needed).
          quiet_system "dhub", "help"

          # run dhub update
          oh1 "Running `dhub update`..."
          safe_system "dhub", "update", "--verbose", "--debug"
          ...
          if actual_end_commit != end_commit
            ...
            odie <<~EOS
              `dhub update` didn't update #{branch}!
      ...
      def git_tags
        ...
        if tags.blank?
          tags = if (DRXHUB_REPOSITORY/".git/shallow").exist?
