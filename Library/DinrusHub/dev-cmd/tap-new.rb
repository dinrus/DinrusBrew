  ...
  module DevCmd
    class TapNew < AbstractCommand
      ...
      def run
        ...
        odie "Invalid tap name '#{tap}'" unless tap.path.to_s.match?(DRXHUB_TAP_PATH_REGEX)
        ...
        readme = <<~MARKDOWN
          ...
          `dhub install #{tap}/<formula>`

          Or `dhub tap #{tap}` and then `dhub install <formula>`.

          Or, in a [`dhub bundle`](https://github.com/Homebrew/homebrew-bundle) `Brewfile`:
          ...
          `dhub help`, `man dhub` or check [DinrusHub's documentation](https://docs.brew.sh).
        MARKDOWN
        # <!-- vale on -->
        ...
        actions_main = <<~YAML
          ...
          jobs:
            test-bot:
              ...
              runs-on: ${{ matrix.os }}
              steps:
                - name: Set up DinrusHub
                  id: set-up-homebrew
                  uses: DinrusHub/actions/setup-homebrew@master

                - name: Cache DinrusHub Bundler RubyGems
                ...
                - run: dhub test-bot --only-cleanup-before

                - run: dhub test-bot --only-setup

                - run: dhub test-bot --only-tap-syntax

                - run: dhub test-bot --only-formulae#{" --root-url='#{root_url}'" if root_url}
        ...
        pr_pull_env = {
          "DRXHUB_GITHUB_API_TOKEN" => "${{ github.token }}",
        }
        if args.github_packages?
          pr_pull_permissions["packages"] = "write"
          pr_pull_env["DRXHUB_GITHUB_PACKAGES_TOKEN"] = "${{ github.token }}"
          pr_pull_env["DRXHUB_GITHUB_PACKAGES_USER"] = "${{ github.repository_owner }}"
        end
        actions_publish = <<~YAML
          ...
          #{pr_pull_permissions.sort.map { |k, v| "      #{k}: #{v}" }.join("\n")}
              steps:
                ...
                - name: Set up git
                  uses: DinrusHub/actions/git-user-config@master

                - name: Pull bottles
                  env:
          #{pr_pull_env.sort.map { |k, v| "          #{k}: #{v}" }.join("\n")}
                    PULL_REQUEST: ${{ github.event.pull_request.number }}
                  run: dhub pr-pull --debug --tap="$GITHUB_REPOSITORY" "$PULL_REQUEST"

                - name: Push commits
                  uses: DinrusHub/actions/git-try-push@master
                  with:
                    token: ${{ github.token }}
                    branch: #{branch}
        ...
        unless args.no_git?
          cd tap.path do |path|
            ...
            if git_owner != Process.uid && git_owner == Process.euid
              # Under DinrusHub user model, EUID is permitted to execute commands under the UID.
