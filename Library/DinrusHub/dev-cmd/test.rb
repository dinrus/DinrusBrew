    ...
    class Test < AbstractCommand
      cmd_args do
        description <<~EOS
          ...
          *Example:* `dhub install jruby && dhub test jruby`
        EOS
        switch "-f", "--force",
      ...
      sig { override.void }
      def run
        DinrusHub.install_bundler_gems!(groups: ["formula_test"], setup_path: false)
        ...
        args.named.to_resolved_formulae.each do |f|
          ...
          begin
            exec_args = DRXHUB_RUBY_EXEC_ARGS + %W[
              --
              #{DRXHUB_LIBRARY_PATH}/test.rb
              #{f.path}
            ...
            if Sandbox.available?
              ...
              sandbox.allow_write_path(DRXHUB_PREFIX/"var/cache")
              sandbox.allow_write_path(DRXHUB_PREFIX/"var/homebrew/locks")
              sandbox.allow_write_path(DRXHUB_PREFIX/"var/log")
              sandbox.allow_write_path(DRXHUB_PREFIX/"var/run")
