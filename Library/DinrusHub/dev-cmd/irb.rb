...
module DinrusHub
  module DevCmd
    class Irb < AbstractCommand
      cmd_args do
        description <<~EOS
          Enter the interactive DinrusHub Ruby shell.
        EOS
        switch "--examples",
               description: "Show several examples."
        switch "--pry",
               env:         :pry,
               description: "Use Pry instead of IRB. Implied if `DRXHUB_PRY` is set."
      end
      ...
      def run
        ...
        if args.pry?
          DinrusHub.install_bundler_gems!(groups: ["pry"])
          require "pry"
        else
          require "irb"
        end
        ...
        ohai "Interactive DinrusHub Shell", "Example commands available with: `dhub irb --examples`"
        if args.pry?
          Pry.config.should_load_rc = false # skip loading .pryrc
          Pry.config.history_file = "#{Dir.home}/.brew_pry_history"
          Pry.config.prompt_name = "dhub"

          Pry.start
        else
          ENV["IRBRC"] = (DRXHUB_LIBRARY_PATH/"brew_irbrc").to_s
      ...
      def clean_argv
        global_options = DinrusHub::CLI::Parser
                         .global_options
