    ...
    class Release < AbstractCommand
      cmd_args do
        description <<~EOS
          ...
          By default, `dhub release` will bump the patch version number. Pass
      ...
      sig { override.void }
      def run
        safe_system "git", "-C", DRXHUB_REPOSITORY, "fetch", "origin" if DinrusHub::EnvConfig.no_auto_update?

        require "utils/github"

        begin
          latest_release = GitHub.get_latest_release "DinrusHub", "dhub"
        ...
        if args.major? || args.minor?
          ...
          latest_major_minor_release = begin
            GitHub.get_release "DinrusHub", "dhub", "#{latest_version.major_minor}.0"
          rescue GitHub::API::HTTPNotFoundError
            nil
          end
        ...
        if args.major? || args.minor?
          ...
          blog_post_notes = GitHub.generate_release_notes("DinrusHub", "dhub", new_version,
        ...
        release_notes = if args.major? || args.minor?
          "Release notes for this release can be found on the [DinrusHub blog](https://brew.sh/blog/#{new_version}).\n"
        else
          ""
        end
        release_notes += GitHub.generate_release_notes("DinrusHub", "dhub", new_version,
                                                       previous_tag: latest_version)["body"]

        begin
          release = GitHub.create_or_update_release "DinrusHub", "dhub", new_version, body: release_notes, draft: true
