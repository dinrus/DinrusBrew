  ...
  module DevCmd
    class BumpFormulaPr < AbstractCommand
      cmd_args do
        ...
        switch "--commit",
               ...
               description: "When passed with `--write-only`, generate a new commit after writing changes " \
                            "to the formula file."
        switch "--no-audit",
               description: "Don't run `dhub audit` before opening the PR."
        switch "--strict",
               description: "Run `dhub audit --strict` before opening the PR."
        switch "--online",
               description: "Run `dhub audit --online` before opening the PR."
        switch "--no-browse",
      ...
      def run
        ...
        ENV["BROWSER"] = DinrusHub::EnvConfig.browser
        ...
        # This will be run by `dhub audit` later so run it first to not start
        # spamming during normal output.
        DinrusHub.install_bundler_gems!(groups: ["audit", "style"]) unless args.no_audit?
        ...
        pr_message = "Created with `dhub bump-formula-pr`."
      ...
      def run_audit(formula, alias_rename, old_contents)
        ...
        if args.dry_run?
          if args.no_audit?
            ohai "Skipping `dhub audit`"
          elsif audit_args.present?
            ohai "dhub audit #{audit_args.join(" ")} #{formula.path.basename}"
          else
            ohai "dhub audit #{formula.path.basename}"
          end
          return
        end
        ...
        if args.no_audit?
          ...
          system DRXHUB_BREW_FILE, "audit", *audit_args, formula.full_name
          failed_audit = !$CHILD_STATUS.success?
        else
          system DRXHUB_BREW_FILE, "audit", formula.full_name
          failed_audit = !$CHILD_STATUS.success?
        end
