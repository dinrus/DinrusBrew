  ...
  module DevCmd
    class PrUpload < AbstractCommand
      cmd_args do
        ...
        switch "--upload-only",
               description: "Skip running `dhub bottle` before uploading."
        flag   "--committer=",
      ...
      def run
        ...
        DinrusHub.install_bundler_gems!(groups: ["pr_upload"])
        ...
        unless args.upload_only?
          ...
          safe_system DRXHUB_BREW_FILE, *bottle_args
          ...
          # Reload the JSON files (in case `dhub bottle --merge` generated
          # `all: $SHA256` bottles)
          bottles_hash = bottles_hash_from_json_files(json_files, args)

          # Check the bottle commits did not break `dhub audit`
          unless args.no_commit?
            ...
            safe_system DRXHUB_BREW_FILE, *audit_args
          end
        end
      ...
      def check_bottled_formulae!(bottles_hash)
        bottles_hash.each do |name, bottle_hash|
          formula_path = DRXHUB_REPOSITORY/bottle_hash["formula"]["path"]
