...
module DinrusHub
  module DevCmd
    class StyleCmd < AbstractCommand
      cmd_args do
        description <<~EOS
          Check formulae or files for conformance to DinrusHub style guidelines.

          Lists of <file>, <tap> and <formula> may not be combined. If none are
          provided, `style` will run style checks on the whole DinrusHub library,
      ...
      sig { override.void }
      def run
        DinrusHub.install_bundler_gems!(groups: ["style"])
        ...
        DinrusHub.failed = !Style.check_style_and_print(target, **options)
      end
    end
  end
end
