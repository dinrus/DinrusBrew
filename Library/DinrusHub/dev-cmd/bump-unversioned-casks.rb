...
module DinrusHub
  module DevCmd
    class BumpUnversionedCasks < AbstractCommand
      ...
      sig { override.void }
      def run
        DinrusHub.install_bundler_gems!(groups: ["bump_unversioned_casks"])

        state_file = if args.state_file.present?
          Pathname(T.must(args.state_file)).expand_path
        else
          DRXHUB_CACHE/"bump_unversioned_casks.json"
        end
            ...
            if version
              if cask.version == version
                ...
                bump_cask_pr_args = [
                  ...
                  "--sha256", ":no_check",
                  "--message", "Automatic update via `dhub bump-unversioned-casks`.",
                ...
                begin
                  system_command! DRXHUB_BREW_FILE, args: bump_cask_pr_args
