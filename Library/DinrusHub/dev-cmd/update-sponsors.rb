  ...
  module DevCmd
    class UpdateSponsors < AbstractCommand
      ...
      def run
        ...
        GitHub.sponsorships("DinrusHub").each do |s|
        ...
        named_sponsors << "many other users and organisations via [GitHub Sponsors](https://github.com/sponsors/DinrusHub)"

        readme = DRXHUB_REPOSITORY/"README.md"
        content = readme.read
        content.gsub!(/(DinrusHub is generously supported by) .*\Z/m, "\\1 #{named_sponsors.to_sentence}.\n")
        content << "\n#{logo_sponsors.join}\n" if logo_sponsors.presence

        File.write(readme, content)

        diff = system_command "git", args: [
          "-C", DRXHUB_REPOSITORY, "diff", "--exit-code", "README.md"
        ]
