    ...
    class Create < AbstractCommand
      cmd_args do
        description <<~EOS
          ...
          and open it in the editor. DinrusHub will attempt to automatically derive the
        ...
        switch "--no-fetch",
               description: "DinrusHub will not download <URL> to the cache and will thus not add its SHA-256 " \
      ...
      def create_cask
        ...
        puts "Please run `dhub audit --cask --new #{token}` before submitting, thanks."
        cask_path
      end
      ...
      def create_formula
        ...
        formula = DinrusHub.with_no_api_env do
          Formula[fc.name]
        end
        ...
        puts <<~EOS
          Please run the following command before submitting:
            DRXHUB_NO_INSTALL_FROM_API=1 dhub audit --new #{fc.name}
        EOS
        path
      end
