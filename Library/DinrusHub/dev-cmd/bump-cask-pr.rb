  ...
  module DevCmd
    class BumpCaskPr < AbstractCommand
      cmd_args do
        ...
        switch "--no-style",
               description: "Don't run `dhub style --fix` before opening the PR."
        switch "--no-browse",
      ...
      sig { override.void }
      def run
        # This will be run by `dhub audit` or `dhub style` later so run it first to
        ...
        DinrusHub.install_bundler_gems!(groups: gem_groups) unless gem_groups.empty?
        ...
        pr_info = {
          ...
          commit_message:,
          old_contents:,
          pr_message:      "Created with `dhub bump-cask-pr`.",
      ...
      def run_cask_audit(cask, old_contents)
        if args.dry_run?
          if args.no_audit?
            ...
            ohai "dhub audit --cask --online #{cask.full_name}"
          end
          return
        end
        failed_audit = false
        if args.no_audit?
          ...
          system DRXHUB_BREW_FILE, "audit", "--cask", "--online", cask.full_name
      ...
      def run_cask_style(cask, old_contents)
        if args.dry_run?
          if args.no_style?
            ohai "Skipping `dhub style --fix`"
          else
            ohai "dhub style --fix #{cask.sourcefile_path.basename}"
          end
          return
        end
        failed_style = false
        if args.no_style?
          ...
          system DRXHUB_BREW_FILE, "style", "--fix", cask.sourcefile_path
          failed_style = !$CHILD_STATUS.success?
        end
        return unless failed_style

        cask.sourcefile_path.atomic_write(old_contents)
        odie "`dhub style --fix` failed!"
      end
    end
  end
end
