    ...
    class DetermineTestRunners < AbstractCommand
      cmd_args do
        usage_banner <<~EOS
          ...
          Determines the runners used to test formulae or their dependents. For internal use in DinrusHub taps.
        EOS
        ...
        switch "--eval-all",
               ...
               env:         :eval_all
        switch "--dependents",
               description: "Determine runners for testing dependents. Requires `--eval-all` or `DRXHUB_EVAL_ALL`.",
               depends_on:  "--eval-all"
      ...
      def run
        if args.no_named? && !args.all_supported?
          raise DinrusHub::CLI::MinNamedArgumentsError, 1
