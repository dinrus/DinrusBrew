...
module DinrusHub
  module DevCmd
    class Audit < AbstractCommand
      cmd_args do
        description <<~EOS
          Check <formula> for DinrusHub coding style violations. This should be run before
        ...
        switch "--eval-all",
               description: "Evaluate all available formulae and casks, whether installed or not, to audit them. " \
                            "Implied if `DRXHUB_EVAL_ALL` is set."
        switch "--new",
               description: "Run various additional style checks to determine if a new formula or cask is eligible " \
                            "for DinrusHub. This should be used when creating new formulae or casks and implies " \
                            "`--strict` and `--online`."
        switch "--new-formula",
               replacement: "--new",
               disable:     true,
        ...
        switch "--skip-style",
               description: "Skip running non-RuboCop style checks. Useful if you plan on running " \
                            "`dhub style` separately. Enabled by default unless a formula is specified by name."
      ...
      def run
        ...
        DinrusHub.auditing = true
        DinrusHub.inject_dump_stats!(FormulaAuditor, /^audit_/) if args.audit_debug?
        ...
        DinrusHub.install_bundler_gems!(groups: gem_groups)

        ENV.activate_extensions!
        ENV.setup_build_environment

        audit_formulae, audit_casks = DinrusHub.with_no_api_env do # audit requires full Ruby source
          if args.tap
            ...
            if !args.eval_all? && !DinrusHub::EnvConfig.eval_all?
              # This odisabled should probably stick around indefinitely.
              odisabled "dhub audit",
                        "dhub audit --eval-all or DRXHUB_EVAL_ALL"
            end
            no_named_args = true
            [
        ...
        formula_problems = audit_formulae.sort.each_with_object({}) do |f, problems|
          ...
          errors = os_arch_combinations.flat_map do |os, arch|
            SimulateSystem.with(os:, arch:) do
              ...
              # don't unnecessarily require a full DinrusHub/core clone.
              fa = if f.core_formula?
                DinrusHub.with_no_api_env(&audit_proc)
              else
                audit_proc.call
              end
