...
module DinrusHub
  module DevCmd
    class PrAutomerge < AbstractCommand
      cmd_args do
        description <<~EOS
          Find pull requests that can be automatically merged using `dhub pr-publish`.
        EOS
        flag   "--tap=",
               description: "Target tap repository (default: `homebrew/core`)."
        flag   "--workflow=",
               description: "Workflow file to use with `dhub pr-publish`."
        flag   "--with-label=",
        ...
        switch "--publish",
               description: "Run `dhub pr-publish` on matching pull requests."
        switch "--autosquash",
               description: "Instruct `dhub pr-publish` to automatically reformat and reword commits " \
      ...
      def run
        ...
        if args.publish?
          safe_system DRXHUB_BREW_FILE, *publish_args, *pr_urls
        else
          ohai "Now run:", "  dhub #{publish_args.join " "} \\\n    #{pr_urls.join " \\\n    "}"
        end
      end
    end
  end
