    ...
    class LivecheckCmd < AbstractCommand
      cmd_args do
        description <<~EOS
          ...
          casks to check is taken from `DRXHUB_LIVECHECK_WATCHLIST` or
      ...
      sig { override.void }
      def run
        DinrusHub.install_bundler_gems!(groups: ["livecheck"])

        all = args.eval_all?

        if args.debug? && args.verbose?
          puts args
          puts DinrusHub::EnvConfig.livecheck_watchlist if DinrusHub::EnvConfig.livecheck_watchlist.present?
        end

        formulae_and_casks_to_check = DinrusHub.with_no_api_env do
          if args.tap
      ...
      sig { returns(String) }
      def watchlist_path
        @watchlist_path ||= T.let(File.expand_path(DinrusHub::EnvConfig.livecheck_watchlist), T.nilable(String))
      end

      sig { returns(T::Boolean) }
      def skip_autobump?
        !(args.autobump? || DinrusHub::EnvConfig.livecheck_autobump?)
      end
    end
  end
end
