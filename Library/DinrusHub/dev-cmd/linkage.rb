  ...
  module DevCmd
    class Linkage < AbstractCommand
      cmd_args do
        ...
        switch "--cached",
               description: "Print the cached linkage values stored in `DRXHUB_CACHE`, set by a previous " \
                            "`dhub linkage` run."
        ...
        CacheStoreDatabase.use(:linkage) do |db|
          ...
          kegs.each do |keg|
            ...
            if args.test?
              ...
              DinrusHub.failed = true if result.broken_library_linkage?(test: true, strict: args.strict?)
            elsif args.reverse?
