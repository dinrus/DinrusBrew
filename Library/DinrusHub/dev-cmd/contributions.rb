...
module DinrusHub
  module DevCmd
    class Contributions < AbstractCommand
      PRIMARY_REPOS = T.let(%w[dhub core cask].freeze, T::Array[String])
      ...
      cmd_args do
        ...
        description <<~EOS
          Summarise contributions to DinrusHub repositories.
        EOS
        ...
        comma_array "--repositories",
                    description: "Specify a comma-separated list of repositories to search. " \
                                 ...
                                 "main repositories: dhub,core,cask. " \
      ...
      def run
        ...
        users = args.user.presence || GitHub.members_by_team("DinrusHub", "maintainers").keys
      ...
      def find_repo_path_for_repo(repo)
        return DRXHUB_REPOSITORY if repo == "dhub"
      ...
      def scan_repositories(repos, person, from:)
        ...
        repos.each do |repo|
          ...
          unless repo_path.exist?
            opoo "Репозиторий #{repo} ещё не таппирован! Сейчас сделаем это..."
            opoo "Репозиторий #{repo} ещё не таппирован! Сейчас сделаем это..."
            tap.install
          end

          repo_full_name = if repo == "dhub"
            "homebrew/brew"
          else
            tap.full_name
          end
