...
module DinrusHub
  module DevCmd
    class VendorGems < AbstractCommand
      cmd_args do
        description <<~EOS
          Install and commit DinrusHub's vendored gems.
        EOS
      ...
      sig { override.void }
      def run
        DinrusHub.install_bundler!

        ENV["BUNDLE_WITH"] = DinrusHub.valid_gem_groups.join(":")

        ohai "cd #{DRXHUB_LIBRARY_PATH}"
        DRXHUB_LIBRARY_PATH.cd do
          ...
          if args.non_bundler_gems?
            %w[
              ...
              (DRXHUB_LIBRARY_PATH/"vendor/gems").cd do
          ...
          unless args.no_commit?
            ...
            ohai "git commit"
            system "git", "commit", "--message", "dhub vendor-gems: commit updates."
          end
        end
      end
