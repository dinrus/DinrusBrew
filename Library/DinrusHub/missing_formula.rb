      ...
      def disallowed_reason(name)
        ...
        when "play" then <<~EOS
          Play 2.3 заменяет команду play c активатором:
            dhub install typesafe-activator
        ...
        when "haskell-platform" then <<~EOS
          ...
          Система построения Cabal:
            dhub install cabal-install

          Haskell Stack tool:
            dhub install haskell-stack
        EOS
        ...
        Tap.each do |old_tap|
          ...
          message += <<~EOS
            ...
            And then you can install it by running:
              dhub #{install_cmd} #{new_tap_new_name}
          EOS
          break
        end
        ...
        tap.path.cd do
            ...
            if (tap.path/".git/shallow").exist?
              opoo <<~EOS
                #{tap} is shallow clone. To get its complete history, run:
                  git -C "$(dhub --repo #{tap})" fetch --unshallow
          ...
          <<~EOS
            ...
            To show the formula before removal, run:
              git -C "$(dhub --repo #{tap})" show #{short_hash}^:#{relative_path}
