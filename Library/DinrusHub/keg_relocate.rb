...
class Keg
  PREFIX_PLACEHOLDER = "@@DRXHUB_PREFIX@@"
  CELLAR_PLACEHOLDER = "@@DRXHUB_CELLAR@@"
  REPOSITORY_PLACEHOLDER = "@@DRXHUB_REPOSITORY@@"
  LIBRARY_PLACEHOLDER = "@@DRXHUB_LIBRARY@@"
  PERL_PLACEHOLDER = "@@DRXHUB_PERL@@"
  JAVA_PLACEHOLDER = "@@DRXHUB_JAVA@@"
  NULL_BYTE = "\x00"
  ...
  def fix_dynamic_linkage
    symlink_files.each do |file|
      ...
      link_starts_cellar = link.to_s.start_with?(DRXHUB_CELLAR.to_s)
      link_starts_prefix = link.to_s.start_with?(DRXHUB_PREFIX.to_s)
  ...
  JAVA_REGEX = %r{#{DRXHUB_PREFIX}/opt/openjdk(@\d+(\.\d+)*)?/libexec(/openjdk\.jdk/Contents/Home)?}

  def prepare_relocation_to_placeholders
    relocation = Relocation.new
    relocation.add_replacement_pair(:prefix, DRXHUB_PREFIX.to_s, PREFIX_PLACEHOLDER, path: true)
    relocation.add_replacement_pair(:cellar, DRXHUB_CELLAR.to_s, CELLAR_PLACEHOLDER, path: true)
    # when DRXHUB_PREFIX == DRXHUB_REPOSITORY we should use DRXHUB_PREFIX for all relocations to avoid
    # being unable to differentiate between them.
    if DRXHUB_PREFIX != DRXHUB_REPOSITORY
      relocation.add_replacement_pair(:repository, DRXHUB_REPOSITORY.to_s, REPOSITORY_PLACEHOLDER, path: true)
    end
    relocation.add_replacement_pair(:library, DRXHUB_LIBRARY.to_s, LIBRARY_PLACEHOLDER, path: true)
    relocation.add_replacement_pair(:perl,
                                    %r{\A#![ \t]*(?:/usr/bin/perl\d\.\d+|#{DRXHUB_PREFIX}/opt/perl/bin/perl)( |$)}o,
                                    "#!#{PERL_PLACEHOLDER}\\1")
  ...
  def prepare_relocation_to_locations
    relocation = Relocation.new
    relocation.add_replacement_pair(:prefix, PREFIX_PLACEHOLDER, DRXHUB_PREFIX.to_s)
    relocation.add_replacement_pair(:cellar, CELLAR_PLACEHOLDER, DRXHUB_CELLAR.to_s)
    relocation.add_replacement_pair(:repository, REPOSITORY_PLACEHOLDER, DRXHUB_REPOSITORY.to_s)
    relocation.add_replacement_pair(:library, LIBRARY_PLACEHOLDER, DRXHUB_LIBRARY.to_s)
    relocation.add_replacement_pair(:perl, PERL_PLACEHOLDER, "#{DRXHUB_PREFIX}/opt/perl/bin/perl")
    if (openjdk = openjdk_dep_name_if_applicable)
      relocation.add_replacement_pair(:java, JAVA_PLACEHOLDER, "#{DRXHUB_PREFIX}/opt/#{openjdk}/libexec")
    end
  ...
  def text_files
    ...
    with_custom_locale("C") do
      files = Set.new path.find.reject { |pn|
        ...
        next true if pn == self/".dhub/#{name}.rb"
