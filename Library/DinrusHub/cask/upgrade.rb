  ...
  class Upgrade
    ...
    def self.upgrade_casks(
      *casks,
      ...
      greedy = true if DinrusHub::EnvConfig.upgrade_greedy?
        ...
        unless c.installed?
          odie <<~EOS
            The cask '#{c.token}' was affected by a bug and cannot be upgraded as-is. To fix this, run:
              dhub reinstall --cask --force #{c.token}
          EOS
        end
    ...
    def self.upgrade_cask(
      ...
      end_time = Time.now
      DinrusHub.messages.package_installed(new_cask.token, end_time - start_time)
    end
  end
end
