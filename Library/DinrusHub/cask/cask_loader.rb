  ...
  module CaskLoader
    ...
    class FromPathLoader < AbstractContentLoader
      ...
      def self.try_new(ref, warn: false)
        ...
        return if DinrusHub::EnvConfig.forbid_packages_from_paths? &&
                  !path.realpath.to_s.start_with?("#{Caskroom.path}/", "#{DRXHUB_LIBRARY}/Taps/")
      ...
      def initialize(path, token: T.unsafe(nil))
        ...
        @path = path
        @tap = Tap.from_path(path) || DinrusHub::API.tap_from_source_download(path)
      end

      sig { override.params(config: T.nilable(Config)).returns(Cask) }
      def load(config:)
        raise CaskUnavailableError.new(token, "'#{path}' не существует.")  unless path.exist?
        raise CaskUnavailableError.new(token, "'#{path}' нечитаем.") unless path.readable?
        raise CaskUnavailableError.new(token, "'#{path}' не является файлом.")   unless path.file?
        ...
        begin
          instance_eval(content, path).tap do |cask|
            raise CaskUnreadableError.new(token, "'#{path}' не содержит каск.") unless cask.is_a?(Cask)
          end
    ...
    class FromURILoader < FromPathLoader
      ...
      def load(config:)
        ...
        begin
          ...
          raise CaskUnavailableError.new(token, "Не удалось загрузить #{Formatter.url(url)}.")
        end
    ...
    class FromAPILoader
      ...
      def self.try_new(ref, warn: false)
        ...
        return unless (token = ref[DRXHUB_DEFAULT_TAP_CASK_REGEX, :token])
        if !DinrusHub::API::Cask.all_casks.key?(token) &&
           !DinrusHub::API::Cask.all_renames.key?(token)
          return
        end
      ...
      def initialize(token, from_json: T.unsafe(nil), path: nil)
        ...
        @sourcefile_path = path || DinrusHub::API::Cask.cached_json_file_path
        @path = path || CaskLoader.default_path(@token)
        @from_json = from_json
      end

      def load(config:)
        json_cask = from_json || DinrusHub::API::Cask.all_casks.fetch(token)
        ...
        json_cask = DinrusHub::API.merge_variations(json_cask).deep_symbolize_keys.freeze
      ...
      def from_h_string_gsubs(string, appdir)
        string.to_s
              ...
              .gsub(DRXHUB_PREFIX_PLACEHOLDER, DRXHUB_PREFIX)
              .gsub(DRXHUB_CELLAR_PLACEHOLDER, DRXHUB_CELLAR)
              .gsub(DRXHUB_CASK_APPDIR_PLACEHOLDER, appdir)
      end
    ...
    class FromNameLoader < FromTapLoader
      ...
      def self.try_new(ref, warn: false)
        return unless ref.is_a?(String)
        return unless ref.match?(/\A#{DRXHUB_TAP_CASK_TOKEN_REGEX}\Z/o)
    ...
    class NullLoader < FromPathLoader
      ...
      def load(config:)
        raise CaskUnavailableError.new(token, "Каск с таким именем не существует.")
      end
    end
    ...
    def self.tap_cask_token_type(tapped_token, warn:)
      ...
      opoo "Каск #{old_token} переименован в #{new_token}." if warn && old_token && new_token
    ...
    def self.for(ref, need_path: false, warn: true)
      [
        ...
        if (loader = loader_class.try_new(ref, warn:))
          $stderr.puts "#{$PROGRAM_NAME} (#{loader.class}): загружается #{ref}" if debug?
          return loader
        end
      end
    end
