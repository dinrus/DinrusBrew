...
module Cask
  ...
  class Config
    ...
    sig { params(args: DinrusHub::CLI::Args).returns(T.attached_class) }
    ...
    def self.canonicalize(config)
      config.to_h do |k, v|
        ...
        if DEFAULT_DIRS.key?(key)
          raise TypeError, "Неполноценную путь для дефолтной директории #{k}: #{v.inspect}" if v.is_a?(Array)
    ...
    def binarydir
      @binarydir ||= T.let(DRXHUB_PREFIX/"bin", T.nilable(Pathname))
    end

    sig { returns(Pathname) }
    def manpagedir
      @manpagedir ||= T.let(DRXHUB_PREFIX/"share/man", T.nilable(Pathname))
    end
