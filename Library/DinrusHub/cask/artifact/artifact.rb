  ...
  module Artifact
    ...
    class Artifact < Moved
      ...
      def self.from_args(cask, *args)
        ...
        raise CaskInvalidError.new(cask.token, "Нет исходников для #{english_name}.") if source.blank?

        unless options&.key?(:target)
          raise CaskInvalidError.new(cask.token, "#{english_name} '#{source}' требуется цель.")
        end
