  ...
  module Artifact
    ...
    class Pkg < AbstractArtifact
      ...
      def run_installer(command: nil, verbose: false, **_options)
        ohai "Выполняется установщик для #{cask} с sudo; может потребоваться пароль."
        unless path.exist?
          ...
          message = "Не найден исходный файл PKG '#{pkg}'"
          message += ", вместо него найден  #{pkgs.map { |path| "'#{path}'" }.to_sentence}" if pkgs.any?
          message += "."
