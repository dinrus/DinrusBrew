  ...
  module Artifact
    ...
    class AbstractUninstall < AbstractArtifact
      ...
      def uninstall_kext(*kexts, command: nil, **_)
        kexts.each do |kext|
          ohai "Выгружается расширение ядра #{kext}"
          ...
          found_kexts.each do |kext_path|
            ohai "Удаляется расширение ядра #{kext_path}"
            system_command!(
              "/bin/rm",
              args:         ["-rf", kext_path],
              sudo:         true,
      ...
      def trash_paths(*paths, command: nil, **_)
        ...
        stdout, = system_command DRXHUB_LIBRARY_PATH/"cask/utils/trash.swift",
                                 args:         paths,
                                 print_stderr: DinrusHub::EnvConfig.developer?
        ...
        trashed_with_permissions, untrashable = untrashable.partition do |path|
          Utils.gain_permissions(path, ["-R"], SystemCommand) do
            system_command! DRXHUB_LIBRARY_PATH/"cask/utils/trash.swift",
                            args:         [path],
                            print_stderr: DinrusHub::EnvConfig.developer?
          end
