      ...
      def move(adopt: false, auto_updates: false, force: false, verbose: false, predecessor: nil, reinstall: false,
        ...
        if Utils.path_occupied?(target)
          if target.directory? && target.children.empty? && matching_artifact?(predecessor)
            ...
            if adopt
              ...
              unless auto_updates
                ...
                same = if source_plist.size? &&
                          (source_bundle_version = DinrusHub::BundleVersion.from_info_plist(source_plist)) &&
                          target_plist.size? &&
                          (target_bundle_version = DinrusHub::BundleVersion.from_info_plist(target_plist))
                ...
                unless same
                  raise CaskError,
                        "Существующий #{self.class.english_name}, кажется, отличается " \
                        "от устанавливаемого."
                end
              end
            ...
            message = "Кажется, уже есть #{self.class.english_article} " \
                      "#{self.class.english_name} у '#{target}'"
            raise CaskError, "#{message}." if !force && !adopt

            opoo "#{message}; переписывается."
            delete(target, force:, command:, **options)
          end
        end

        ohai "Перемещается #{self.class.english_name} '#{source.basename}' в '#{target}'"
        ...
        if target.directory? && Quarantine.app_management_permissions_granted?(app: target, command:)
          ...
          # default sudo user isn't necessarily able to write to DinrusHub's locations
