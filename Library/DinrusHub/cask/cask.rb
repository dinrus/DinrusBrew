  ...
  class Cask
    ...
    def self.all(eval_all: false)
      if !eval_all && !DinrusHub::EnvConfig.eval_all?
        raise ArgumentError, "Cask::Cask#all не может использоваться без `--eval-all` или DRXHUB_EVAL_ALL"
      end
    ...
    def to_hash_with_variations(hash_method: :to_h)
      ...
      when :to_h
        if loaded_from_api? && !DinrusHub::EnvConfig.no_install_from_api?
          return api_to_local_hash(DinrusHub::API::Cask.all_casks[token].dup)
        end
    ...
    sig { returns(T.nilable(DinrusHub::BundleVersion)) }
    def bundle_version
      @bundle_version ||= if (bundle = artifacts.find { |a| a.is_a?(Artifact::App) }&.target) &&
        ...
        DinrusHub::BundleVersion.from_info_plist(plist)
      end
    end
