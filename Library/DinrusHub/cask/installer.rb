...
module Cask
  ...
  class Installer
    ...
    def self.caveats(cask)
      ...
      DinrusHub.messages.record_caveats(cask.token, caveats)
...
on_request: true)
      ...
      puts summary
      end_time = Time.now
      DinrusHub.messages.package_installed(@cask.token, end_time - start_time)
    rescue
      restore_backup
      raise
    end
    ...
    def verify_has_sha
      ...
      raise CaskError, <<~EOS
        ...
        This means you have the #{Formatter.identifier("--require-sha")} option set, perhaps in your DRXHUB_CASK_OPTS.
      EOS
    end
    ...
    def extract_primary_container(to: @cask.staged_path)
      ...
      if (nested_container = @cask.container&.nested)
        Dir.mktmpdir("cask-installer", DRXHUB_TEMP) do |tmpdir|
    ...
    def satisfy_cask_and_formula_dependencies
      ...
      missing_formulae_and_casks.each do |cask_or_formula|
        if cask_or_formula.is_a?(Cask)
          ...
          DinrusHub::Install.perform_preinstall_checks_once
    ...
    def zap
      load_installed_caskfile!
      ohai "Implied `dhub uninstall --cask #{@cask}`"
      uninstall_artifacts
    ...
    def forbidden_tap_check
      ...
      error_message = "The installation of #{@cask.full_name} has the tap #{cask_tap}\n" \
                      "but #{owner} "
      error_message << "has not allowed this tap in `DRXHUB_ALLOWED_TAPS`" unless cask_tap.allowed_by_env?
      error_message << " and\n" if !cask_tap.allowed_by_env? && cask_tap.forbidden_by_env?
      error_message << "has forbidden this tap in `DRXHUB_FORBIDDEN_TAPS`" if cask_tap.forbidden_by_env?
      error_message << ".#{owner_contact}"
    ...
    def forbidden_cask_and_formula_check
      ...
      forbidden_casks = Set.new(DinrusHub::EnvConfig.forbidden_casks.to_s.split)
      ...
      unless skip_cask_deps?
        cask_and_formula_dependencies.each do |dep_cask_or_formula|
          dep_name, dep_type, variable = if dep_cask_or_formula.is_a?(Cask) && forbidden_casks.present?
            ...
            [dep_cask_name, "cask", "DRXHUB_FORBIDDEN_CASKS"]
            ...
            [formula_name, "formula", "DRXHUB_FORBIDDEN_FORMULAE"]
          end
      ...
      raise CaskCannotBeInstalledError.new(@cask, <<~EOS
        forbidden for installation by #{owner} in `DRXHUB_FORBIDDEN_CASKS`.#{owner_contact}
      EOS
      )
    end
    ...
    def load_cask_from_source_api!
      @cask = DinrusHub::API::Cask.source_download(@cask)
    end
  end
end
