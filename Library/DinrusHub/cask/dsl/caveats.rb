  ...
  class DSL
    ...
    class Caveats < Base
      ...
      caveat :files_in_usr_local do
        next unless DRXHUB_PREFIX.to_s.downcase.start_with?("/usr/local")

        <<~EOS
          Cask #{@cask} installs files under /usr/local. The presence of such
          files can cause warnings when running `dhub doctor`, which is considered
          to be a bug in DinrusHub Cask.
        EOS
      end
        ...
        if java_version == :any
          ...
          <<~EOS
            #{@cask} requires Java #{java_version}. You can install it with:
              dhub install --cask temurin@#{java_version}
          EOS
        end
      end

      caveat :requires_rosetta do
        next if DinrusHub::SimulateSystem.current_arch != :arm
