...
module Cask
  ...
  class MultipleCaskErrors < CaskError
    ...
    def to_s
      <<~EOS
        Проблемы с несколькими касками:
  ...
  class TapCaskUnavailableError < CaskUnavailableError
    ...
    def to_s
      s = super
      s += "\nPlease tap it and then try again: dhub tap #{tap}" unless tap.installed?
      s
    end
  end
  ...
  class CaskAlreadyCreatedError < AbstractCaskErrorWithToken
    sig { returns(String) }
    def to_s
      %Q(Cask '#{token}' already exists. Run #{Formatter.identifier("dhub edit --cask #{token}")} to edit it.)
    end
  end
