  ...
  class Audit
    ...
    sig {
      params(
        ...
        location:    T.nilable(DinrusHub::SourceLocation),
    ...
    def audit_untrusted_pkg
      ...
      add_error "allow_untrusted is not permitted in official DinrusHub Cask taps"
    end
    ...
    sig { void }
    def audit_token
      token_auditor = DinrusHub::FormulaNameCaskTokenAuditor.new(cask.token)
    ...
    def audit_token_conflicts
      ...
      DinrusHub.with_no_api_env do
        return unless core_formula_names.include?(cask.token)

        add_error(
          "possible duplicate, cask token conflicts with DinrusHub core formula: #{Formatter.url(core_formula_url)}",
          strict_only: true,
        )
      end
    end
    ...
    def extract_artifacts
      ...
      @tmpdir ||= Pathname(Dir.mktmpdir("cask-audit", DRXHUB_TEMP))
    ...
    def audit_rosetta
      return if (url = cask.url).nil?
      return unless online?
      return if DinrusHub::SimulateSystem.current_arch != :arm
    ...
    def audit_livecheck_version
      ...
      referenced_cask, = DinrusHub::Livecheck.resolve_livecheck_reference(cask)

      # Respect skip conditions for a referenced cask
      if referenced_cask
        skip_info = DinrusHub::Livecheck::SkipConditions.referenced_skip_information(
          referenced_cask,
          DinrusHub::Livecheck.package_or_resource_name(cask),
        )
      end

      # Respect cask skip conditions (e.g. deprecated, disabled, latest, unversioned)
      skip_info ||= DinrusHub::Livecheck::SkipConditions.skip_information(cask)
      return :skip if skip_info.present?

      latest_version = DinrusHub::Livecheck.latest_version(
        cask,
    ...
    def audit_min_os
      ...
      return if min_os.nil? || min_os <= DRXHUB_MACOS_OLDEST_ALLOWED
    ...
    def livecheck_min_os
      ...
      content = DinrusHub::Livecheck::Strategy.page_content(cask.livecheck.url)[:content]
      return if content.blank?

      begin
        items = DinrusHub::Livecheck::Strategy::Sparkle.sort_items(
          DinrusHub::Livecheck::Strategy::Sparkle.filter_items(
            DinrusHub::Livecheck::Strategy::Sparkle.items_from_content(content),
          ),
        )
      rescue
        return
