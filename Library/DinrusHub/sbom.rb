...
class SBOM
  ...
  SCHEMA_FILE = (DRXHUB_LIBRARY_PATH/"data/schemas/sbom.json").freeze
  ...
  def self.create(formula, tab)
    ...
    attributes = {
      name:                 formula.name,
      homebrew_version:     DRXHUB_VERSION,
  ...
  def schema_validation_errors(bottling: false)
    unless require? "json_schemer"
      error_message = "Требуется json_schemer для оценки SBOM, выполните `dhub install-bundler-gems --add-groups=bottle`!"
      odie error_message if ENV["DRXHUB_ENFORCE_SBOM"]
      return []
    end
  ...
  def valid?(bottling: false)
    ...
    odie "Не удалось валидировать SBOM относительно схемы JSON!" if ENV["DRXHUB_ENFORCE_SBOM"]
        ...
        externalRefs:     [
          {
            ...
            referenceLocator:  "pkg:dhub/#{tap}/#{name}@#{stable_version}",
            referenceType:     "purl",
          },
        ],
        checksums:        [
    ...
    @runtime_dependencies.compact.filter_map do |dependency|
      ...
      dependency_json = {
        ...
        externalRefs:     [
          {
            referenceCategory: "PACKAGE-MANAGER",
            referenceLocator:  "pkg:dhub/#{dependency["full_name"]}@#{dependency["pkg_version"]}",
            referenceType:     "purl",
          },
        ],
      }
