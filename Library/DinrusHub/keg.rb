...
class Keg
    ...
    def to_s
      ...
      s << <<~EOS
        Чтобы принудительно это сделать и переписать все конфликтующие файлы:
          dhub link --overwrite #{keg.name}

        Чирьы получить список всех файлов, которые будут удалены:
          dhub link --overwrite #{keg.name} --dry-run
      EOS
      s.join("\n")
    end
  end
  ...
  def self.for(path)
    ...
    if (path = original_path.realpath)
      until path.root?
        return Keg.new(path) if path.parent.parent == DRXHUB_CELLAR.realpath
  ...
  ).map { |dir| DRXHUB_PREFIX/dir }.sort.uniq.freeze
  end
  ...
  def self.must_be_writable_directories
    ...
    %w[
      ...
      var/log
    ].map { |dir| DRXHUB_PREFIX/dir } + must_exist_subdirectories + [
      DRXHUB_CACHE,
      DRXHUB_CELLAR,
      DRXHUB_LOCKS,
      DRXHUB_LOGS,
  ...
  def initialize(path)
    path = path.resolved_path if path.to_s.start_with?("#{DRXHUB_PREFIX}/opt/")
    raise "#{path} неполноценный кег" if path.parent.parent.realpath != DRXHUB_CELLAR.realpath
    raise "#{path} не директория" unless path.directory?

    @path = path
    @name = path.parent.basename.to_s
    @linked_keg_record = DRXHUB_LINKED_KEGS/name
    @opt_record = DRXHUB_PREFIX/"opt/#{name}"
    @oldname_opt_records = []
  ...
  def unlink(verbose: false, dry_run: false)
    ...
    keg_directories.each do |dir|
      dir.find do |src|
        dst = DRXHUB_PREFIX + src.relative_path_from(path)
  ...
  def oldname_opt_records
    ...
    @oldname_opt_records = if (opt_dir = DRXHUB_PREFIX/"opt").directory?
  ...
  def link(verbose: false, dry_run: false, overwrite: false)
    ...
    link_dir("Frameworks", verbose:, dry_run:, overwrite:) do |relative_path|
      ...
      # can link their versions into it and `dhub [un]link` works.
  ...
  # symlinks the contents of path+relative_dir recursively into #{DRXHUB_PREFIX}/relative_dir
  def link_dir(relative_dir, verbose: false, dry_run: false, overwrite: false)
    ...
    root.find do |src|
      ...
      if src.symlink? || src.file?
        ...
        # cached object files and next time dhub wants to link, the
        # file is in the way.
