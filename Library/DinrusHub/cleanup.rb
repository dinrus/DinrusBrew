...
module DinrusHub
  # Helper class for cleaning up the DinrusHub cache.
  class Cleanup
    CLEANUP_DEFAULT_DAYS = DinrusHub::EnvConfig.cleanup_periodic_full_days.to_i.freeze
    ...
    class << self
      ...
      def stale_formula?(pathname, scrub)
        return false unless DRXHUB_CELLAR.directory?

        version = if DRXHUB_BOTTLES_EXTNAME_REGEX.match?(to_s)
          begin
        ...
        formula = begin
          Formulary.from_rack(DRXHUB_CELLAR/formula_name)
    ...
    PERIODIC_CLEAN_FILE = (DRXHUB_CACHE/".cleaned").freeze

    attr_predicate :dry_run?, :scrub?, :prune?
    attr_reader :args, :days, :cache, :disk_cleanup_size

    def initialize(*args, dry_run: false, scrub: false, days: nil, cache: DRXHUB_CACHE)
      @disk_cleanup_size = 0
      @args = args
      @dry_run = dry_run
      @scrub = scrub
      @prune = days.present?
      @days = days || DinrusHub::EnvConfig.cleanup_max_age_days.to_i
      @cache = cache
      @cleaned_up_paths = Set.new
    end

    def self.install_formula_clean!(formula, dry_run: false)
      return if DinrusHub::EnvConfig.no_install_cleanup?
      return unless formula.latest_version_installed?
      return if skip_clean_formula?(formula)

      if dry_run
        ohai "Был бы выполнен `dhub cleanup #{formula}`"
      else
        ohai "Выполняется `dhub cleanup #{formula}`..."
      end
    ...
    def self.puts_no_install_cleanup_disable_message
      return if DinrusHub::EnvConfig.no_env_hints?
      return if DinrusHub::EnvConfig.no_install_cleanup?

      puts "Отключите это поведение, установив DRXHUB_NO_INSTALL_CLEANUP."
      puts "Скрыть эти подсказки можно посредством DRXHUB_NO_ENV_HINTS (см. `man dhub`)."
    end
    ...
    def self.skip_clean_formula?(formula)
      no_cleanup_formula = DinrusHub::EnvConfig.no_cleanup_formulae
    ...
    def self.periodic_clean_due?
      return false if DinrusHub::EnvConfig.no_install_cleanup?
    ...
    def self.periodic_clean!(dry_run: false)
      ...
      if dry_run
        oh1 "Would run `dhub cleanup` which has not been run in the last #{CLEANUP_DEFAULT_DAYS} days"
      else
        oh1 "`dhub cleanup` не выполнялся последние #{CLEANUP_DEFAULT_DAYS} дней, выполняется сейчас..."
      end
    ...
    def clean!(quiet: false, periodic: false)
      if args.empty?
        ...
        if ENV["DRXHUB_AUTOREMOVE"].present?
          opoo "DRXHUB_AUTOREMOVE теперь no-op, так как это его дефолтное поведение. " \
               "Чтобы отключить его, установите DRXHUB_NO_AUTOREMOVE=1."
        end
        Cleanup.autoremove(dry_run: dry_run?) unless DinrusHub::EnvConfig.no_autoremove?
        ...
        # avoid having to try to do a `dhub install` when we've just deleted
        ...
        args.each do |arg|
          ...
          if formula && Cleanup.skip_clean_formula?(formula)
            onoe "Отказ чистить #{formula}, так как она в списках у " \
                 "#{Tty.bold}DRXHUB_NO_CLEANUP_FORMULAE#{Tty.reset}!"
          elsif formula
    ...
    def cleanup_logs
      return unless DRXHUB_LOGS.directory?

      logs_days = [days, CLEANUP_DEFAULT_DAYS].min

      DRXHUB_LOGS.subdirs.each do |dir|
    ...
    def cleanup_lockfiles(*lockfiles)
      ...
      lockfiles = DRXHUB_LOCKS.children.select(&:file?) if lockfiles.empty? && DRXHUB_LOCKS.directory?
    ...
    def cleanup_portable_ruby
      ...
      bundler_paths.each do |bundler_path|
        if dry_run?
          puts Utils.popen_read("git", "-C", DRXHUB_REPOSITORY, "clean", "-nx", bundler_path).chomp
        else
          puts Utils.popen_read("git", "-C", DRXHUB_REPOSITORY, "clean", "-ffqx", bundler_path).chomp
        end
      end
    ...
    def cleanup_bootsnap
      ...
      bootsnap.each_child do |subdir|
        cleanup_path(subdir) { FileUtils.rm_r(subdir) } if subdir.basename.to_s != DinrusHub.bootsnap_key
      end
    end
    ...
    def cleanup_python_site_packages
      ...
      DRXHUB_PREFIX.glob("lib/python*/site-packages").each do |site_packages|
    ...
    def self.autoremove(dry_run: false)
      ...
      # Remove formulae listed in DRXHUB_NO_CLEANUP_FORMULAE and their dependencies.
      if DinrusHub::EnvConfig.no_cleanup_formulae.present?
