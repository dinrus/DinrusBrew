...
module FormulaCellarChecks
  ...
  def check_env_path(bin)
    ...
    prefix_bin = (DRXHUB_PREFIX/bin.basename)
  ...
  def check_manpages
    ...
    <<~EOS
      ...
      DinrusHub requires that man pages live under "share".
  ...
  def check_infopages
    ...
    <<~EOS
      ...
      DinrusHub suggests that info pages live under "share".
  ...
  def check_generic_executables(bin)
    ...
    <<~EOS
      ...
      DinrusHub suggests that this software is installed to "libexec" and then
      symlinked as needed.
      The offending files are:
        #{generics * "\n  "}
    EOS
  ...
  def check_elisp_root(share, name)
    ...
    <<~EOS
      Emacs Lisp files were linked directly to "#{DRXHUB_PREFIX}/share/emacs/site-lisp".
  ...
  def check_shim_references(prefix)
    ...
    matches = []
    keg.each_unique_file_matching(DRXHUB_SHIMS_PATH) do |f|
    ...
    <<~EOS
      Files were found with references to the DinrusHub shims directory.
      The offending files are:
        #{matches * "\n  "}
    EOS
  end
  ...
  def check_python_symlinks(name, keg_only)
    ...
    return if %w[pip3 wheel3].none? do |l|
      link = DRXHUB_PREFIX/"bin"/l
      link.exist? && File.realpath(link).start_with?(DRXHUB_CELLAR/name)
    end
  ...
  def check_cpuid_instruction(formula)
    ...
    dot_brew_formula = formula.prefix/".dhub/#{formula.name}.rb"
