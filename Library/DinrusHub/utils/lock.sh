...
lock() {
  ...
  local lock_dir="${DRXHUB_PREFIX}/var/homebrew/locks"
  local lock_file="${lock_dir}/${lock_filename}"
  [[ -d "${lock_dir}" ]] || mkdir -p "${lock_dir}"
  if [[ ! -w "${lock_dir}" ]]
  then
    odie <<EOS
Can't create \`dhub ${command_name_and_args}\` lock in ${lock_dir}!
Fix permissions by running:
  sudo chown -R ${USER-\$(whoami)} ${DRXHUB_PREFIX}/var/homebrew
EOS
  fi
  ...
  then
    local lock_context
    if [[ -n "${DRXHUB_LOCK_CONTEXT}" ]]
    then
      lock_context="\n${DRXHUB_LOCK_CONTEXT}"
    fi

    odie <<EOS
Another \`dhub ${command_name_and_args}\` process is already running.${lock_context}
...
_create_lock() {
  ...
  local utils_lock_sh="${DRXHUB_LIBRARY}/DinrusHub/utils/lock_sh"
  ...
  then
    ...
    onoe "Cannot create \`dhub ${command_name_and_args}\` lock due to missing/too old ruby/flock/python, please avoid running DinrusHub in parallel."
  fi
}
