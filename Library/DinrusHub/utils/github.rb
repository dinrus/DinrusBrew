...
module GitHub
  ...
  def self.print_pull_requests_matching(query, only = nil)
    open_or_closed_prs = search_issues(query, is: only, type: "pr", user: "DinrusHub")
  ...
  def self.fetch_pull_requests(name, tap_remote_repo, state: nil, version: nil)
    return [] if DinrusHub::EnvConfig.no_github_api?
    ...
    if API.credentials_type == :none
    ...
    elsif state == "open" && ENV["GITHUB_REPOSITORY_OWNER"] == "DinrusHub"
      ...
      # The PR API is also not very scalable so limit to DinrusHub CI.
  ...
  def self.multiple_short_commits_exist?(user, repo, commit)
    return false if DinrusHub::EnvConfig.no_github_api?
  ...
  def self.count_repo_commits(nwo, user, from: nil, to: nil, max: nil)
    odie "Cannot count commits, DRXHUB_NO_GITHUB_API set!" if DinrusHub::EnvConfig.no_github_api?
  ...
  def self.too_many_open_prs?(tap)
    ...
    return false if ENV["DRXHUB_TEST_BOT_AUTOBUMP"].present?

    odie "Cannot count PRs, DRXHUB_NO_GITHUB_API set!" if DinrusHub::EnvConfig.no_github_api?
