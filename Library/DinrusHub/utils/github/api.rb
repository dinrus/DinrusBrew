  ...
  def self.pat_blurb(scopes = ALL_SCOPES)
    ...
    <<~EOS
      Create a GitHub personal access token:
        #{Formatter.url(
          "https://github.com/settings/tokens/new?scopes=#{scopes.join(",")}&description=DinrusHub",
        )}
      #{Utils::Shell.set_variable_in_profile("DRXHUB_GITHUB_API_TOKEN", "your_token_here")}
    EOS
  end
  ...
  module API
    ...
    class AuthenticationFailedError < Error
      def initialize(credentials_type, github_message)
        ...
        when :env_token
          require "utils/formatter"
          <<~EOS
            DRXHUB_GITHUB_API_TOKEN may be invalid or expired; check:
    ...
    def self.github_cli_token
      ...
      Utils::UID.drop_euid do
        # Avoid `Formula["gh"].opt_bin` so this method works even with `DRXHUB_DISABLE_LOAD_FORMULA`.
        env = {
          "PATH" => PATH.new(DRXHUB_PREFIX/"opt/gh/bin", ENV.fetch("PATH")),
    ...
    def self.credentials
      @credentials ||= DinrusHub::EnvConfig.github_api_token.presence
    ...
    def self.credentials_type
      if DinrusHub::EnvConfig.github_api_token.present?
        :env_token
    ...
    CREDENTIAL_NAMES = {
      env_token:                  "DRXHUB_GITHUB_API_TOKEN",
    ...
    def self.open_rest(
      ...
      return block_given? ? yield({}) : {} if DinrusHub::EnvConfig.no_github_api?
      ...
      if data
        begin
          ...
          data_tmpfile = Tempfile.new("github_api_post", DRXHUB_TEMP)
      ...
      headers_tmpfile = Tempfile.new("github_api_headers", DRXHUB_TEMP)
      begin
        if data_tmpfile
