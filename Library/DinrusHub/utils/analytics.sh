...
# DRXHUB_LINUX, DRXHUB_REPOSITORY is set by bin/dhub
# DRXHUB_NO_ANALYTICS is from the user environment.
# shellcheck disable=SC2154

setup-analytics() {
  local git_config_file="${DRXHUB_REPOSITORY}/.git/config"
  ...
  then
    # Internal variable for dhub's use, to differentiate from user-supplied setting
    export DRXHUB_NO_ANALYTICS_THIS_RUN="1"
    return
  fi
}
