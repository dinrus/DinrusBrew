...
module Utils
  ...
  module Curl
    ...
    def curl_executable(use_homebrew_curl: false)
      return DRXHUB_BREWED_CURL_PATH if use_homebrew_curl

      @curl_executable ||= T.let(DRXHUB_SHIMS_PATH/"shared/curl", T.nilable(T.any(Pathname, String)))
    end
    ...
    def curl_args(
      *extra_args,
      connect_timeout: nil,
      max_time: nil,
      retries: DinrusHub::EnvConfig.curl_retries.to_i,
      retry_max_time: nil,
      show_output: false,
      show_error: true,
      user_agent: nil,
      ...
      if show_output != true
        ...
        args << "--verbose" if DinrusHub::EnvConfig.curl_verbose?
    ...
    def curl_check_http_content(url, url_type, specs: {}, user_agents: [:default], referer: nil,
      ...
      user_agents.each do |user_agent|
        loop do
          ...
          attempts += 1
          break if attempts >= DinrusHub::EnvConfig.curl_retries.to_i
        end
      ...
      unless http_status_ok?(details[:status_code])
        ...
        check_github_api = url_type == SharedAudits::URL_TYPE_HOMEPAGE &&
                           ...
                           repo_details &&
                           DinrusHub::EnvConfig.github_api_token.present?
      ...
      if url.start_with?("https://") && DinrusHub::EnvConfig.no_insecure_redirect? &&
