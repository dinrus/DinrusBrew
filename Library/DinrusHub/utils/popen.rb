  ...
  def self.popen(args, mode, options = {})
    IO.popen("-", mode) do |pipe|
      if pipe
        ...
        options[:err] ||= File::NULL unless ENV["DRXHUB_STDERR"]
        begin
          exec(*args, options)
        rescue Errno::ENOENT
          $stderr.puts "dhub: command not found: #{args[0]}" if options[:err] != :close
          exit! 127
        rescue SystemCallError
          $stderr.puts "dhub: exec failed: #{args[0]}" if options[:err] != :close
          exit! 1
        end
      end
    end
