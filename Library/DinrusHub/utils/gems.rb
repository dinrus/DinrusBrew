...
# work as the first item in `dhub.rb` so we can load gems with Bundler when
...
module DinrusHub
  ...
  # After updating this, run `dhub vendor-gems --update=--bundler`.
  DRXHUB_BUNDLER_VERSION = "2.5.20"

  # Bump this whenever a committed vendored gem is later added to or exclusion removed from gitignore.
  # This will trigger it to reinstall properly if `dhub install-bundler-gems` needs it.
  VENDOR_VERSION = 7
  private_constant :VENDOR_VERSION

  RUBY_BUNDLE_VENDOR_DIRECTORY = (DRXHUB_LIBRARY_PATH/"vendor/bundle/ruby").freeze
  ...
  def self.gemfile
    File.join(ENV.fetch("DRXHUB_LIBRARY"), "DinrusHub", "Gemfile")
  end
  ...
  def self.setup_gem_environment!(setup_path: true)
    ...
    ENV["TMPDIR"] = ENV.fetch("DRXHUB_TEMP", nil)
  ...
  def self.install_bundler!
    ...
    ENV["BUNDLER_VERSION"] = DRXHUB_BUNDLER_VERSION # Set so it correctly finds existing installs
    install_gem_setup_path!(
      "bundler",
      version:               DRXHUB_BUNDLER_VERSION,
      executable:            "bundle",
  ...
  def self.install_bundler_gems!(only_warn_on_failure: false, setup_path: true, groups: [])
    ...
    if (homebrew_bundle_user_cache = ENV.fetch("DRXHUB_BUNDLE_USER_CACHE", nil))
