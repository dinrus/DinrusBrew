...
ohai() {
  ...
  if [[ -n "${DRXHUB_COLOR}" || (-t 1 && -z "${DRXHUB_NO_COLOR}") ]]
  then
...
opoo() {
  ...
  if [[ -n "${DRXHUB_COLOR}" || (-t 2 && -z "${DRXHUB_NO_COLOR}") ]]
  then
...
dhub() {
  # This variable is set by bin/dhub
  # shellcheck disable=SC2154
  "${DRXHUB_BREW_FILE}" "$@"
}

curl() {
  "${DRXHUB_LIBRARY}/DinrusHub/shims/shared/curl" "$@"
}

git() {
  "${DRXHUB_LIBRARY}/DinrusHub/shims/shared/git" "$@"
}
