...
module Utils
  ...
  module Bottles
    class << self
      ...
      def tag(tag = nil)
        ...
        when Tag
          ...
          @tag ||= Tag.new(
            system: DRXHUB_SYSTEM.downcase.to_sym,
            arch:   DRXHUB_PROCESSOR.downcase.to_sym,
          )
        end
      end
      ...
      def extname_tag_rebuild(filename)
        DRXHUB_BOTTLES_EXTNAME_REGEX.match(filename).to_a
      end
      ...
      def formula_contents(bottle_file,
        ...
        formula_path = "#{name}/#{bottle_version}/.dhub/#{name}.rb"
    ...
    class Tag
      ...
      def default_prefix
        if linux?
          DRXHUB_LINUX_DEFAULT_PREFIX
        elsif standardized_arch == :arm64
          DRXHUB_MACOS_ARM_DEFAULT_PREFIX
        else
          DRXHUB_DEFAULT_PREFIX
        end
      end
