...
module Utils
  # Вспом. функции для опроса инфы о Git.
  #
  # @see GitRepository
  module Git
    ...
    def self.ensure_installed!
      ...
      if CoreTap.instance.installed?
        begin
          ...
          raise "Отказ в установке Git на генерную ОС." if ENV["DRXHUB_TEST_GENERIC_OS"]

          ensure_formula_installed!("git")
          clear_available_cache
       # rescue
          # raise "Git недоступен"
        end
      end

    #  raise "Git недоступен" unless available?
    end

    def self.set_name_email!(author: true, committer: true)
      if DinrusHub::EnvConfig.git_name
        ENV["GIT_AUTHOR_NAME"] = DinrusHub::EnvConfig.git_name if author
        ENV["GIT_COMMITTER_NAME"] = DinrusHub::EnvConfig.git_name if committer
      end

      if DinrusHub::EnvConfig.git_committer_name && committer
        ENV["GIT_COMMITTER_NAME"] = DinrusHub::EnvConfig.git_committer_name
      end

      if DinrusHub::EnvConfig.git_email
        ENV["GIT_AUTHOR_EMAIL"] = DinrusHub::EnvConfig.git_email if author
        ENV["GIT_COMMITTER_EMAIL"] = DinrusHub::EnvConfig.git_email if committer
      end

      return unless committer
      return unless DinrusHub::EnvConfig.git_committer_email

      ENV["GIT_COMMITTER_EMAIL"] = DinrusHub::EnvConfig.git_committer_email
    end

    def self.setup_gpg!
      gnupg_bin = DRXHUB_PREFIX/"opt/gnupg/bin"
