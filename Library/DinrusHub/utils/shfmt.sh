...
# DRXHUB_PREFIX is set by extend/ENV/super.rb
# shellcheck disable=SC2154
if [[ -z "${DRXHUB_PREFIX}" ]]
then
  odie "${0##*/}: This program is internal and must be run via dhub."
fi

# DRXHUB_PREFIX is set by extend/ENV/super.rb
# shellcheck disable=SC2154
SHFMT="${DRXHUB_PREFIX}/opt/shfmt/bin/shfmt"
if [[ ! -x "${SHFMT}" ]]
then
  odie "${0##*/}: Please install shfmt by running \`dhub install shfmt\`."
fi

# DRXHUB_PREFIX is set by extend/ENV/super.rb
# shellcheck disable=SC2154
DIFF="${DRXHUB_PREFIX}/opt/diffutils/bin/diff"
DIFF_ARGS=("-d" "-C" "1")
if [[ ! -x "${DIFF}" ]]
then
  # DRXHUB_PATH is set by global.rb
  # shellcheck disable=SC2154
  if [[ -x "$(PATH="${DRXHUB_PATH}" command -v diff)" ]]
  then
    DIFF="$(PATH="${DRXHUB_PATH}" command -v diff)" # fall back to `diff` in PATH without coloring
  elif [[ -z "${DRXHUB_PATH}" && -x "$(command -v diff)" ]]
  then
    # DRXHUB_PATH may unset if shfmt.sh is called by vscode
    DIFF="$(command -v diff)" # fall back to `diff` in PATH without coloring
  else
    odie "${0##*/}: Please install diff by running \`dhub install diffutils\`."
  fi
else
...
do
  ...
  then
    case "${retcode}" in
      ...
      4) onoe "${0##*/}: Fixable bad styles detected in file \"${file}\", run \`dhub style --fix\` to apply. Formatter exited with code 4." ;;
