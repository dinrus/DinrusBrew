...
# Repology API client.
module Repology
  DRXHUB_CORE = "homebrew"
  DRXHUB_CASK = "homebrew_casks"
  MAX_PAGINATION = 15
  ...
  def self.latest_version(repositories)
    # The status is "unique" when the package is present only in DinrusHub, so
    # Repology has no way of knowing if the package is up-to-date.
    is_unique = repositories.find do |repo|
      repo["status"] == "unique"
    end.present?

    return "present only in DinrusHub" if is_unique
