...
module Utils
  ...
  module Link
    ...
    def self.link_manpages(path, command)
      link_src_dst_dirs(path/"manpages", DRXHUB_PREFIX/"share/man/man1", command)
    end

    sig { params(path: Pathname).void }
    def self.unlink_manpages(path)
      unlink_src_dst_dirs(path/"manpages", DRXHUB_PREFIX/"share/man/man1")
    end

    sig { params(path: Pathname, command: String).void }
    def self.link_completions(path, command)
      link_src_dst_dirs(path/"completions/bash", DRXHUB_PREFIX/"etc/bash_completion.d", command)
      link_src_dst_dirs(path/"completions/zsh", DRXHUB_PREFIX/"share/zsh/site-functions", command)
      link_src_dst_dirs(path/"completions/fish", DRXHUB_PREFIX/"share/fish/vendor_completions.d", command)
    end

    sig { params(path: Pathname).void }
    def self.unlink_completions(path)
      unlink_src_dst_dirs(path/"completions/bash", DRXHUB_PREFIX/"etc/bash_completion.d")
      unlink_src_dst_dirs(path/"completions/zsh", DRXHUB_PREFIX/"share/zsh/site-functions")
      unlink_src_dst_dirs(path/"completions/fish", DRXHUB_PREFIX/"share/fish/vendor_completions.d")
    end

    sig { params(path: Pathname, command: String).void }
    def self.link_docs(path, command)
      link_src_dst_dirs(path/"docs", DRXHUB_PREFIX/"share/doc/homebrew", command, link_dir: true)
    end
  end
end
