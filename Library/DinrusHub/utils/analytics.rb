...
module Utils
  ...
  module Analytics
    ...
    class << self
      ...
      sig { params(command_instance: DinrusHub::AbstractCommand).void }
      def report_command_run(command_instance)
        ...
        tags = {
          command:,
          ci:        ENV["CI"].present?,
          devcmdrun: DinrusHub::EnvConfig.devcmdrun?,
          developer: DinrusHub::EnvConfig.developer?,
        }
      ...
      def report_test_bot_test(step_command_short, passed)
        return if not_this_run? || disabled?
        return if ENV["DRXHUB_TEST_BOT_ANALYTICS"].blank?
        ...
        tags = {
          passed:,
          arch:   DRXHUB_PHYSICAL_PROCESSOR,
          os:     DRXHUB_SYSTEM,
        }
      ...
      sig { returns(T::Boolean) }
      def disabled?
        return true if DinrusHub::EnvConfig.no_analytics?
      ...
      sig { returns(T::Boolean) }
      def not_this_run?
        ENV["DRXHUB_NO_ANALYTICS_THIS_RUN"].present?
      end

      sig { returns(T::Boolean) }
      def no_message_output?
        # Used by DinrusHub/install
        ENV["DRXHUB_NO_ANALYTICS_MESSAGE_OUTPUT"].present?
      end

      sig { void }
      def messages_displayed!
        DinrusHub::Settings.write :analyticsmessage, true
        DinrusHub::Settings.write :caskanalyticsmessage, true
        DinrusHub::Settings.write :influxanalyticsmessage, true
      end

      sig { void }
      def enable!
        DinrusHub::Settings.write :analyticsdisabled, false
        delete_uuid!
        messages_displayed!
      end
      ...
      sig { void }
      def disable!
        DinrusHub::Settings.write :analyticsdisabled, true
        delete_uuid!
      end

      sig { void }
      def delete_uuid!
        DinrusHub::Settings.delete :analyticsuuid
      end

      sig { params(args: DinrusHub::Cmd::Info::Args, filter: T.nilable(String)).void }
      def output(args:, filter: nil)
        ...
        begin
          json = DinrusHub::API::Analytics.fetch category, days
      ...
      sig { params(json: T::Hash[String, T.untyped], args: DinrusHub::Cmd::Info::Args).void }
      ...
      sig { params(formula: Formula, args: DinrusHub::Cmd::Info::Args).void }
      ...
      def formula_output(formula, args:)
        return if DinrusHub::EnvConfig.no_analytics? || DinrusHub::EnvConfig.no_github_api?

        require "api"

        json = DinrusHub::API::Formula.fetch formula.name
      ...
      sig { params(cask: Cask::Cask, args: DinrusHub::Cmd::Info::Args).void }
      def cask_output(cask, args:)
        return if DinrusHub::EnvConfig.no_analytics? || DinrusHub::EnvConfig.no_github_api?

        require "api"

        json = DinrusHub::API::Cask.fetch cask.token
        return if json.blank? || json["analytics"].blank?
      ...
      def default_package_tags
        cache[:default_package_tags] ||= begin
          ...
          prefix = DinrusHub.default_prefix? ? DRXHUB_PREFIX.to_s : "custom-prefix"

          # Tags are always strings and must have low cardinality.
          {
            ci:             ENV["CI"].present?,
            prefix:,
            default_prefix: DinrusHub.default_prefix?,
            developer:      DinrusHub::EnvConfig.developer?,
            devcmdrun:      DinrusHub::EnvConfig.devcmdrun?,
            arch:           DRXHUB_PHYSICAL_PROCESSOR,
            os:             DRXHUB_SYSTEM,
      ...
      def default_package_fields
        cache[:default_package_fields] ||= begin
          version = if (match_data = DRXHUB_VERSION.match(/^[\d.]+/))
            suffix = "-dev" if DRXHUB_VERSION.include?("-")
      ...
      def config_true?(key)
        DinrusHub::Settings.read(key) == "true"
      end
