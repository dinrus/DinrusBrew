...
module Commands
  DRXHUB_CMD_PATH = (DRXHUB_LIBRARY_PATH/"cmd").freeze
  DRXHUB_DEV_CMD_PATH = (DRXHUB_LIBRARY_PATH/"dev-cmd").freeze
  # If you are going to change anything in below hash,
  # be sure to also update appropriate case statement in brew.sh
  DRXHUB_INTERNAL_COMMAND_ALIASES = {
    "ls"           => "list",
    "homepage"     => "home",
    "-S"           => "search",
    "up"           => "update",
  ...
  def self.internal_cmd_path(cmd)
    [
      DRXHUB_CMD_PATH/"#{cmd}.rb",
      DRXHUB_CMD_PATH/"#{cmd}.sh",
    ].find(&:exist?)
  end

  def self.internal_dev_cmd_path(cmd)
    [
      DRXHUB_DEV_CMD_PATH/"#{cmd}.rb",
      DRXHUB_DEV_CMD_PATH/"#{cmd}.sh",
    ].find(&:exist?)
  end
  ...
  def self.path(cmd)
    internal_cmd = DRXHUB_INTERNAL_COMMAND_ALIASES.fetch(cmd, cmd)
  ...
  def self.tap_cmd_directories
    Pathname.glob DRXHUB_TAP_DIRECTORY/"*/*/cmd"
  end

  def self.internal_commands_paths
    find_commands DRXHUB_CMD_PATH
  end

  def self.internal_developer_commands_paths
    find_commands DRXHUB_DEV_CMD_PATH
  end
  ...
  def self.internal_commands
    find_internal_commands(DRXHUB_CMD_PATH).map(&:to_s)
  end

  def self.internal_developer_commands
    find_internal_commands(DRXHUB_DEV_CMD_PATH).map(&:to_s)
  end

  def self.internal_commands_aliases
    DRXHUB_INTERNAL_COMMAND_ALIASES.keys
  end
  ...
  def self.rebuild_internal_commands_completion_list
    ...
    cmds.reject! { |cmd| DinrusHub::Completions::COMPLETIONS_EXCLUSION_LIST.include? cmd }

    file = DRXHUB_REPOSITORY/"completions/internal_commands_list.txt"
  ...
  def self.rebuild_commands_completion_list
    ...
    cmds = commands(aliases: true) - DinrusHub::Completions::COMPLETIONS_EXCLUSION_LIST

    all_commands_file = DRXHUB_CACHE/"all_commands_list.txt"
    external_commands_file = DRXHUB_CACHE/"external_commands_list.txt"
  ...
  def self.command_options(command)
    ...
    if (cmd_parser = DinrusHub::CLI::Parser.from_cmd_path(path))
  ...
  def self.named_args_type(command)
    ...
    cmd_parser = DinrusHub::CLI::Parser.from_cmd_path(path)
