...
class AbstractDownloadStrategy
  ...
  def initialize(url, name, version, **meta)
    ...
    @name = name
    @version = version
    @cache = meta.fetch(:cache, DRXHUB_CACHE)
    @meta = meta
    @quiet = false
...
class AbstractFileDownloadStrategy < AbstractDownloadStrategy
  ...
  def cached_location
    ...
    downloads = Pathname.glob(DRXHUB_CACHE/"downloads/#{url_sha256}--*")
                        .reject { |path| path.extname.end_with?(".incomplete") }

    @cached_location = if downloads.count == 1
      downloads.first
    else
      DRXHUB_CACHE/"downloads/#{url_sha256}--#{Utils.safe_filename(resolved_basename)}"
    end
  end
...
class CurlDownloadStrategy < AbstractFileDownloadStrategy
  ...
  def fetch(timeout: nil)
    ...
    begin
      ...
      begin
        ...
        if (domain = DinrusHub::EnvConfig.artifact_domain)
          url = url.sub(%r{^https?://#{GitHubPackages::URL_DOMAIN}/}o, "#{domain.chomp("/")}/")
          urls = [] if DinrusHub::EnvConfig.artifact_domain_no_fallback?
        end
  ...
  def _fetch(url:, resolved_url:, timeout:)
    ...
    if DinrusHub::EnvConfig.no_insecure_redirect? &&
       url.start_with?("https://") && !resolved_url.start_with?("https://")
      $stderr.puts "Обнаружен редирект из HTTPS в HTTP и установлена DRXHUB_NO_INSECURE_REDIRECT."
      raise CurlDownloadStrategyError, url
    end
...
class CurlGitHubPackagesDownloadStrategy < CurlDownloadStrategy
  ...
  def initialize(url, name, version, **meta)
    ...
    # DRXHUB_GITHUB_PACKAGES_AUTH set in brew.sh
    meta[:headers] << "Authorization: #{DRXHUB_GITHUB_PACKAGES_AUTH}"
    super
  end
...
class GitDownloadStrategy < VCSDownloadStrategy
  ...
  def fix_absolute_submodule_gitdir_references!
    ...
    submodule_dirs.lines.map(&:chomp).each do |submodule_dir|
      ...
      if git_dir.nil?
        onoe "Не удалось разобрать '#{dot_git}'." if DinrusHub::EnvConfig.developer?
        next
      end
