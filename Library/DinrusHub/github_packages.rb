...
class GitHubPackages
  ...
  # Translate DinrusHub tab.arch to OCI platform.architecture
  TAB_ARCH_TO_PLATFORM_ARCHITECTURE = {
    "arm64"  => "arm64",
    "x86_64" => "amd64",
  }.freeze

  # Translate DinrusHub built_on.os to OCI platform.os
  ...
  def upload_bottles(bottles_hash, keep_old:, dry_run:, warn_on_error:)
    user = DinrusHub::EnvConfig.github_packages_user
    token = DinrusHub::EnvConfig.github_packages_token

    raise UsageError, "DRXHUB_GITHUB_PACKAGES_USER is unset." if user.blank?
    raise UsageError, "DRXHUB_GITHUB_PACKAGES_TOKEN is unset." if token.blank?
  ...
  def download(user, token, skopeo, image_uri, root, dry_run:)
    ...
    if dry_run
      puts "#{skopeo} #{args.join(" ")} --src-creds=#{user}:$DRXHUB_GITHUB_PACKAGES_TOKEN"
    else
  ...
  def preupload_check(user, token, skopeo, _formula_full_name, bottle_hash, keep_old:, dry_run:, warn_on_error:)
    ...
    if dry_run
      puts "#{skopeo} #{inspect_args.join(" ")} --creds=#{user}:$DRXHUB_GITHUB_PACKAGES_TOKEN"
    else
  ...
  def upload_bottle(user, token, skopeo, formula_full_name, bottle_hash, keep_old:, dry_run:, warn_on_error:)
    ...
    manifests += bottle_hash["bottle"]["tags"].map do |bottle_tag, tag_hash|
      ...
      descriptor_annotations_hash = {
        ...
        "sh.dhub.bottle.cpu.variant"        => cpu_variant,
        "sh.dhub.bottle.digest"             => tar_gz_sha256,
        "sh.dhub.bottle.glibc.version"      => glibc_version,
        "sh.dhub.bottle.size"               => local_file_size.to_s,
        "sh.dhub.bottle.installed_size"     => tag_hash["installed_size"].to_s,
        "sh.dhub.license"                   => license,
        "sh.dhub.tab"                       => tab.to_json,
        "sh.dhub.path_exec_files"           => path_exec_files_string,
      }.compact_blank
    ...
    if dry_run
      puts "#{skopeo} #{args.join(" ")} --dest-creds=#{user}:$DRXHUB_GITHUB_PACKAGES_TOKEN"
    else
