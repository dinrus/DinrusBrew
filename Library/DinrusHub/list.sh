# does the quickest output of dhub list possible for no named arguments.
# DRXHUB_CELLAR, DRXHUB_PREFIX are set by brew.sh
...
homebrew-list() {
  ...
  then
    ...
    formula_output="$(/usr/bin/env "${ls_env[@]}" ls "${ls_args[@]}" "${DRXHUB_CELLAR}")" || exit 1
    if [[ -n "${formula_output}" ]]
    then
      echo "${formula_output}"
    fi
  ...
  if [[ -z "${formula}" && -d "${DRXHUB_CASKROOM}" ]]
  then
    if [[ -n "${tty}" && -z "${cask}" ]]
    then
      ohai "Casks"
    ...
    local cask_output
    cask_output="$(/usr/bin/env "${ls_env[@]}" ls "${ls_args[@]}" "${DRXHUB_CASKROOM}")" || exit 1
    if [[ -n "${cask_output}" ]]
    then
      echo "${cask_output}"
    fi
