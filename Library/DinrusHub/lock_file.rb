...
# A lock file to prevent multiple DinrusHub processes from modifying the same path.
class LockFile
  ...
  def initialize(type, locked_path)
    ...
    @path = DRXHUB_LOCKS/"#{lock_name}.#{type}.lock"
    @lockfile = nil
  end
...
class FormulaLock < LockFile
  ...
  def initialize(rack_name)
    super(:formula, DRXHUB_CELLAR/rack_name)
  end
end
...
class CaskLock < LockFile
  ...
  def initialize(cask_token)
    super(:cask, DRXHUB_PREFIX/"Caskroom/#{cask_token}")
  end
end
