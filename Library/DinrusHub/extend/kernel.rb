...
# Contains shorthand DinrusHub utility methods like `ohai`, `opoo`, `odisabled`.
...
module Kernel
  ...
  def odeprecated(method, replacement = nil,
    ...
    # Don't throw deprecations at all for cached, .dhub or .metadata files.
    return if backtrace.any? do |line|
      next true if line.include?(DRXHUB_CACHE.to_s)
      next true if line.include?("/.dhub/")
      next true if line.include?("/.metadata/")

      next false unless line.match?(DRXHUB_TAP_PATH_REGEX)
    ...
    backtrace.each do |line|
      next unless (match = line.match(DRXHUB_TAP_PATH_REGEX))

      require "tap"

      tap = Tap.fetch(match[:user], match[:repository])
      tap_message = "\nPlease report this issue to the #{tap.full_name} tap"
      tap_message += " (not Homebrew/brew or DinrusHub/homebrew-core)" unless tap.official?
    ...
    disable = true if disable_for_developers && DinrusHub::EnvConfig.developer?
    if disable || DinrusHub.raise_deprecation_exceptions?
      require "utils/github/actions"
      GitHub::Actions.puts_annotation_if_env_set(:error, message, file:, line:)
      exception = MethodDeprecatedError.new(message)
      exception.set_backtrace(backtrace)
      raise exception
    elsif !DinrusHub.auditing?
      opoo message
    end
  end
  ...
  def pretty_installed(formula)
    if !$stdout.tty?
      formula.to_s
    elsif DinrusHub::EnvConfig.no_emoji?
  ...
  def interactive_shell(formula = nil)
    unless formula.nil?
      ENV["DRXHUB_DEBUG_PREFIX"] = formula.prefix
      ENV["DRXHUB_DEBUG_INSTALL"] = formula.full_name
    end

    if Utils::Shell.preferred == :zsh && (home = Dir.home).start_with?(DRXHUB_TEMP.resolved_path.to_s)
  ...
  def safe_system(cmd, *args, **options)
    ...
    return if DinrusHub.system(cmd, *args, **options)
  ...
  def which_editor(silent: false)
    editor = DinrusHub::EnvConfig.editor
    ...
    unless silent
      opoo <<~EOS
        ...
        Рекомендуем установить переменную EDITOR или DRXHUB_EDITOR
  ...
  def exec_browser(*args)
    browser = DinrusHub::EnvConfig.browser
    browser ||= OS::PATH_OPEN if defined?(OS::PATH_OPEN)
    return unless browser

    ENV["DISPLAY"] = DinrusHub::EnvConfig.display

    with_env(DBUS_SESSION_BUS_ADDRESS: ENV.fetch("DRXHUB_DBUS_SESSION_BUS_ADDRESS", nil)) do
  ...
  # This is useful for installing a utility formula (e.g. `shellcheck` for `dhub style`)
  def ensure_formula_installed!(formula_or_name, reason: "", latest: false,
    ...
    unless formula.any_version_installed?
      ...
      safe_system DRXHUB_BREW_FILE, "install", "--formula", formula.full_name
    end

    if latest && !formula.latest_version_installed?
      ohai "Проводится апгрейд `#{formula.name}`#{reason}..."
      safe_system DRXHUB_BREW_FILE, "upgrade", "--formula", formula.full_name
    end
  ...
  def ensure_executable!(name, formula_name = nil, reason: "", latest: false)
    ...
    executable = [
      ...
      DRXHUB_PREFIX/"opt/#{formula_name}/bin/#{name}",
      DRXHUB_PREFIX/"bin/#{name}",
    ].compact.first
