...
# DinrusHub extends Ruby's `ENV` to make our code more readable.
...
module SharedEnvExtension
  ...
  def compiler
    @compiler ||= if (cc = @cc)
      ...
      compiler = fetch_compiler(cc, "DRXHUB_CC")
  ...
  def fortran
    ...
    # despite it often being the DinrusHub-provided one set up in the first call.
    ...
    if fc
      ...
      if (gfortran = which("gfortran", (DRXHUB_PREFIX/"bin").to_s))
        ohai "Using DinrusHub-provided Fortran compiler"
  ...
  def warn_about_non_apple_gcc(name)
    begin
      ...
      raise <<~EOS
        DinrusHub GCC requested, but formula #{e.name} not found!
      EOS
    end
    ...
    raise <<~EOS
      The requested DinrusHub GCC was not installed. You must:
        dhub install #{gcc_formula.full_name}
    EOS
  end
  ...
  sig { returns(Integer) }
  def make_jobs
    DinrusHub::EnvConfig.make_jobs.to_i
  end
