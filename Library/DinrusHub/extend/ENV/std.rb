...
module Stdenv
  ...
  def setup_build_environment(formula: nil, cc: nil, build_bottle: false, bottle_arch: nil, testing_formula: false,
    ...
    prepend_path "PATH", DRXHUB_SHIMS_PATH/"shared"
    ...
    if DRXHUB_PREFIX.to_s != "/usr/local"
      # /usr/local is already an -isystem and -L directory so we skip it
      self["CPPFLAGS"] = "-isystem#{DRXHUB_PREFIX}/include"
      self["LDFLAGS"] = "-L#{DRXHUB_PREFIX}/lib"
      # CMake ignores the variables above
      self["CMAKE_PREFIX_PATH"] = DRXHUB_PREFIX.to_s
    end

    frameworks = DRXHUB_PREFIX.join("Frameworks")
    ...
    begin
      ...
      # We don't care if our compiler fails to build the formula during `dhub test`.
  ...
  def determine_pkg_config_libdir
    PATH.new(
      DRXHUB_PREFIX/"lib/pkgconfig",
      DRXHUB_PREFIX/"share/pkgconfig",
