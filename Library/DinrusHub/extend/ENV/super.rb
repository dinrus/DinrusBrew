...
module Superenv
  ...
  # The location of DinrusHub's shims.
  #
  # @api public
  sig { returns(Pathname) }
  def self.shims_path
    DRXHUB_SHIMS_PATH/"super"
  end
  ...
  def setup_build_environment(formula: nil, cc: nil, build_bottle: false, bottle_arch: nil, testing_formula: false,
    ...
    self["DRXHUB_ENV"] = "super"
    self["MAKEFLAGS"] ||= "-j#{determine_make_jobs}"
    self["RUSTFLAGS"] = Hardware.rustflags_target_cpu(effective_arch)
    self["PATH"] = determine_path
    self["PKG_CONFIG_PATH"] = determine_pkg_config_path
    self["PKG_CONFIG_LIBDIR"] = determine_pkg_config_libdir || ""
    self["DRXHUB_CCCFG"] = determine_cccfg
    self["DRXHUB_OPTIMIZATION_LEVEL"] = "Os"
    self["DRXHUB_BREW_FILE"] = DRXHUB_BREW_FILE.to_s
    self["DRXHUB_PREFIX"] = DRXHUB_PREFIX.to_s
    self["DRXHUB_CELLAR"] = DRXHUB_CELLAR.to_s
    self["DRXHUB_OPT"] = "#{DRXHUB_PREFIX}/opt"
    self["DRXHUB_TEMP"] = DRXHUB_TEMP.to_s
    self["DRXHUB_OPTFLAGS"] = determine_optflags
    self["DRXHUB_ARCHFLAGS"] = ""
    self["DRXHUB_MAKE_JOBS"] = determine_make_jobs.to_s
    self["CMAKE_PREFIX_PATH"] = determine_cmake_prefix_path
    self["CMAKE_FRAMEWORK_PATH"] = determine_cmake_frameworks_path
    self["CMAKE_INCLUDE_PATH"] = determine_cmake_include_path
    self["CMAKE_LIBRARY_PATH"] = determine_cmake_library_path
    self["ACLOCAL_PATH"] = determine_aclocal_path
    self["M4"] = "#{DRXHUB_PREFIX}/opt/m4/bin/m4" if deps.any? { |d| d.name == "libtool" }
    self["DRXHUB_ISYSTEM_PATHS"] = determine_isystem_paths
    self["DRXHUB_INCLUDE_PATHS"] = determine_include_paths
    self["DRXHUB_LIBRARY_PATHS"] = determine_library_paths
    self["DRXHUB_DEPENDENCIES"] = determine_dependencies
    self["DRXHUB_FORMULA_PREFIX"] = @formula.prefix unless @formula.nil?
    ...
    # The DRXHUB_CCCFG ENV variable is used by the ENV/cc tool to control
  ...
  def cc=(val)
    self["DRXHUB_CC"] = super
  end

  sig { params(val: T.any(String, Pathname)).returns(String) }
  def cxx=(val)
    self["DRXHUB_CXX"] = super
  end
  ...
  def determine_library_paths
    ...
    if compiler.match?(GNU_GCC_REGEXP)
      ...
      # so that the linker will find those libs before any that may be linked in $DRXHUB_PREFIX/lib.
  ...
  def deparallelize(&block)
    ...
    old_make_jobs = delete("DRXHUB_MAKE_JOBS")
    self["DRXHUB_MAKE_JOBS"] = "1"
    if block
      begin
        yield
      ensure
        self["MAKEFLAGS"] = old_makeflags
        self["DRXHUB_MAKE_JOBS"] = old_make_jobs
      end
    end
  ...
  def O0(&block)
    if block
      with_env(DRXHUB_OPTIMIZATION_LEVEL: "O0", &block)
    else
      self["DRXHUB_OPTIMIZATION_LEVEL"] = "O0"
    end
  end
  ...
  def O1(&block)
    if block
      with_env(DRXHUB_OPTIMIZATION_LEVEL: "O1", &block)
    else
      self["DRXHUB_OPTIMIZATION_LEVEL"] = "O1"
    end
  end
  ...
  def O3(&block)
    if block
      with_env(DRXHUB_OPTIMIZATION_LEVEL: "O3", &block)
    else
      self["DRXHUB_OPTIMIZATION_LEVEL"] = "O3"
    end
  end
