...
module OnSystem
  ...
  def self.os_condition_met?(os_name, or_condition = nil)
    return DinrusHub::SimulateSystem.send(:"simulating_or_running_on_#{os_name}?") if BASE_OS_OPTIONS.include?(os_name)
    ...
    return false if DinrusHub::SimulateSystem.simulating_or_running_on_linux?

    base_os = MacOSVersion.from_symbol(os_name)
    current_os = if DinrusHub::SimulateSystem.current_os == :macos
      # Assume the oldest macOS version when simulating a generic macOS version
      # Version::NULL is always treated as less than any other version.
      Version::NULL
    else
      MacOSVersion.from_symbol(DinrusHub::SimulateSystem.current_os)
    end
    ...
    MacOSVersion::SYMBOLS.each_key do |os_name|
      base.define_method(:"on_#{os_name}") do |or_condition = nil, &block|
        ...
        @on_system_block_min_os = if or_condition == :or_older
          @called_in_on_system_block ? @on_system_block_min_os : MacOSVersion.new(DRXHUB_MACOS_OLDEST_ALLOWED)
        else
