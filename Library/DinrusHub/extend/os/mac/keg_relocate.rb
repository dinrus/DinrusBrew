  ...
  module Mac
    module Keg
      ...
      def loader_name_for(file, target)
        # Use @loader_path-relative install names for other DinrusHub-installed binaries.
        if ENV["DRXHUB_RELOCATABLE_INSTALL_NAMES"] && target.start_with?(DRXHUB_PREFIX)
      ...
      def fixed_name(file, bad_name)
        if bad_name.start_with? ::Keg::PREFIX_PLACEHOLDER
          bad_name.sub(::Keg::PREFIX_PLACEHOLDER, DRXHUB_PREFIX)
        elsif bad_name.start_with? ::Keg::CELLAR_PLACEHOLDER
          bad_name.sub(::Keg::CELLAR_PLACEHOLDER, DRXHUB_CELLAR)
      ...
      def prepare_relocation_to_locations
        ...
        perl_path = if brewed_perl || name == "perl"
          "#{DRXHUB_PREFIX}/opt/perl/bin/perl"
        ...
        if (openjdk = openjdk_dep_name_if_applicable)
          openjdk_path = DRXHUB_PREFIX/"opt"/openjdk/"libexec/openjdk.jdk/Contents/Home"
      ...
      CELLAR_RX = %r{\A#{DRXHUB_CELLAR}/(?<formula_name>[^/]+)/[^/]+}

      # Replace DRXHUB_CELLAR references with DRXHUB_PREFIX/opt references
      # if the Cellar reference is to a different keg.
      def opt_name_for(filename)
        return filename unless filename.start_with?(DRXHUB_PREFIX.to_s)
        return filename if filename.start_with?(path.to_s)
        return filename if (matches = CELLAR_RX.match(filename)).blank?

        filename.sub(CELLAR_RX, "#{DRXHUB_PREFIX}/opt/#{matches[:formula_name]}")
      end

      def rooted_in_build_directory?(filename)
        # CMake normalises `/private/tmp` to `/tmp`.
        # https://gitlab.kitware.com/cmake/cmake/-/issues/23251
        return true if DRXHUB_TEMP.to_s == "/private/tmp" && filename.start_with?("/tmp/")

        filename.start_with?(DRXHUB_TEMP.to_s) || filename.start_with?(DRXHUB_TEMP.realpath.to_s)
      end
    end
  end
end
