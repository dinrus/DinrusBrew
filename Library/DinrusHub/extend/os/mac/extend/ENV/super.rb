...
module Superenv
  class << self
    # The location of DinrusHub's shims on macOS.
    def shims_path
      DRXHUB_SHIMS_PATH/"mac/super"
    end
  ...
  def homebrew_extra_pkg_config_paths
    [Pathname("/usr/lib/pkgconfig"), Pathname("#{DRXHUB_LIBRARY}/DinrusHub/os/mac/pkgconfig/#{MacOS.version}")]
  end
  ...
  def libxml2_include_needed?
    ...
    return false if Pathname("#{self["DRXHUB_SDKROOT"]}/usr/include/libxml").directory?
  ...
  def homebrew_extra_isystem_paths
    paths = []
    paths << "#{self["DRXHUB_SDKROOT"]}/usr/include/libxml2" if libxml2_include_needed?
    paths << "#{self["DRXHUB_SDKROOT"]}/usr/include/apache2" if MacOS::Xcode.without_clt?
    paths << "#{self["DRXHUB_SDKROOT"]}/System/Library/Frameworks/OpenGL.framework/Versions/Current/Headers"
    paths
  end
  ...
  def homebrew_extra_library_paths
    ...
    if compiler == :llvm_clang
      paths << "#{self["DRXHUB_SDKROOT"]}/usr/lib"
      paths << Formula["llvm"].opt_lib.to_s
    end
    paths << "#{self["DRXHUB_SDKROOT"]}/System/Library/Frameworks/OpenGL.framework/Versions/Current/Libraries"
    paths
  end
  ...
  def homebrew_extra_cmake_library_paths
    [Pathname("#{self["DRXHUB_SDKROOT"]}/System/Library/Frameworks/OpenGL.framework/Versions/Current/Libraries")]
  end

  def homebrew_extra_cmake_frameworks_paths
    paths = []
    paths << "#{self["DRXHUB_SDKROOT"]}/System/Library/Frameworks" if MacOS::Xcode.without_clt?
    paths
  end
  ...
  def setup_build_environment(formula: nil, cc: nil, build_bottle: false, bottle_arch: nil, testing_formula: false,
    ...
    if is_xcode_sdk || MacOS.sdk_root_needed?
      DinrusHub::Diagnostic.checks(:fatal_setup_build_environment_checks)
      self["DRXHUB_SDKROOT"] = sdk.path if sdk
    end

    self["DRXHUB_DEVELOPER_DIR"] = if is_xcode_sdk
    ...
    self["DRXHUB_PREFER_CLT_PROXIES"] = "1"
