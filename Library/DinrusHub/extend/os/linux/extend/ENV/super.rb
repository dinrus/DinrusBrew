...
module Superenv
  ...
  def setup_build_environment(formula: nil, cc: nil, build_bottle: false, bottle_arch: nil, testing_formula: false,
    ...
    self["DRXHUB_OPTIMIZATION_LEVEL"] = "O2"
    self["DRXHUB_DYNAMIC_LINKER"] = determine_dynamic_linker_path
    self["DRXHUB_RPATH_PATHS"] = determine_rpath_paths(@formula)
    m4_path_deps = ["libtool", "bison"]
    self["M4"] = "#{DRXHUB_PREFIX}/opt/m4/bin/m4" if deps.any? { m4_path_deps.include?(_1.name) }
  end
  ...
  def determine_rpath_paths(formula)
    PATH.new(
      *formula&.lib,
      "#{DRXHUB_PREFIX}/opt/gcc/lib/gcc/current",
