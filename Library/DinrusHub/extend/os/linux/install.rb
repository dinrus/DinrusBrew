...
module DinrusHub
  module Install
    ...
    def self.check_cpu
      ...
      message = "Sorry, DinrusHub does not support your computer's CPU architecture!"
      if Hardware::CPU.ppc64le?
        message += <<~EOS
          For OpenPOWER Linux (PPC64LE) support, see:
            #{Formatter.url("https://github.com/homebrew-ppc64le/dhub")}
        EOS
      end
      abort message
    end
    ...
    def self.symlink_ld_so
      brew_ld_so = DRXHUB_PREFIX/"lib/ld.so"

      ld_so = DRXHUB_PREFIX/"opt/glibc/bin/ld.so"
      ...
      FileUtils.mkdir_p DRXHUB_PREFIX/"lib"
      FileUtils.ln_sf ld_so, brew_ld_so
    end
    private_class_method :symlink_ld_so

    def self.setup_preferred_gcc_libs
      gcc_opt_prefix = DRXHUB_PREFIX/"opt/#{OS::LINUX_PREFERRED_GCC_RUNTIME_FORMULA}"
      glibc_installed = (DRXHUB_PREFIX/"opt/glibc/bin/ld.so").readable?

      return unless gcc_opt_prefix.readable?

      if glibc_installed
        ld_so_conf_d = DRXHUB_PREFIX/"etc/ld.so.conf.d"
        ...
        ld_gcc_conf_content = <<~EOS
          # This file is generated by DinrusHub. Do not modify.
        ...
        if !ld_gcc_conf.exist? || (ld_gcc_conf.read != ld_gcc_conf_content)
          ...
          FileUtils.rm_f DRXHUB_PREFIX/"etc/ld.so.cache"
          system DRXHUB_PREFIX/"opt/glibc/sbin/ldconfig"
        end
      else
        odie "#{DRXHUB_PREFIX}/lib does not exist!" unless (DRXHUB_PREFIX/"lib").readable?
      end

      GCC_RUNTIME_LIBS.each do |library|
        gcc_library_symlink = DRXHUB_PREFIX/"lib/#{library}"
