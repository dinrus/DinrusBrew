      ...
      def locate(tool)
        ...
        @locate.fetch(tool) do |key|
          @locate[key] = if ::DevelopmentTools.needs_build_formulae? &&
                            (binutils_path = DRXHUB_PREFIX/"opt/binutils/bin/#{tool}").executable?
            binutils_path
          elsif ::DevelopmentTools.needs_build_formulae? &&
                (glibc_path = DRXHUB_PREFIX/"opt/glibc/bin/#{tool}").executable?
            glibc_path
          elsif (homebrew_path = DRXHUB_PREFIX/"bin/#{tool}").executable?
            homebrew_path
